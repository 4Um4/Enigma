...
class TestSocialDecayIncludesFear:
    ...
    def test_fear_decay_to_zero(self):
        ...
        npcs = [{
            "npc_id": "maid_lusya",
            "relationship_cache": {
                "player": {"trust": 30.0, "fear": 50.0, "base_trust": 50.0}
            },
    ...
    def test_trust_decay_still_works(self):
        ...
        npcs = [{
            "npc_id": "maid_lusya",
            "relationship_cache": {
                "player": {"trust": 30.0, "fear": 0.0, "base_trust": 50.0}
            },
            "base_values": {"player": 50.0},
        }]
        results = handler.handle(npcs, "test_campaign", current_tick=1)
...
class TestDirectiveNoFearOnSummon:
    ...
    def _make_npc_states(self, npc_id="npc_2", fear=0.5, trust=0.0):
        return [{"npc_id": npc_id, "social_stats": {"fear_of_player": fear, "trust": trust},
    ...
    def test_summon_generates_zero_fear(self):
        ...
        for d in social_deltas:
            assert d.payload.fear_delta == 0.0, f"MOVE (подзыв) не должен генерировать fear_delta, получено {d.payload.fear_delta}"
