...
from app.models.spatial_contracts import NodeRole, NodeRef
...
def test_runtime_applies_boundary_snap():
    ...
    assert entry.get("location_id") == "city_gate", (
        f"NPC должен быть в city_gate, а не в {entry.get('location_id')}"
    )
