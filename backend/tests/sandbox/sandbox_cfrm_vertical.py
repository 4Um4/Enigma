...
TAVERN_CLUSTERS = {
    "tavern:main_hall": ClusterDef(cluster_id="tavern:main_hall", boundary_cells=frozenset({"tavern:bar", "tavern:kitchen"})),
...
def simulate_tick(
    ...
    log.info(f"[CFRM] classify_event: {action} -> {result.axis.value} (confidence={result.confidence}, source={result.source.value})")
    ...
    if result.axis == CausalAxis.PHYSICAL:
        ...
        if "attack" in action: vectors.append(DisturbanceVector.MATTER)
    ...
    for entity_id in occupancy.entity_to_cluster.keys():
        ...
        if not entity_cluster: continue
        ...
        log.info(f"[{entity_id}] Phenomenon: '{phenomenon.perceived_archetype}' "
    ...
    log.info(f"[CFRM] LocalCausalSolver: {total_disturbances} disturbances -> {len(pressures)} phenomena -> {sum(1 for p in pressures.values() if p.fear > 0.1 or p.uncertainty > 0.1)} pressure")
...
def run_cfrm_sandbox() -> bool:
    ...
    assert p2.get("borko", PsychologicalPressure()).fear > p2.get("lucy", PsychologicalPressure()).fear, "Borko должен бояться сильнее (прямое попадание)"
    ...
    assert p4.get("player", PsychologicalPressure()).uncertainty == 0.18, "Неизвестное событие должно давать неуверенность 0.18 (с ослаблением)"
