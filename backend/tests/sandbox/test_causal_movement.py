...
class TestCausalOntology:
    ...
    def test_pressure_modifies_utility_not_commands(self):
        ...
        assert not hasattr(semantic_field, 'movement_intent'), "ОНТОЛОГИЯ НАРУШЕНА: Давление генерирует MovementIntent напрямую"
...
class TestSocialPhysics:
    ...
    def test_speech_act_generates_pressure_landscape(self, minimal_world):
        ...
        for delta in deltas:
            assert "MovementIntent" not in str(delta.payload), "ФИЗИКА ВЛАСТИ НАРУШЕНА: Давление напрямую создало MovementIntent (Aggro Controller)"
    ...
    def test_legitimacy_affects_pressure(self, minimal_world):
        ...
        assert emotion_delta.payload.stress_delta < 15.0, "ФИЗИКА ВЛАСТИ НАРУШЕНА: Давление не зависит от легитимности/страха"
