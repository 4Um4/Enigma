...
async def run_player_stress_test():
    ...
    try:
        ...
        for i, action_text in enumerate(TEST_ACTIONS, 1):
            req = ChatTurnRequest(
                world_id="manual", campaign_id=campaign_id, location=location,
            ...
            try:
                ...
                if result is None:
                    ...
                    for err in errors_in_tick:
                        ...
                        if "NameError" in err: error_stats["NameError"] = error_stats.get("NameError", 0) + 1
                        elif "AttributeError" in err: error_stats["AttributeError"] = error_stats.get("AttributeError", 0) + 1
                        else: error_stats["OtherError"] = error_stats.get("OtherError", 0) + 1
