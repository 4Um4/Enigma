...
def causal_class(intent: IntentSemanticField) -> tuple:
    ...
    THREATENING_ACTS = {SpeechAct.THREAT, SpeechAct.ACCUSATION, SpeechAct.INSULT}
    return (
        intent.action.value if intent.action else "UNCERTAIN",
        intent.social_intent.value if intent.social_intent else "neutral",
        intent.target or "",
        bool(intent.proposition),
        intent.speech_act in THREATENING_ACTS if intent.speech_act else False,
    )
