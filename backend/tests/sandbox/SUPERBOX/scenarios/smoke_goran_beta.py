...
POS_THIEF = (11.5, 11.0)
POS_PLAYER = (8.59, 0.81)  # D0/Р-В: луч «вор→Горан→игрок»: 5.5 м от Горана
# (normal-слух 6.0, запас 0.5) и 10.6 м от вора — выше порога прямого наблюдения
# _OBSERVATION_SIGHT_RADIUS=10.0 (ADR-O-360, запас 0.6). Игрок слышит warn
# Горана, но НЕ видит кражу сам — канал свидетельства чист в обоих плечах.
# (7.5, 4.5) ломала CONTROL: 7.63 ≤ 10 → игрок наблюдал THEFT напрямую.
...
def run_scenario(goran_in_los: bool, arm: str) -> dict:
    ...
    _diag_psyche = _states_map(world).get(THIEF, {}).get("psyche", {})
    print(f"[DIAG-WILL] pre-tick psyche.state={_diag_psyche.get('state', 'MISSING')}")
