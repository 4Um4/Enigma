...
def test_brave_npc_gets_annoyed(subscriber):
    ...
    assert 0 < payload.compliance_bias_delta < 0.5, "БАГ: Храбрый NPC слишком подчиняется или наоборот сопротивляется подходу"
    assert 0 < payload.aggression_inhibition_delta < 0.5, "БАГ: Храбрый NPC полностью подавляет агрессию или наоборот слишком её разблокирует"
    assert payload.recent_directive_data["is_obedience"] is False, "БАГ: Флаг подчинения установлен для храброго NPC"
    assert payload.recent_directive_data["interrupts_routine"] is True, "БАГ: Храбрый NPC не прерывает бытовуху для реакции"