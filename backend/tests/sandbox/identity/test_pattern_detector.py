...
from app.services.npc.pattern_detector import PatternDetector, MIN_EVENTS_FOR_PERSISTENCE


def _make_event(tick: int, source: str, effect: float, weight: float = 1.0, event_type: str = "test") -> TraitDriftEvent:
...
def test_4_l1_append_only_independence():
    ...
    detector.detect(events, chronicle=mock_chronicle) if 'chronicle' in detector.detect.__code__.co_varnames else detector.detect(events)
...
def test_B_observation_weight_semantics():
    ...
    events_A = [_make_event(tick=i, source="player", effect=-1.0, weight=1.0) for i in range(MIN_EVENTS_FOR_PERSISTENCE)]
    
    # Группа B: Слабое наблюдение (weight = 0.1)
    events_B = [_make_event(tick=i, source="player", effect=-1.0, weight=0.1) for i in range(MIN_EVENTS_FOR_PERSISTENCE)]
