...
def npc_threatened_schedule_change():
    return {
        "id": "guard_1",
        "tier": "minor",
        "perceptual_kernel": {"recent_directive": {"source": "player", "salience": 0.9, "interrupts_routine": True}}, # ADR-056: Attention Capture
        "position": "barracks",
        "location": "tavern_silver_wolf",
        "routine": {
            "current": "sleep",
...
def test_schedule_generates_intent_when_calm(engine, npc_calm_schedule_change):
    ...
    with patch.object(engine, '_resolve_position', return_value=("tavern_silver_wolf", "gate", "patrolling")):
        changes, intents = engine._simulate_minor(npc_calm_schedule_change, current_time="08:00", tick=MINOR_TICK_INTERVAL + 1)
...
def test_schedule_blocked_by_cognitive_guard(engine, npc_threatened_schedule_change):
    ...
    with patch.object(engine, '_resolve_position', return_value=("tavern_silver_wolf", "gate", "patrolling")):
        changes, intents = engine._simulate_minor(npc_threatened_schedule_change, current_time="08:00", tick=MINOR_TICK_INTERVAL + 1)
    
    assert intents is None or len(intents) == 0, f"БАГ: Напуганный NPC (threat=0.9) всё равно идёт по расписанию! Intents: {intents}"
