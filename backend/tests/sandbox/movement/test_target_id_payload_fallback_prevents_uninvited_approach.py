...
def test_target_id_payload_fallback_prevents_uninvited_approach():
    ...
    relevance_uninvited = hub._context_relevance(
        intent=Intent.TALK.value, 
        event=event, 
        state=state_uninvited,
        personality=None
    )
    
    # VERDICT: Целевой NPC получает бонус (больше базового 0.5), незваный — нет
    assert relevance_targeted > _BASE_RELEVANCE, f"Целевой NPC (guard_1) НЕ получил бонус! Relevance={relevance_targeted}, expected > {_BASE_RELEVANCE}"
    assert relevance_uninvited == _BASE_RELEVANCE, f"Незваный NPC (guard_2) получил бонус! DOUBLE TRUTH! Relevance={relevance_uninvited}, expected {_BASE_RELEVANCE}"
