...
def test_belief_modifies_decision():
    ...
    # 3. Патчим внутренности DecisionHub, чтобы изолировать логику модификаторов.
    # Мы заставляем _get_possible_intents вернуть ровно два интента: APPROACH и FLEE.
    # Мы заставляем _score_all вернуть им одинаковые базовые скоры (1.0).
    # Таким образом, единственным фактором, определяющим выбор, становится drive_modifiers.
    with patch(
        ...
        DecisionHub, "_get_possible_intents", return_value=["APPROACH", "FLEE"]
    ), patch.object(
        DecisionHub,
        "_score_all",
        return_value=({"APPROACH": 1.0, "FLEE": 1.0}, {}),
    ):
        hub = DecisionHub(rng=MagicMock())

        # Baseline: без модификаторов FLEE и APPROACH равны (1.0).
        # DecisionHub должен выбрать APPROACH (обычно первый или по приоритету).
    ...
    assert decision_mod.decision.intent == "FLEE", (
    ...
    assert decision_base.decision.intent != "FLEE", (
