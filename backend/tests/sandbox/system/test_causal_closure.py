...
def _print_causal_trace(tick: int, npc_id: str, pk: PerceptualKernel, ctx: DecisionContext, result_intent: str, result_score: float, competing: str, comp_score: float):
    ...
    print(f"├── [PRESSURE] aggression_sup={ctx.deformation.aggression_suppression:.2f} initiative_sup={ctx.deformation.initiative_suppression:.2f}")
...
def test_full_obedience_closure(fearful_profile, idle_event):
    ...
    # Трассировка
    _print_causal_trace(
        tick=1, npc_id=state.npc_id, pk=state.perceptual_kernel, ctx=decision_ctx,
        result_intent=result.intent.value, result_score=result.score,
        competing="ATTACK", comp_score=0.0 # ATTACK заблокирован
    )
...
def test_brave_resistance_closure(brave_profile, idle_event):
    ...
    state.perceptual_kernel = PerceptualKernel(
        threat_gradient=0.1,
        aggression_inhibition=0.1,
        initiative_suppression=0.05,
        compliance_bias=0.05
    )
    ...
    _print_causal_trace(
        tick=1, npc_id=state.npc_id, pk=state.perceptual_kernel, ctx=decision_ctx,
        result_intent=result.intent.value, result_score=result.score,
        competing="APPROACH", comp_score=0.0 # Моковое значение для отчета
    )