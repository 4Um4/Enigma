...
def test_avatar_to_prompt_includes_life_status_alive():
    ...
    assert result.get("life_status") == "ALIVE", "ALIVE avatar must have life_status=ALIVE in pdata"
...
def test_avatar_to_prompt_includes_life_status_dead():
    ...
    assert result.get("life_status") == "DEAD", "DEAD avatar must have life_status=DEAD in pdata"
...
def test_avatar_to_prompt_life_status_fallback_when_no_body_state():
    ...
    assert result.get("life_status") == "ALIVE", "Missing body_state must fallback to ALIVE"
...
def test_dm_contract_dead_player_gets_death_block():
    ...
    contract = agent._build_contract(
        location="tavern",
        actions_str="Player: последнее действие",
        rules_result={},
        npc_result={},
        world_result={},
        context=context,
    ...
    assert "СМЕРТЬ ИГРОКА" in contract.user_prompt, "DEAD player must trigger death scene block"
    assert "необратимо" in contract.user_prompt, "Death block must contain irreversible clause"
    assert "МЁРТВ" in contract.user_prompt, "Player state block must show DEAD marker"
...
def test_dm_contract_alive_player_no_death_block():
    ...
    contract = agent._build_contract(
        location="tavern",
        actions_str="Player: осматривает комнату",
        rules_result={},
        npc_result={},
        world_result={},
        context=context,
    ...
    assert "СМЕРТЬ ИГРОКА" not in contract.user_prompt, "ALIVE player must NOT get death scene block"
