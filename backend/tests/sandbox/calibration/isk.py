...
def run_perturbation_test(
    engine: PhasePhysicsEngine, base_csv: CausalStateVector, perturbation_amplitude: float = 0.1, num_samples: int = 100
) -> dict:
    """
    ...
    return {"mu_delta_g": np.mean(delta_g_norms), "sigma_delta_g": np.std(delta_g_norms)}
