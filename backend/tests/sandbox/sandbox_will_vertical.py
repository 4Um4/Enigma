...
PSYCHE_STOIC = {
    "identity_rigidity": 0.9, "fear": 0.1, "conviction": 0.8, 
    "shame": 0.2, "aggression": 0.3, "curiosity": 0.1, "willpower": 0.9
}

PSYCHE_FEARFUL = {
    "identity_rigidity": 0.2, "fear": 0.9, "conviction": 0.3, 
    "shame": 0.6, "aggression": 0.1, "curiosity": 0.2, "willpower": 0.2
}


# ── Ядро Симуляции ──────────────────────────────────────────────────────

def simulate_will_tick(
    tick_num: int, 
    action: str, 
    ...
    log.info(f"[WILL] Intent '{action}' -> Pressure: violence={pressure.violence:.2f}, self_risk={pressure.self_risk:.2f}, identity_deviation={pressure.identity_deviation:.2f}")
...
def run_will_sandbox() -> bool:
    ...
    assert p2.state in (WillState.RELUCTANT, WillState.DISTRESSED, WillState.PANICKED), "Стоик должен сопротивляться насилию (даже через панику)"
    ...
    assert p4.state in (WillState.PANICKED, WillState.DISTRESSED, WillState.DISSOCIATING), "Трус должен паниковать при угрозах"
    ...
    assert p5.state in (WillState.DISSOCIATING, WillState.BROKEN, WillState.PANICKED), "Трус должен ломаться при насилии"
