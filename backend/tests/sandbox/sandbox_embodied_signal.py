...
def test_embodied_signal_flow():
    ...
    scene_state = {"npc_positions": {npc_id: {"local_position": [10, 10]}}, "active_traversals": {}}
    ...
    trace = traces[0]
    print(
        f"[TRACE_STEP] Trace for {trace.npc_id}: instability={trace.locomotion_instability:.2f}, rigidity={trace.posture_rigidity:.2f}, is_shaking={trace.is_shaking}, is_frozen={trace.is_frozen}"
    )
    ...
    for t_dict in dm_traces:
        _symptoms = []
        if t_dict.get("is_shaking"):
            _symptoms.append("дрожит")
        if t_dict.get("is_frozen"):
            _symptoms.append("окаменел")
        if t_dict.get("locomotion_instability", 0) > 0.3:
            _symptoms.append("покачивается")
        if t_dict.get("posture_rigidity", 0) > 0.5:
