...
def test_directive_pressure_materializes_traversal():
    ...
    pressure_frame_id = pressure_probe.observe_pressure(tick_1, "thief_shadow", "DIRECTIVE_OBEDIENCE", fear_pressure, speech_frame_id)
    ...
    if new_approach_score > 0.5:
        utility_probe.observe_utility_shift(tick_2, "thief_shadow", "APPROACH_PLAYER", base_approach_score, new_approach_score, pressure_frame_id)
    ...
    if dist > 0.1: # Защита от микро-прыжков
        ...
        traversal_probe.observe_traversal_started(
            tick=tick_3, entity_id="thief_shadow", from_node=from_node, 
            to_node=target_node, duration=duration, parent_id=goal_frame_id
        )
