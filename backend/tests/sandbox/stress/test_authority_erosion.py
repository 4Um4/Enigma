...
def test_cumulative_strain_erosion(trace):
    ...
    for tick in range(1, 6):
        ...
        if tick < 5:
            ...
            assert "ATTACK" not in decision_ctx.compression.constraints or decision_ctx.compression.constraints["ATTACK"] > 0.0, \
...
def test_resistance_recovery_balance(trace):
    ...
    for tick in range(1, 4):
        ...
        trace.observe(
            tick, "RECOVERY", "npc_target", "kernel_decay", {
                "compliance_bias": round(kernel.compliance_bias, 2),
