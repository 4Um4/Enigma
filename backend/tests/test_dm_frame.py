...
def make_decision(
    npc_id: str, intent: Intent = Intent.IDLE, stress: float = 0.0, fear: float = 0.0, distance: float = 5.0, **kwargs
) -> DecisionResult:
...
class TestDMFrameFocusSplit:
    ...
    def test_five_npcs_two_focus_three_background(self):
        ...
        decisions = [make_decision(f"npc_{i}", stress=i * 0.1, distance=10.0 - i) for i in range(5)]
    ...
    def test_focus_npcs_have_higher_salience(self):
        ...
        decisions = [make_decision(f"npc_{i}", stress=i * 0.15, distance=10.0 - i) for i in range(4)]
...
class TestPromptBlockFormatting:
    ...
    def test_background_npcs_as_list(self):
        ...
        decisions = [make_decision(f"bg_{i}", stress=0.01, distance=10.0) for i in range(3)]
