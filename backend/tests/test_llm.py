...
try:
    from app.services.llm import get_router, initialize_router, Capability
...
def test_multimodel():
    ...
    try:
        ...
        print(f"[WARN] Could not read ports, using 8080: {str(e).encode('utf-8', errors='replace').decode('utf-8', errors='ignore')}")
        llm_port = 8080
    ...
    for agent_name in agents:
        ...
        try:
            ...
            print(f"   Response: {response[:200].encode('utf-8', errors='replace').decode('utf-8', errors='ignore')}...")
        except Exception as e:
            print(f"   Error: {e}")
