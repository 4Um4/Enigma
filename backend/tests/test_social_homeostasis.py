...
class FakeSpatialQuery:
    ...
    def distance(self, a: str, b: str) -> float:
        if a == "npc_1" and b == "npc_2":
            return 5.0
        if a == "npc_1" and b == "npc_3":
            return 15.0
        return 999.0
...
def test_social_input_projector_generates_ema_deltas():
    ...
    event = make_event_dto(
        EventType.NPC_SPOKE, source="npc_1", payload={"speaker_id": "npc_1", "listener_ids": ["npc_2"]}
    )
...
def test_social_target_resolver_picks_nearest_when_hungry():
    ...
    state = types.SimpleNamespace(
        ...
        relationship_cache={},
        perceptual_kernel=types.SimpleNamespace(
            threat_gradient=0.0, uncertainty=0.0, anomaly_score=0.0, somatic_urgency=0.0
        ),
    ...
    talk_target = hub._resolve_target(
        intent=Intent.TALK.value, event=event, state=state, spatial_query=spatial, all_npc_ids=npc_ids
    )
