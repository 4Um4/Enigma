...
class TestPatchB1_FrontendAuthority:
        ...
        for node in ast.walk(tree):
            ...
            if isinstance(node, ast.AugAssign) and isinstance(node.op, ast.Add):
                if isinstance(node.target, ast.Attribute) and \
                   ...
                   node.target.attr == 'game_time_seconds':
                    mutations.append(node.lineno)
                    
        assert len(mutations) == 0, \
            f"game_time_seconds mutation (+=) found in game_screen at lines: {mutations}"
    ...
    def test_no_dialog_journal_append(self):
        screen_path = Path(__file__).resolve().parents[2] / "frontend" / "game_screen.py"
        source = screen_path.read_text(encoding="utf-8")
        assert "dialog_journal.append" not in source, \
            "dialog_journal local append found (B1.3-FIX not applied)"
...
class TestPatchB2_SpatialOracleNoSilentFailure:
    ...
    def test_no_silent_pass_in_bridge(self):
        ...
        assert "except Exception:\n pass" not in source, \
            "Silent pass found in game_loop_bridge (B2-FIX not applied)"

    def test_logger_warnings_present(self):
        bridge_path = Path(__file__).resolve().parents[2] / "frontend" / "game_loop_bridge.py"
        source = bridge_path.read_text(encoding="utf-8")
        assert "SPATIAL_ORACLE" in source, \
            "SPATIAL_ORACLE logging not found (B2-FIX not applied)"
...
class TestPatchC1_NoSilentSuppress:
    ...
    def test_no_suppress_exception_in_game_screen(self):
        ...
        lines = [l for l in source.splitlines() if "contextlib.suppress(Exception)" in l and not l.strip().startswith("#")]
        assert len(lines) == 0, \
            f"suppress(Exception) found in game_screen: {lines}"
...
class TestPatchC2_NoDuplication:
    ...
    def test_minimal_registry_has_find_chunks(self):
        ...
        assert "def find_chunks" in source, \
            "_MinimalFrontendRegistry.find_chunks not found (C2-FIX not applied)"