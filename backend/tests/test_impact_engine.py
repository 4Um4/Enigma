...
def _make_snapshot(
    npc_id: str = "npc_1", dexterity: float = 10.0, pain: float = 0.0, fatigue: float = 0.0, blood_loss: float = 0.0
) -> NPCStateSnapshot:
...
def _make_intent(
    ...
    return ImpactIntentDTO(
        actor_id=actor_id, target_id=target_id, damage_type=damage_type, target_zone=target_zone, force=force
    )
