...
# Основные сущности: test_p1_*, NPCS
import copy
...
def harness():
    ...
    _h.new_game()
    # Прогрев: inspect_npc читает LifeEngine-кэш — пуст до первого тика
    _h.advance_ticks(3)
    yield _h
    _h.dispose()
...
def test_p1_reload_sqlite_same_knowledge(harness):
    ...
    memories_batch -> load_event_memories) без потери secret_id."""
...
def test_p1_old_save_loads(harness):
    """ТЗ-P1: старый сейв (без secret_id) грузится (None-дефолт); сеялка восполняет."""
