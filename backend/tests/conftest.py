...
def initialized_model_pool():
    ...
    test_configs = [
        ModelConfig(
            ...
            name="Test Qwen",
            provider_type=ProviderType.LLAMA_CPP,
            path="/test/path/Qwen2.5-7B-Instruct-abliterated-v2.Q5_K_M.gguf",
        ),
    ]
...
def mock_model_router(monkeypatch):
    ...
    def mock_request(self, capability, prompt, params=None, system_prompt=None):
        return {"dm_response": f"Mock narrative for {capability}", "npc_reactions": [], "world_changes": []}
...
def test_environment(monkeypatch):
    ...
    monkeypatch.setattr(
        "app.core.config.settings.model_qwen_7b_path", MODELS_DIR / "Qwen2.5-7B-Instruct-abliterated-v2.Q5_K_M.gguf"
    )
