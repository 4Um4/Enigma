...
class Need:
    ...
    def effective_decay_rate(self) -> float:
        ...
        return self.decay_rate if self.decay_rate > 0 else NEED_DECAY_RATES.get(self.need_type, 0.01)
...
class Transaction:
    ...
    def to_dict(self) -> Dict:
        return {
            ...
            "duration_ticks": self.duration_ticks,
            "reason": self.reason,
            "need_satisfied": self.need_satisfied.value if self.need_satisfied else None,
            "tick": self.tick,
...
class EconomicProfile:
    ...
    def calculate_selling_price(
        ...
        final_price = base * inflation_factor * local_factor * trust_factor * urgency_factor
    ...
    def satisfy_need(self, need_type: NeedType) -> None:
        ...
        self.base_needs = [
            Need(
                ...
                budget_share=need.budget_share,
                skill_required=need.skill_required,
                neglected_ticks=0 if need.need_type == need_type else need.neglected_ticks,
                decay_rate=need.decay_rate,
            )
            for need in self.base_needs
        ]
    ...
    def to_dict(self) -> Dict:
        return {
            ...
            "base_needs": [
                {"need_type": n.need_type.value, "base_urgency": n.base_urgency, 
                 "budget_share": n.budget_share, "skill_required": n.skill_required,
                 "neglected_ticks": n.neglected_ticks, "decay_rate": n.decay_rate}
                for n in self.base_needs
            ],
            "current_employer": self.current_employer,
            "employment_remaining": self.employment_remaining,
            "obligations": [
                {"obligation_type": o.obligation_type, "amount": o.amount,
                 "due_in_ticks": o.due_in_ticks, "penalty_per_tick": o.penalty_per_tick,
