...
from app.domain.intent_profile import (
    ...
    EmotionalVector, ConfidenceVector
)
...
class IntentCompressor:
    ...
    async def compress(self, raw_text: str, scene_context: Dict[str, Any]) -> IntentSemanticField:
    ...
    def _fast_path_parse(self, raw_text: str) -> Optional[IntentSemanticField]:
        ...
        if PYMORPHY_AVAILABLE:
            ...
            for token in tokens_raw:
                ...
                if parsed and parsed[0].tag.POS == 'NOUN' and parsed[0].normal_form not in _ACTION_LEMMAS_FLAT:
    ...
    async def _slow_path_parse(self, raw_text: str, scene_context: Dict[str, Any]) -> IntentSemanticField:
        ...
        if llm_response is None:
            return IntentSemanticField(
                ...
                raw_text=raw_text,
                confidence=ConfidenceVector(parse=0.1, target=0.0, emotion=0.0, action=0.1),
        ...
        try:
            ...
            return IntentSemanticField(
                action_type=ActionType.UNCERTAIN,
                raw_text=raw_text,
                confidence=ConfidenceVector(parse=0.3, target=0.1, emotion=0.1, action=0.3),
                ambiguity=SemanticAmbiguity.AMBIGUOUS
            )