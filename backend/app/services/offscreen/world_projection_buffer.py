    ...
    def project(
        ...
        for event in significant_events:
            if event.get("type") in ("combat", "death", "steal"):
                projections.append(WorldProjectionEvent(
        ...
        _prev_map = {
            (n.get("npc_id") or n.get("id")): n for n in previous_npcs_raw if n.get("npc_id") != "player"
        }
        ...
        for npc_dict in all_npcs_raw:
            ...
            if current_bs.get("blood_loss", 0) > 0.5 and prev_bs.get("blood_loss", 0) <= 0.5:
                projections.append(WorldProjectionEvent(
                    event_id=f"proj_{uuid.uuid4().hex[:8]}",
                    tick=tick,
