...
class NpcTickPipeline:
    ...
    def run(
        ...
        for npc in _npcs_to_process:
            ...
            # Линейный по urgency; только production-рождённые желания
            # (desire_generator Фазы 0); предмет — рыночный (GOODS_PRICES).
            # OFF (WORK_ENABLED, default) = побитово прежний блок (L-W5).
            _work_trade_pressure: float = 0.0
            from app.services.economy.work_orders import work_enabled
            ...
            if work_enabled():
                ...
                for _d in npc.get("desires") or []:
                    ...
                    if (
                        _subj in GOODS_PRICES
                        and _urg >= 0.5
                        and _subj in {
                            _s for (_s, _t) in _NEED_TO_DESIRE.values()
                        }
                    ):
