...
def _convert_origin_events(origin_list: List[Dict], npc_id: str) -> Tuple[Any, ...]:
    ...
    for _d in origin_list:
        ...
        _mem = EventMemory(
            ...
            secret_id=_d.get("secret_id"),  # M1/P2 (Q1=MAP): origin — авторитет идентичности факта
        )
        _result.append(_mem)
...
def _seed_canon_secret_memories(npc_id: str, cache: Tuple[Any, ...]) -> Tuple[Any, ...]:
    ...
    initial_holders). Дедуп по secret_id — идемпотентность. Формат —
    паттерн _convert_origin_events (day=-1000, decay 0.001). Q1=MAP (Мастер):
    origin-конфиг — авторитетный носитель идентичности факта; сеялка =
    fill-the-gaps. Рассинхроны канон↔конфиг — зона P2."""
