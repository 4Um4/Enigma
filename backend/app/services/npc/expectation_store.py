...
class ExpectationStore:
    ...
    def get_expectation(self, npc_id: str, source_id: str) -> Expectation:
        ...
        try:
            ...
            cur = conn.cursor()
            cur.execute(
                "SELECT expected_reward, expected_threat, confidence FROM npc_expectations WHERE npc_id=? AND source_id=?",
                (npc_id, source_id),
            )
            row = cur.fetchone()
            conn.close()
            if row:
                exp = Expectation(
                    source_id=source_id,
                    expected_reward=row[0],
                    expected_threat=row[1],
                    confidence=row[2],
                )
                self._cache[key] = exp
                return exp
    ...
    def update_expectation(
        ...
        ) * exp.expected_reward + self._LEARNING_RATE * actual_reward
        exp.expected_threat = (
            1 - self._LEARNING_RATE
        ) * exp.expected_threat + self._LEARNING_RATE * actual_threat
