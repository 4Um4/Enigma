...
class PEModifierResolver:
    ...
    def resolve(self, expectation: Expectation) -> Dict[str, float]:
        ...
        _reward_mod = math.tanh(abs(expectation.expected_reward) * 2.0) * math.copysign(
            1, expectation.expected_reward
        )
        ...
        if _threat_mod > 0:
            ...
            mods["ATTACK"] = min(
                self.MAX_PE_INF, _threat_mod * 0.2
            )  # Лёгкая превентивная агрессия
