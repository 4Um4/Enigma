...
def _resolve_contact(
    ...
    dodge_chance = (_abilities.get("dexterity", 10.0) + _modifiers.get("dexterity", 0.0)) / 100.0
    ...
    effective_dodge = max(0.0, dodge_chance - pain_penalty - fatigue_penalty - shock_penalty)
...
def resolve_physical_impact(
    ...
    structural_damage = _calculate_tissue_damage(intent.force, contact, intent.damage_type)
    ...
    deltas.append(StateDeltas(
        ...
        domain=DeltaDomain.PHYSIOLOGY,
        payload=PhysiologyPayload(
            fatigue_delta=5.0 + (structural_damage * 0.1)
        ),
        source="impact_resolution"
    ))
