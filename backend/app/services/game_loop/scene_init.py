...
def _update_player_position(
    ...
    if player_position is None:
        # ENIGMA SELF-HEALING (Level 0): Логируем, если координат нет, но не крашим init (игрок может заспавниться позже).
...
def _inject_spatial_service(
    life_engine: Any, campaign_id: str, scene_state: dict
) -> None:
...
def _check_economy_tracker(loop: Any, campaign_id: str) -> None:
    ...
    if inc_sat or soc_sat:
        logger.warning(
            f"[ECO_TRACKER] day_end: income={inc_sat} social={soc_sat} satisfied"
        )
...
def _load_or_create_scene(
    ...
    campaign_id: str,
    location: str,
    preserved_game_time: float | None,
    campaign_state: Any,
) -> dict:
...
def ensure_scene_initialized(loop: Any, campaign_id: str) -> dict:
    ...
    if scene_state is None:
        time_of_day = (
            campaign_state.metadata.get("time_of_day", "12:00")
            if campaign_state
            else "12:00"
        )
