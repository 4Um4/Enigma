...
try:
    ...
    from backend.app.services.verbalization.prompt_loader import get_prompt_loader
...
def build_npc_core_data(ctx: VerbalizationContext) -> dict:
    ...
    return {
        ...
        "voice_profile": ctx.voice_profile or "обычная речь",
        ...
        # Передаем флаг мата отдельно, чтобы шаблон Jinja2 сам решил, как это оформить
        "allow_profanity": ctx.content_profile.profanity_level > 0
    }
