...
class SceneStateManager:
    ...
    def get_scene_state_uncached(self, campaign_id: str, location_id: str) -> dict | None:
        ...
        if not isinstance(scene, dict):
            logger.warning(f"[SCENE_CONTRACT] get_scene_state_uncached: тип={type(scene).__name__}, ожидается dict")
            return None
    ...
    def get_scene_state(self, campaign_id: str, location_id: str) -> dict | None:
        ...
        if self._tick_locked and self._tick_campaign_id == campaign_id and self._tick_scene is not None:
            return self._tick_scene
        ...
        # P0 FIX (S71): SceneState Contract — reject non-dict
        if not isinstance(scene, dict):
            logger.warning(f"[SCENE_CONTRACT] get_scene_state: тип={type(scene).__name__}, ожидается dict")
            return None
        ...
        return {
            "tavern_silver_wolf": {
                ...
                "npc_defaults": {
                    ...
                    "guard_borko": {
                        "position": "corner_table", "activity": "drinking", "visible": True
                    },
                    "blacksmith_orm": {
                        "position": "fireplace", "activity": "drinking", "visible": True
                    },
                    "merchant_goran": {
                        "position": "right_table", "activity": "haggling", "visible": True
                    },
                },
                        ...
                        try:
                            ...
                            if node := svc.get_node(change.value) or svc.get_node(f"{target_loc}:{change.value}"):
                                ...
                                if exact_xy and isinstance(exact_xy, (tuple, list)) and len(exact_xy) == 2:
                                    entry["local_position"] = {"x": float(exact_xy[0]), "y": float(exact_xy[1])}
                                else:
                                    entry["local_position"] = {"x": node.x, "y": node.y}
                                
                                from_xy = entry.get("local_position", {"x": 0.0, "y": 0.0})
                                if not isinstance(from_xy, dict): from_xy = {"x": 0.0, "y": 0.0}
                                
                                _active_travs = scene_state.get("active_traversals", {})
                                if change.target not in _active_travs or _active_travs[change.target].get("status") != "MOVING":
                                    import math
                                    from app.domain.traversal_schema import build_traversal_dict
                                    speed = 2.0
                                    current_tick = scene_state.get("tick", 0)
                                    _waypoints = [[from_xy.get("x", 0.0), from_xy.get("y", 0.0)], [entry["local_position"]["x"], entry["local_position"]["y"]]]
                                    dist = math.hypot(_waypoints[0][0] - _waypoints[1][0], _waypoints[0][1] - _waypoints[1][1])
        ...
        for npc_id, entry in npc_positions.items():
            ...
            if npc_id in active_traversals and active_traversals[npc_id].get("status") == "MOVING":
                ...
                if len(wp) >= 2:
                    ...
                    _trav_prog = min(1.0, max(0.0, (current_tick - _trav_started) / _trav_dur)) if _trav_dur > 0 else 1.0
                    _num_seg = len(wp) - 1
