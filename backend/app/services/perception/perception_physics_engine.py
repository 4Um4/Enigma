...
class PerceptionPhysicsEngine:
    ...
    def compute_relation(
        self, observer_id: str, target_id: str, spatial_query: Any, scene_state: Dict[str, Any]
    ...
    def filter_manifestation(
        ...
        if visibility > 0.1:
            ...
            if resolution >= self.MIN_RESOLUTION_FOR_MICRO:
                signals.extend(
                    self._filter_micro_expression(
                        manifest.micro_expression,
                        relation,
                        target_id,
                        current_tick,
                        visibility,
        ...
        if audibility > 0.1:
            signals.extend(
                self._filter_voice(
                    manifest.voice, relation, target_id, current_tick, audibility
                )
            )
            signals.extend(
                self._filter_breathing(
                    manifest.breathing, relation, target_id, current_tick, audibility
                )
            )
    ...
    def _filter_body(
        ...
        if body.standing_balance < 0.9 and resolution > 0.5:
            conf = self._compute_confidence(
                visibility, resolution, 1.0 - body.standing_balance
            )
            if conf > 0.2:
                signals.append(
                    self._make_signal(
    ...
    def _filter_micro_expression(
        ...
        if micro.pupil_dilation > 0.1:
            conf = self._compute_confidence(
                visibility, resolution, micro.pupil_dilation
            )
            if conf > 0.2:
                signals.append(
                    self._make_signal(
        ...
        if abs(micro.brow_position) > 0.1:
            conf = self._compute_confidence(
                visibility, resolution, abs(micro.brow_position)
            )
            if conf > 0.2:
                signals.append(
                    self._make_signal(
    ...
    def _filter_breathing(
        ...
        if breathing.rate > 16.0:
            conf = self._compute_confidence(
                audibility, 1.0, (breathing.rate - 16.0) / 10.0
            )
            if conf > 0.2:
                signals.append(
                    self._make_signal(
    ...
    def _filter_movement(
        ...
        if movement.coordination < 0.9 and visibility > 0.1:
            conf = self._compute_confidence(
                visibility, 0.8, 1.0 - movement.coordination
            )
            if conf > 0.2:
                signals.append(
                    self._make_signal(
