...
class FactExtractor:
    ...
    def extract(self, signals: List[PerceivedSignal], current_tick: float) -> List[ObservedFact]:
        facts = []
    ...
    def _make_fact(
        self, signal: PerceivedSignal, fact_type: str, fact_name: str, 
        value: Any, inaccuracy: List[str] = None
    ) -> ObservedFact:
        return ObservedFact(
    ...
    def _extract_hands_facts(self, signal: PerceivedSignal, tick: float) -> List[ObservedFact]:
        facts = []
        if signal.field in ("held_object_left", "held_object_right"):
            if signal.perceived_value is not None:
                facts.append(self._make_fact(
                    signal, "body_state", "weapon_visible" if "weapon" in str(signal.perceived_value) else "holding_object",
                    signal.perceived_value, ["hidden_weapon", "small_object_concealed"]
                ))
        elif signal.field == "grip_strength":
            facts.append(self._make_fact(signal, "behavior", "grip_strength", signal.perceived_value))
        return facts

    def _extract_body_facts(self, signal: PerceivedSignal, tick: float) -> List[ObservedFact]:
        facts = []
        if signal.field == "muscle_tension":
            facts.append(self._make_fact(signal, "behavior", "muscle_tension_level", signal.perceived_value))
        elif signal.field == "tremor":
            facts.append(self._make_fact(signal, "behavior", "tremor_amplitude", signal.perceived_value))
        elif signal.field == "collapse":
            facts.append(self._make_fact(signal, "body_state", "posture_collapse", signal.perceived_value))
        return facts

    def _extract_movement_facts(self, signal: PerceivedSignal, tick: float) -> List[ObservedFact]:
        facts = []
        if signal.field == "speed":
            facts.append(self._make_fact(signal, "movement", "movement_speed", signal.perceived_value))
        elif signal.field == "coordination":
            facts.append(self._make_fact(signal, "movement", "movement_coordination", signal.perceived_value))
        return facts

    def _extract_voice_facts(self, signal: PerceivedSignal, tick: float) -> List[ObservedFact]:
        facts = []
        if signal.field == "tempo":
            facts.append(self._make_fact(signal, "voice", "voice_tempo", signal.perceived_value))
        elif signal.field == "tremor":
            facts.append(self._make_fact(signal, "voice", "voice_tremor_amplitude", signal.perceived_value))
        return facts