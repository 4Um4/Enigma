        ...
        for d in (combat_result.deltas or []):
            if d.domain == DeltaDomain.PHYSIOLOGY and d.payload and isinstance(d.payload, PhysiologyPayload):
                ...
                _combat_data[_target] = {
                    ...
                    "injuries": [
                        {"zone": i.target_zone, "severity": i.structural_damage, "damage_type": i.damage_type}
                        for i in d.payload.add_injuries
                    ] if d.payload.add_injuries else []
                }
                # SHI-FIX: L1Chronicle emission for attack/damage
                if d.payload.hp_delta < 0 or d.payload.add_injuries:
                    _combat_l1_events.append(TraitDriftEvent(
                        tick_id=ctx.tick_number, target_id=_target,
                        source_id="player", effect_value=-0.2, observation_weight=1.0, event_type="attack"
                    ))
                    _combat_l1_events.append(TraitDriftEvent(
                        tick_id=ctx.tick_number, target_id=_target,
                        source_id="combat", effect_value=d.payload.hp_delta, observation_weight=1.0, event_type="damage"
                    ))
...
def _execute_handler(
    ...
    try:
        ...
        logger.warning(f"[PHASE8_CRASH] handler={handler.name} error={type(e).__name__}: {e}")
        logger.error(f"[PHASE_8] {handler.name} handle() failed: {e}. Events lost this tick.")
        return None

    # Применяем Phase8Result к _TickContext
    _apply_handler_result(ctx, result, handler.name, dynamic_field=None, resolve_spatial_fn=None)
    return result
...
def _apply_handler_result(
    ...
    if result.deltas:
        ...
        ctx.prop_dirty = True
        logger.debug(f"[PHASE_8] {handler_name}: {len(result.deltas)} deltas routed to delta_buffer")

        # S91: Эмит стигмергического следа (safety_confidence) при акте насилия
        if handler_name == "combat" and dynamic_field is not None and resolve_spatial_fn is not None:
