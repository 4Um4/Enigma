...
def run_phase_0_5(ctx: _TickContext, deps: Phase0_5Deps) -> None:
    ...
    for npc_dict in ctx.all_npcs_raw:
        ...
        try:
            ...
            decayed = decay_affective_imprints(
                imprints, float(GAME_TICK_INTERVAL_SECONDS), _current_time
            )
    ...
    if snapshots:
        _sample = snapshots[0]
        logger.debug(
            f"[AFF_DEBUG] handlers={[type(h).__name__ for h in deps.idle_handlers]} aff_load={_sample.get('affective_load', '<MISSING>')} emo={_sample.get('emotion', '<MISSING>')}"
        )
        ...
        if deltas:
            ...
            if hasattr(handler, "name") and handler.name == "physiology_decay":
                if deps.l1_chronicle is not None:
                    ...
                    for _d in deltas:
                        if _d.npc_id and (
                            abs(getattr(_d.payload, "pain_delta", 0.0)) > 0
                            or abs(getattr(_d.payload, "blood_loss_delta", 0.0)) > 0
                        ):
                            _decay_events.append(
                                TraitDriftEvent(
                                    tick_id=ctx.tick_number,
                                    target_id=_d.npc_id,
                                    source_id="physiology_decay",
                                    effect_value=-0.01,
