...
class TraversalExecutionSystem:
    ...
    @staticmethod
    def advance(scene_state: dict, current_tick: int) -> None:
        """Вызывается в Фазе 0.5. Продвигает все активные маршруты."""
        traversals = scene_state.get("active_traversals", {})
        if not traversals:
            return
        
        logger.debug(f"[TRAV_EXEC_ADVANCE] tick={current_tick} active_traversals={list(traversals.keys())}")
        ...
        for npc_id, trav in traversals.items():
            ...
            if elapsed_ticks >= duration_ticks:
                ...
                npc_positions.setdefault(npc_id, {})["local_position"] = {"x": target_xy[0], "y": target_xy[1]}
                trav["status"] = "COMPLETED"
                completed_npcs.append(npc_id)
                logger.debug(f"[TRAV_EXEC] COMPLETED: npc={npc_id} snapped to {target_xy}")
            else:
                ...
                if pos:
                    npc_positions.setdefault(npc_id, {})["local_position"] = {"x": pos[0], "y": pos[1]}
                    # Обновляем current_waypoint_idx для фронтенда
                    trav["current_waypoint_idx"] = TraversalExecutionSystem._get_current_wp_idx(waypoints, progress)
    ...
    @staticmethod
    def resolve(npc_id: str, scene_state: dict, current_tick: int) -> Tuple[float, float]:
        ...
        lp = scene_state.get("npc_positions", {}).get(npc_id, {}).get("local_position", {})
        return (lp.get("x", 0.0), lp.get("y", 0.0))

    @staticmethod
    def _interpolate_path(waypoints: list, progress: float) -> Tuple[float, float]:
        ...
        for i, seg_dist in enumerate(segment_dists):
            if current_dist + seg_dist >= target_dist:
                seg_progress = (target_dist - current_dist) / seg_dist if seg_dist > 0 else 0.0
    ...
    @staticmethod
    def _get_current_wp_idx(waypoints: list, progress: float) -> int:
