...
class SignificanceDetector:
    ...
    SIGNIFICANT_EVENT_TYPES = frozenset({
        "npc_death", "combat_start", "faction_war", "marriage", "succession",
        "npc_enters_player_location", "trauma_event", "npc_speaks_about_player",
    })
    ...
    def check(
        ...
        for delta in result_dto.significant_events:
            event_type = getattr(delta, "source", None) or (delta.get("source", "") if isinstance(delta, dict) else "")
            if event_type in self.SIGNIFICANT_EVENT_TYPES:
                return SignificantEvent(type=event_type, tick=tick, details={"raw_delta": delta})
...
class SemanticMilestoneFilter:
    ...
    def check(
        ...
        for event in result_dto.significant_events:
            event_type = getattr(event, "source", None) or (event.get("source") if isinstance(event, dict) else "")
        ...
        if child_id:
            ...
            prev_child = next((n for n in prev_npcs if n.get("npc_id") == child_id), None)
        ...
        for curr_npc in curr_npcs:
            ...
            if prev_integrity > 0.3 >= curr_integrity:
                return Milestone(
                    ...
                    tick=tick,
                    details={"npc_id": npc_id, "prev": prev_integrity, "curr": curr_integrity},
                    requires_playback=True
                )
    ...
    def _requires_playback(self, milestone_type: str) -> bool:
        playback_required = {
            ...
            "attachment_formed", "personality_trait_formed",
            "combat_won", "combat_lost", "ally_lost", "betrayal_committed",
        }
...
class SkipPolicyC:
    ...
    def execute(
        ...
        for _ in range(max_ticks):
            ...
            _curr_npcs = get_npcs(campaign_id)
            milestone = self._filter.check(_tick, result, _prev_npcs, _curr_npcs, context)
    ...
    def _group_by_period(self, checkpoints: List[MilestoneCheckpoint], context: dict) -> List[Dict[str, Any]]:
...
class TimeSkipExecutor:
    ...
    def _kernel_execute_wrapper(self, campaign_id: str, scene_state: dict, tick_number: int, spatial_service: Any, npc_services: Any):
    ...
    def skip(
        ...
        if policy == "A":
            return self._policy_a.execute(
                self._kernel_execute_wrapper, campaign_id, scene_state, ticks,
                spatial_service, npc_services, get_npcs_callback,
            )
        elif policy == "B":
            ...
            return self._policy_c.execute(
                self._kernel_execute_wrapper, campaign_id, scene_state, ticks,
                spatial_service, npc_services, get_npcs_callback, context or {},
            )
        else:
