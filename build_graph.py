...
class ArchitectureGraphBuilder:
    def __init__(
        self, yaml_dir="architecture", output_file="docs/ARCHITECTURE_FLOW_GENERATED.md"
    ):
    ...
    def _get_mermaid_node_shape(self, node_id, node_data):
        ...
        if ntype in ("external", "api"):
            return f'{node_id}[["{label}"]]'
        if ntype in ("queue", "event_bus"):
            return f'{node_id}(("{label}"))'
        if ntype == "dto":
            return f'{node_id}["{label}"]'
    ...
    def generate_flowchart(self):
        ...
        sorted_nodes = sorted(
            self.nodes.items(),
            key=lambda x: (
                self.layer_order.index(x[1].get("layer", "Domain")),
                x[1]["domain"],
            ),
        )
