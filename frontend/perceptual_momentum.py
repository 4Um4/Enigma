...
class ManifestationProfile:
    ...
    temporal_assembly_delay: float = (
        0.0  # Задержка подтверждения реальности (PerceptualLatency)
    )
...
class PerceptualMomentum:
    ...
    def _apply_controlled_stochasticity(
        self, base_value: float, noise_level: float
    ) -> float:
    ...
    def _map_vectors_to_target(
        self, vectors: SanitizedPerceptualVectors
        ...
        return ManifestationProfile(
            visual_instability=self._apply_controlled_stochasticity(
                vectors.visual_instability, noise
            ),
            motor_disruption=self._apply_controlled_stochasticity(
                vectors.motor_disruption, noise * 0.5
            ),
            contrast_instability=self._apply_controlled_stochasticity(
                vectors.sensory_noise, noise
            ),
    ...
    def update(self, delta_seconds: float, vectors: SanitizedPerceptualVectors) -> None:
        ...
        self.current.visual_instability = self._lerp_with_momentum(
            self.current.visual_instability,
            target.visual_instability,
            delta_seconds,
            recon_rate,
        )

        self.current.attention_tunneling = self._lerp_with_momentum(
            self.current.attention_tunneling,
            target.attention_tunneling,
            delta_seconds,
            recon_rate,
        )

        self.current.temporal_distortion = self._lerp_with_momentum(
            self.current.temporal_distortion,
            target.temporal_distortion,
            delta_seconds,
            recon_rate,
        )

        self.current.temporal_assembly_delay = self._lerp_with_momentum(
            self.current.temporal_assembly_delay,
            target.temporal_assembly_delay,
            delta_seconds,
            recon_rate,
        )

        self.current.motor_disruption = self._lerp_with_momentum(
            self.current.motor_disruption,
            target.motor_disruption,
            delta_seconds,
            recon_rate,
        )

        self.current.contrast_instability = self._lerp_with_momentum(
            self.current.contrast_instability,
            target.contrast_instability,
            delta_seconds,
            recon_rate,
        )
        ...
        self.current.blood_visibility = self._lerp_with_momentum(
            self.current.blood_visibility,
            target.blood_visibility,
            delta_seconds,
            recon_rate,
            degrade_speed=8.0,
            recover_base_speed=0.3,
        )

        # Моторный и визуальный снос интерполируем напрямую (векторы)
        self.current.motion_bias = (
            self._lerp_with_momentum(
                self.current.motion_bias[0],
                target.motion_bias[0],
                delta_seconds,
                recon_rate,
            ),
            self._lerp_with_momentum(
                self.current.motion_bias[1],
                target.motion_bias[1],
                delta_seconds,
                recon_rate,
            ),
        )
