...
ВНИМАНИЕ: NPC-логика перенесена в data.npc_data. Этот файл - фасад для обратной совместимости.
"""
...
class DataManager:
        ...
        for item in items:
            ...
            if iid.startswith(prefix):
                try:
                    ...
                    )  # iid имеет неожиданный формат, пропускаем
            ...
            try:
                with open(f, "r", encoding="utf-8") as file:
                    ...
                    data.setdefault(
                        "origin",
                        {
                            "x": (len(self.locations) % 5) * 30,
                            "y": (len(self.locations) // 5) * 25,
                        },
                    )
                    ...
                    data.setdefault(
                        "passages", []
                    )  # Внутренние проходы (двери в стенах)
    ...
    def create_location(
        ...
        self.locations[filename] = {
            ...
            "location_id": filename.replace(
                ".json", ""
            ),  # ADR-061: Инференс из имени файла при создании
    ...
    def _migrate_to_global_coords(self, data: dict) -> None:
        ...
        for room in data.get("rooms", []):
            room.setdefault(
                "area_sqm", round(room.get("width", 0) * room.get("height", 0), 1)
            )
    ...
    def rename_entity(
        self, filename: str, entity_type: str, entity_id: str, new_name: str
    ) -> bool:
    ...
    # === Управление узлами ===
    def add_node(
        self, filename: str, node_id: str, x: float, y: float, label: str = ""
    ) -> bool:
    ...
    def add_object(
        ...
        obj = {
            ...
            "show_name": False,
            "properties": {
                k: deepcopy(v) for k, v in preset.get("default_properties", {}).items()
            },
        }
    ...
    # === Управление NPC ===
    def add_npc(
        self, filename: str, ref_id: str, x: float, y: float, room_id: str = ""
    ) -> str:
    ...
    def move_npc(
        self, filename: str, npc_ref_id: str, x: float, y: float, room_id: str = ""
    ) -> bool:
    ...
    def move_label(self, filename: str, label_id: str, x: float, y: float) -> bool:
        """Перемещает надпись"""
        loc = self.locations[filename]
        for lbl in loc["labels"]:
            if lbl["id"] == label_id:
    ...
    def validate_external_link(self, link: str) -> Tuple[bool, str]:
        ...
        if target_id in data.get("nodes", {}) or any(
            p["id"] == target_id for p in data.get("portals", [])
        ):
            return True, ""
