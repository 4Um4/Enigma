...
class NarrativeRenderer:
    ...
    def draw_beat(self, surface: pygame.Surface, beat: NarrativeBeat, x: int, y: int, max_width: int) -> int:
        ...
        border_width = 3 if beat.delivery == DeliveryType.SHOUT else 1 if beat.delivery in (DeliveryType.WHISPER, DeliveryType.INTERNAL) else 2
        pygame.draw.rect(bubble_surf, border_color, bubble_surf.get_rect(), border_width, border_radius=4)
    ...
    def draw_input_bubble(self, surface: pygame.Surface, player_name: str, text: str, cursor_pos: int, x: int, y: int, max_width: int) -> int:
        ...
        lines = self._wrap_text(display_text, self.font_normal, max_width - pad * 2 - accent_width)
        ...
        # Фон плашки
        pygame.draw.rect(name_bg_surf, (*self.COLOR_BORDER_PLAYER, 200), (0, 0, name_bg_width, name_height - 6), border_radius=4)
        # Стрелка-вырез снизу плашки
        pygame.draw.polygon(name_bg_surf, (*self.COLOR_BORDER_PLAYER, 200), [
        ...
        pygame.draw.rect(bubble_surf, self.COLOR_BG_PLAYER, (0, 0, bubble_rect.width, bubble_rect.height), border_radius=6)
        
        # Акцентная полоса справа (стиль Persona)
        pygame.draw.rect(bubble_surf, self.COLOR_BORDER_PLAYER, (bubble_rect.width - accent_width, 0, accent_width, bubble_rect.height), border_top_right_radius=6, border_bottom_right_radius=6)
        
        # Рамка пузыря
        pygame.draw.rect(bubble_surf, (*self.COLOR_BORDER_PLAYER, 160), (0, 0, bubble_rect.width, bubble_rect.height), 2, border_radius=6)
        ...
        pygame.draw.polygon(bubble_surf, (*self.COLOR_BG_PLAYER[:3], 255), [
            (tail_x_start, tail_y),
        ...
        if text and cursor_pos <= len(text):
            ...
            cursor_lines = self._wrap_text(text_before_cursor, self.font_normal, max_width - pad * 2 - accent_width)
