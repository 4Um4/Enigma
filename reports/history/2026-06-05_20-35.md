# ENIGMA Session State — 2026-06-05 20:35
...
| **CVS** (Causal Velocity) | 0.00/мин | ↓ -2.8 | LLM не вызывалась: сессия без действий игрока |
| **PFI** (Pre-Bus Failure) | 133% | ↓ +90.5% | ⛔ 4 пред-шинных отказов — pipeline молча умирает, CDS слеп |

_История: `reports/dna_history.jsonl` — 355 записей_
...
|-----|--------|-------|-----------|------------|--------------|
| blacksmith_orm | REQUEST_SERVICE | 0.402 | ❌ | None | ❌ |
| guard_borko | BLOCK_PATH | 0.310 | ❌ узел не найден | None | ❌ |
| maid_lusya | SEEK_ALLY | 0.208 | ❌ | None | ❌ |
| merchant_goran | REQUEST_SERVICE | 0.415 | ❌ узел не найден | None | ❌ |
| tavern_keeper_tornin | SPREAD_RUMOR | 0.222 | ❌ | x=4.0 y=2.5 | ❌ |
| thief_shadow | CHANGE_ROLE | 0.409 | ❌ | None | ❌ |
...
**NPC с разрывом в pipeline (intent есть, traversal нет):**
  ...
  - `maid_lusya`: intent=SEEK_ALLY, traversal=❌, coords=None
