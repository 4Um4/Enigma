# ENIGMA Session State — 2026-07-26 21:56
...
_Сессия: 1.1 мин | Тиков: 1 | LLM-вызовов: 16_
...
| **CVS** (Causal Velocity) | 14.02/мин | ↑ +14.0 | ✅ 14.02/мин: активная сессия |
| **PFI** (Pre-Bus Failure) | 0% | → +0.0% | ✅ норма: пред-шинных отказов нет — CDS видит всё |

_История: `reports/dna_history.jsonl` — 827 записей_
...
**Tick Pipeline:**
Тиков: 1 | Decisions > 0: 2/1 | LLM: 16 вызовов / 16 ответов | Симуляция: ✅ живёт
...
|-----|--------|-------|-----------|------------|--------------|
| blacksmith_orm | request_service | 0.764 | ❌ | None | ❌ |
| guard_borko | idle | 0.193 | ❌ | None | ❌ |
| maid_lusya | observe | 0.202 | ❌ | None | ❌ |
| merchant_goran | idle | 0.222 | ❌ | None | ❌ |
| player | observe | 0.223 | ❌ | None | ❌ |
| tavern_keeper_tornin | idle | 0.213 | ❌ | None | ❌ |
| thief_shadow | idle | 0.236 | ❌ | None | ❌ |

**NPC с разрывом в pipeline (intent есть, traversal нет):**
  - `guard_borko`: intent=idle, traversal=❌, coords=None
  - `merchant_goran`: intent=idle, traversal=❌, coords=None
  ...
  - `tavern_keeper_tornin`: intent=idle, traversal=❌, coords=None
