# ENIGMA Session State — 2026-08-06 22:51
...
_Сессия: 1.4 мин | Тиков: 45 | LLM-вызовов: 162_
...
| **CVS** (Causal Velocity) | 118.40/мин | ↓ -30.4 | ✅ 118.40/мин: активная сессия |
| **PFI** (Pre-Bus Failure) | 0% | → +0.0% | ✅ норма: пред-шинных отказов нет — CDS видит всё |
| **Tracebacks** | 2 (AttrErr=0, TypeErr=2) | → | ⚠️ КРИТИЧНО: невидимые регрессии (Tracebacks) |
| **BCI** (Belief Crystallization) | 241 (idx=5.36) | → | ✅ Убеждения формируются |
| **BPI** (Break Progress) | 266 (broken=0) | → | ✅ Давление доходит |
| **NEI** (Need Urgency) | 0 (critical=0) | → | ⚠️ NPC слишком комфортны (NEI=0) |
| **DRI** (Response Integrity) | 99% | ↑ +20.1% | ✅ LLM отвечает на все запросы |
| **DPI** (Dialogue Pipeline) | 100% | → +0.0% | ✅ Конвейер диалогов стабилен |

_История: `reports/dna_history.jsonl` — 929 записей_
...
**Tick Pipeline:**
Тиков: 45 | Decisions > 0: 1/45 | LLM: 162 вызовов / 156 ответов | Симуляция: ✅ живёт
...
|-----|--------|-------|-----------|------------|--------------|
| blacksmith_orm | request_service | 0.660 | ❌ | x=6.5 y=5.5 | ❌ |
| guard_borko | call_for_help | 0.230 | ❌ | x=19.0 y=4.5 | ❌ |
| maid_lusya | flee | 0.200 | ❌ | x=6.5 y=5.5 | ❌ |
| merchant_goran | request_service | 0.625 | ❌ | x=6.5 y=5.5 | ❌ |
| player | observe | 0.269 | ❌ | None | ❌ |
| tavern_keeper_tornin | spread_rumor | 0.234 | ❌ | x=6.5 y=5.5 | ❌ |
