# ENIGMA Session State — 2026-08-08 17:07
...
_Сессия: 0.3 мин | Тиков: 22 | LLM-вызовов: 32_
...
| **CVS** (Causal Velocity) | 96.12/мин | ↓ -12.8 | ✅ 96.12/мин: активная сессия |
| **PFI** (Pre-Bus Failure) | 0% | → +0.0% | ✅ норма: пред-шинных отказов нет — CDS видит всё |
| **Tracebacks** | 0 (AttrErr=0, TypeErr=0) | → | ✅ норма |
| **BCI** (Belief Crystallization) | 133 (idx=6.05) | → | ✅ Убеждения формируются |
| **BPI** (Break Progress) | 132 (broken=0) | → | ✅ Давление доходит |
| **NEI** (Need Urgency) | 0 (critical=0) | → | ⚠️ NPC слишком комфортны (NEI=0) |
| **DRI** (Response Integrity) | 94% | ↓ -6.2% | ✅ LLM отвечает на все запросы |
| **DPI** (Dialogue Pipeline) | 100% | → +0.0% | ✅ Конвейер диалогов стабилен |

_История: `reports/dna_history.jsonl` — 936 записей_
...
**Tick Pipeline:**
Тиков: 22 | Decisions > 0: 1/22 | LLM: 32 вызовов / 26 ответов | Симуляция: ✅ живёт
...
|-----|--------|-------|-----------|------------|--------------|
| blacksmith_orm | request_service | 0.829 | ❌ | x=6.5 y=5.5 | ❌ |
| guard_borko | ambush | 0.226 | ❌ | x=19.0 y=4.5 | ❌ |
| maid_lusya | observe | 0.197 | ❌ | x=10.5 y=6.5 | ❌ |
| merchant_goran | offer_job | 0.646 | ❌ | x=6.5 y=5.5 | ❌ |
| tavern_keeper_tornin | spread_rumor | 0.230 | ❌ | x=10.5 y=6.5 | ❌ |
