# ENIGMA Session State — 2026-06-06 22:37
...
| **PFI** (Pre-Bus Failure) | 40% | ↑ -60.0% | ⛔ 2 пред-шинных отказов — pipeline молча умирает, CDS слеп |

_История: `reports/dna_history.jsonl` — 379 записей_
...
- NPC без координат (lerp не работает, 6):
  - `guard_borko` (intent=HELP)
  - `merchant_goran` (intent=OBSERVE)
  - `blacksmith_orm` (intent=IDLE)
...
**Tick Pipeline:**
Тиков: 5 | Decisions > 0: 3/5 | LLM: 2 вызовов / 2 ответов | Симуляция: ✅ живёт
...
|-----|--------|-------|-----------|------------|--------------|
| blacksmith_orm | IDLE | 0.000 | ❌ | None | ❌ |
| guard_borko | HELP | 0.193 | ❌ узел не найден | None | ❌ |
| maid_lusya | OBSERVE | 0.326 | ❌ | None | ❌ |
| merchant_goran | OBSERVE | 0.240 | ❌ узел не найден | None | ❌ |
| tavern_keeper_tornin | WARN | 0.249 | ✅ | None | ❌ |
| thief_shadow | APPROACH | 0.248 | ❌ | None | ❌ |

**NPC с разрывом в pipeline (intent есть, traversal нет):**
  - `guard_borko`: intent=HELP, traversal=❌, coords=None
...
**Симптом:** NPC получил intent=HELP но узел назначения отсутствует в графе
**Цепь отказа:**
