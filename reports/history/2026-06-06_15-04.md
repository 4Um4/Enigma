# ENIGMA Session State — 2026-06-06 15:04
...
| **CVS** (Causal Velocity) | 0.00/мин | ↓ -3.4 | LLM не вызывалась: сессия без действий игрока |
| **PFI** (Pre-Bus Failure) | 100% | ↓ +57.1% | ⛔ 3 пред-шинных отказов — pipeline молча умирает, CDS слеп |

_История: `reports/dna_history.jsonl` — 376 записей_
...
|-----|--------|-------|-----------|------------|--------------|
| blacksmith_orm | OFFER_JOB | 0.499 | ❌ | None | ❌ |
| guard_borko | SPREAD_RUMOR | 0.354 | ❌ узел не найден | None | ❌ |
| maid_lusya | OFFER_JOB | 0.279 | ❌ | None | ❌ |
| merchant_goran | CHANGE_ROLE | 0.343 | ❌ узел не найден | None | ❌ |
| tavern_keeper_tornin | CALL_FOR_HELP | 0.323 | ❌ | None | ❌ |
| thief_shadow | REQUEST_SERVICE | 0.329 | ✅ | None | ❌ |
...
**NPC с разрывом в pipeline (intent есть, traversal нет):**
  ...
  - `maid_lusya`: intent=OFFER_JOB, traversal=❌, coords=None
...
**Симптом:** NPC получил intent=SPREAD_RUMOR но узел назначения отсутствует в графе
**Цепь отказа:**
