# ENIGMA Session State — 2026-05-24 00:21
...
| **ADR** (Debt Ratio) | 0.00 | ↑ -2.0 | нет ADR-записей — невозможно оценить |
| **CVS** (Causal Velocity) | 0.00/мин | → +0.0 | LLM не вызывалась: сессия без действий игрока |

_История: `reports/dna_history.jsonl` — 48 записей_
...
|-----|--------|-------|-----------|------------|--------------|
| blacksmith_orm | APPROACH | 0.480 | ❌ | None | ❌ |
| guard_borko | WARN | 0.526 | ❌ узел не найден | None | ❌ |
| maid_lusya | OBSERVE | 0.661 | ✅ | None | ❌ |
| merchant_goran | TRADE | 0.451 | ❌ узел не найден | None | ❌ |
| tavern_keeper_tornin | TALK | 0.423 | ✅ | None | ❌ |
| thief_shadow | FLEE | 1.244 | ✅ | x=16.0 y=4.0 | ❌ |

**NPC с разрывом в pipeline (intent есть, traversal нет):**
  - `guard_borko`: intent=WARN, traversal=❌, coords=None
  - `merchant_goran`: intent=TRADE, traversal=❌, coords=None
