# ENIGMA Session State — 2026-08-08 18:04
...
_Сессия: 0.3 мин | Тиков: 21 | LLM-вызовов: 25_
...
| **CVS** (Causal Velocity) | 73.05/мин | ↓ -34.2 | ✅ 73.05/мин: активная сессия |
| **PFI** (Pre-Bus Failure) | 0% | → +0.0% | ✅ норма: пред-шинных отказов нет — CDS видит всё |
| **Tracebacks** | 0 (AttrErr=0, TypeErr=0) | → | ✅ норма |
| **BCI** (Belief Crystallization) | 126 (idx=6.00) | → | ✅ Убеждения формируются |
| **BPI** (Break Progress) | 126 (broken=0) | → | ✅ Давление доходит |
| **NEI** (Need Urgency) | 0 (critical=0) | → | ⚠️ NPC слишком комфортны (NEI=0) |
| **DRI** (Response Integrity) | 80% | ↓ -20.0% | ⚠️ Деградация: 5 провалов LLM. Ответы нестабильны |
| **DPI** (Dialogue Pipeline) | 100% | → +0.0% | ✅ Конвейер диалогов стабилен |

_История: `reports/dna_history.jsonl` — 937 записей_
...
**Tick Pipeline:**
Тиков: 21 | Decisions > 0: 1/21 | LLM: 25 вызовов / 18 ответов | Симуляция: ✅ живёт
...
| tavern_keeper_tornin | spread_rumor | 0.249 | ❌ | x=10.5 y=4.8 | ❌ |
