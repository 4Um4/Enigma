# ENIGMA Session State — 2026-06-05 20:39
...
| **CVS** (Causal Velocity) | 0.00/мин | ↓ -2.7 | LLM не вызывалась: сессия без действий игрока |
| **PFI** (Pre-Bus Failure) | 133% | ↓ +13.3% | ⛔ 4 пред-шинных отказов — pipeline молча умирает, CDS слеп |

_История: `reports/dna_history.jsonl` — 356 записей_
...
- NPC без координат (lerp не работает, 5):
  ...
  - `maid_lusya` (intent=REQUEST_SERVICE)
...
|-----|--------|-------|-----------|------------|--------------|
| blacksmith_orm | CHANGE_ROLE | 0.405 | ❌ | None | ❌ |
| guard_borko | BLOCK_PATH | 0.456 | ❌ узел не найден | None | ❌ |
| maid_lusya | REQUEST_SERVICE | 0.247 | ❌ | None | ❌ |
| merchant_goran | REQUEST_SERVICE | 0.402 | ❌ узел не найден | None | ❌ |
| tavern_keeper_tornin | CALL_FOR_HELP | 0.291 | ❌ | x=4.0 y=2.5 | ❌ |
| thief_shadow | CHANGE_ROLE | 0.427 | ❌ | None | ❌ |
...
**NPC с разрывом в pipeline (intent есть, traversal нет):**
  ...
  - `maid_lusya`: intent=REQUEST_SERVICE, traversal=❌, coords=None
