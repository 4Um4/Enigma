# ENIGMA Session State — 2026-06-06 00:56
...
_Сессия: 1.9 мин | Тиков: 5 | LLM-вызовов: 2_
...
| **CVS** (Causal Velocity) | 1.03/мин | ↓ -0.7 | ✅ 1.03/мин: активная сессия |
| **PFI** (Pre-Bus Failure) | 100% | ↓ +80.0% | ⛔ 5 пред-шинных отказов — pipeline молча умирает, CDS слеп |

_История: `reports/dna_history.jsonl` — 368 записей_
...
- NPC без координат (lerp не работает, 6):
  ...
  - `blacksmith_orm` (intent=TALK)
  - `maid_lusya` (intent=OBSERVE)
  - `thief_shadow` (intent=APPROACH)
...
**Tick Pipeline:**
Тиков: 5 | Decisions > 0: 2/5 | LLM: 2 вызовов / 2 ответов | Симуляция: ✅ живёт
...
|-----|--------|-------|-----------|------------|--------------|
| blacksmith_orm | TALK | 0.175 | ❌ | None | ❌ |
| guard_borko | OBSERVE | 0.220 | ❌ узел не найден | None | ❌ |
| maid_lusya | OBSERVE | 0.449 | ❌ | None | ❌ |
| merchant_goran | TALK | 0.275 | ❌ узел не найден | None | ❌ |
| tavern_keeper_tornin | TALK | 0.218 | ❌ | None | ❌ |
| thief_shadow | APPROACH | 0.237 | ❌ | None | ❌ |
...
**NPC с разрывом в pipeline (intent есть, traversal нет):**
  ...
  - `blacksmith_orm`: intent=TALK, traversal=❌, coords=None
  - `maid_lusya`: intent=OBSERVE, traversal=❌, coords=None
  - `thief_shadow`: intent=APPROACH, traversal=❌, coords=None
