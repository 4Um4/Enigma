# ENIGMA Session State — 2026-06-06 22:42
...
| **PFI** (Pre-Bus Failure) | 100% | ↓ +33.3% | ⛔ 3 пред-шинных отказов — pipeline молча умирает, CDS слеп |

_История: `reports/dna_history.jsonl` — 380 записей_
...
- NPC без координат (lerp не работает, 6):
  ...
  - `tavern_keeper_tornin` (intent=OFFER_JOB)
...
|-----|--------|-------|-----------|------------|--------------|
| blacksmith_orm | OFFER_JOB | 0.357 | ❌ | None | ❌ |
| guard_borko | AMBUSH | 0.391 | ❌ узел не найден | None | ❌ |
| maid_lusya | CHANGE_ROLE | 0.257 | ❌ | None | ❌ |
| merchant_goran | OFFER_JOB | 0.415 | ❌ узел не найден | None | ❌ |
| tavern_keeper_tornin | OFFER_JOB | 0.177 | ✅ | None | ❌ |
| thief_shadow | REQUEST_SERVICE | 0.410 | ❌ | None | ❌ |
...
**NPC с разрывом в pipeline (intent есть, traversal нет):**
  ...
  - `maid_lusya`: intent=CHANGE_ROLE, traversal=❌, coords=None
