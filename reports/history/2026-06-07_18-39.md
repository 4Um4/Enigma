# ENIGMA Session State — 2026-06-07 18:39
...
| **PFI** (Pre-Bus Failure) | 200% | ↓ +100.0% | ⛔ 2 пред-шинных отказов — pipeline молча умирает, CDS слеп |

_История: `reports/dna_history.jsonl` — 404 записей_
...
- NPC без координат (lerp не работает, 6):
  ...
  - `tavern_keeper_tornin` (intent=SEEK_ALLY)
...
|-----|--------|-------|-----------|------------|--------------|
| blacksmith_orm | REQUEST_SERVICE | 0.413 | ❌ | None | ❌ |
| guard_borko | SEEK_ALLY | 0.348 | ❌ узел не найден | None | ❌ |
| maid_lusya | SPREAD_RUMOR | 0.262 | ❌ | None | ❌ |
| merchant_goran | REQUEST_SERVICE | 0.419 | ❌ узел не найден | None | ❌ |
| tavern_keeper_tornin | SEEK_ALLY | 0.322 | ❌ | None | ❌ |
| thief_shadow | CHANGE_ROLE | 0.454 | ❌ | None | ❌ |

**NPC с разрывом в pipeline (intent есть, traversal нет):**
  - `guard_borko`: intent=SEEK_ALLY, traversal=❌, coords=None
  ...
  - `tavern_keeper_tornin`: intent=SEEK_ALLY, traversal=❌, coords=None
...
**Симптом:** NPC получил intent=SEEK_ALLY но узел назначения отсутствует в графе
**Цепь отказа:**
