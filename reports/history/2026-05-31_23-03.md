# ENIGMA Session State — 2026-05-31 23:03
...
_История: `reports/dna_history.jsonl` — 251 записей_
...
- **[КРИТИЧНО]** NPC `merchant_goran`: узел `main_hall` не найден в графе
  PowerShell: `Select-String -Path "backend/app/services/spatial/*.py" -Pattern "main_hall"`
- **[КРИТИЧНО]** NPC `tavern_keeper_tornin`: узел `bed` не найден в графе
...
|-----|--------|-------|-----------|------------|--------------|
| blacksmith_orm | FLEE | 1.346 | ❌ | None | ❌ |
| guard_borko | FLEE | 1.699 | ❌ узел не найден | None | ❌ |
| maid_lusya | FLEE | 2.432 | ❌ узел не найден | None | ❌ |
| merchant_goran | FLEE | 1.512 | ❌ узел не найден | None | ❌ |
| tavern_keeper_tornin | FLEE | 1.541 | ❌ узел не найден | None | ❌ |
| thief_shadow | FLEE | 2.109 | ❌ узел не найден | None | ❌ |
...
#### [BREAK-N] Узел `bed` не найден для `tavern_keeper_tornin`
**Симптом:** NPC получил intent=FLEE но узел назначения отсутствует в графе
**Цепь отказа:**
