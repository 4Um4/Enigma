# ENIGMA Session State — 2026-08-08 21:05
...
_Сессия: 0.5 мин | Тиков: 42 | LLM-вызовов: 44_
...
| **CVS** (Causal Velocity) | 80.41/мин | ↑ +80.4 | ✅ 80.41/мин: активная сессия |
| **PFI** (Pre-Bus Failure) | 0% | → +0.0% | ✅ норма: пред-шинных отказов нет — CDS видит всё |
| **Tracebacks** | 0 (AttrErr=0, TypeErr=0) | → | ✅ норма |
| **BCI** (Belief Crystallization) | 266 (idx=6.33) | → | ✅ Убеждения формируются |
| **BPI** (Break Progress) | 252 (broken=0) | → | ✅ Давление доходит |
| **NEI** (Need Urgency) | 0 (critical=0) | → | ⚠️ NPC слишком комфортны (NEI=0) |
| **DRI** (Response Integrity) | 96% | ↓ -4.5% | ✅ LLM отвечает на все запросы |
| **DPI** (Dialogue Pipeline) | 100% | → +0.0% | ✅ Конвейер диалогов стабилен |

_История: `reports/dna_history.jsonl` — 952 записей_
...
**Tick Pipeline:**
Тиков: 42 | Decisions > 0: 1/42 | LLM: 44 вызовов / 40 ответов | Симуляция: ✅ живёт
...
|-----|--------|-------|-----------|------------|--------------|
| blacksmith_orm | request_service | 0.835 | ❌ | x=6.5 y=5.5 | ❌ |
| guard_borko | seek_ally | 0.232 | ❌ | x=10.5 y=6.5 | ❌ |
| maid_lusya | observe | 0.235 | ❌ | x=7.8 y=5.8 | ❌ |
| merchant_goran | request_service | 0.551 | ❌ | x=6.5 y=5.5 | ❌ |
| tavern_keeper_tornin | talk | 0.208 | ❌ | x=10.5 y=6.5 | ❌ |
