# ENIGMA Session State — 2026-06-20 12:38
...
| **NPI** (NPC Pipeline) | 0% | → +0.0% | ⛔ 0/5 NPC имеют координаты: traversal полностью сломан |
...
| **CVS** (Causal Velocity) | 0.00/мин | ↓ -0.8 | LLM не вызывалась: сессия без действий игрока |
| **PFI** (Pre-Bus Failure) | 1700% | ↑ -100.0% | ⛔ 17 пред-шинных отказов — pipeline молча умирает, CDS слеп |

_История: `reports/dna_history.jsonl` — 593 записей_
...
**Предупреждения:**
  - ⚠️ КРИТИЧНО: 17 пред-шинных отказов (pipeline=0, causality=0, phase8=0, tick_orch=17) — CDS слеп к этим багам без Инварианта 3
