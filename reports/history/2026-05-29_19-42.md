# ENIGMA Session State — 2026-05-29 19:42
...
_История: `reports/dna_history.jsonl` — 194 записей_
...
- **[КРИТИЧНО]** NPC `thief_shadow`: узел `main_hall` не найден в графе
  PowerShell: `Select-String -Path "backend/app/services/spatial/*.py" -Pattern "main_hall"`
- **[КРИТИЧНО]** NPC `tavern_keeper_tornin`: узел `bar_area` не найден в графе
...
|-----|--------|-------|-----------|------------|--------------|
| blacksmith_orm | FLEE | 1.365 | ❌ | None | ❌ |
| guard_borko | FLEE | 1.528 | ❌ | x=14.0 y=2.0 | ❌ |
| maid_lusya | FLEE | 2.302 | ❌ узел не найден | None | ❌ |
| merchant_goran | FLEE | 1.453 | ❌ | x=14.0 y=2.0 | ❌ |
| tavern_keeper_tornin | FLEE | 1.557 | ❌ узел не найден | None | ❌ |
| thief_shadow | FLEE | 2.197 | ❌ узел не найден | None | ❌ |
...
#### [BREAK-N] Узел `bar_area` не найден для `tavern_keeper_tornin`
**Симптом:** NPC получил intent=FLEE но узел назначения отсутствует в графе
**Цепь отказа:**
