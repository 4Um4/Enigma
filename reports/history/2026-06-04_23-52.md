# ENIGMA Session State — 2026-06-04 23:52
...
_Сессия: 2.7 мин | Тиков: 13 | LLM-вызовов: 18_

| Метрика | Значение | Δ от прошлой | Интерпретация для LLM |
|---------|----------|--------------|----------------------|
| **SHI** (Simulation Health) | 100% | → +0.0% | ✅ норма: NPC активно принимают решения |
| **NPI** (NPC Pipeline) | 43% | ↓ -7.1% | ⚠️ 3/7 NPC с координатами: spatial pipeline частично сломан |
...
| **CVS** (Causal Velocity) | 6.55/мин | ↑ +6.5 | ✅ 6.55/мин: активная сессия |
| **PFI** (Pre-Bus Failure) | 8% | ↓ +7.7% | ⚠️ 1 пред-шинных отказов — CDS слеп к части крахов |

_История: `reports/dna_history.jsonl` — 331 записей_
...
- NPC без координат (lerp не работает, 4):
  ...
  - `blacksmith_orm` (intent=ATTACK)
  - `player` (intent=?)
- Граф-fallback локаций: нет

### Визуальные аномалии:
- spatial_fallback triggered: ⚠️ ДА
- Узлы не найдены (NPC не могут добраться до цели): 20
...
**Tick Pipeline:**
Тиков: 13 | Decisions > 0: 13/13 | LLM: 18 вызовов / 18 ответов | Симуляция: ✅ живёт
...
|-----|--------|-------|-----------|------------|--------------|
| blacksmith_orm | ATTACK | 1.325 | ✅ | None | ❌ |
| guard_borko | ATTACK | 1.646 | ❌ | x=19.5 y=4.2 | ❌ |
| maid_lusya | FLEE | 0.501 | ✅ | None | ❌ |
| merchant_goran | ATTACK | 0.864 | ❌ | x=18.0 y=4.7 | ❌ |
| player | ? | 0.000 | ❌ узел не найден | None | ❌ |
| tavern_keeper_tornin | ATTACK | 1.841 | ❌ узел не найден | x=17.6 y=2.1 | ❌ |
| thief_shadow | ATTACK | 1.286 | ❌ узел не найден | None | ❌ |

**NPC с разрывом в pipeline (intent есть, traversal нет):**
  - `thief_shadow`: intent=ATTACK, traversal=❌, coords=None
