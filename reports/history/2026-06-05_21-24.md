# ENIGMA Session State — 2026-06-05 21:24
...
| **CVS** (Causal Velocity) | 0.00/мин | ↓ -2.1 | LLM не вызывалась: сессия без действий игрока |
| **PFI** (Pre-Bus Failure) | 200% | ↓ +142.9% | ⛔ 2 пред-шинных отказов — pipeline молча умирает, CDS слеп |

_История: `reports/dna_history.jsonl` — 359 записей_
...
|-----|--------|-------|-----------|------------|--------------|
| blacksmith_orm | REQUEST_SERVICE | 0.369 | ❌ | None | ❌ |
| guard_borko | AMBUSH | 0.375 | ❌ узел не найден | None | ❌ |
| maid_lusya | OFFER_JOB | 0.251 | ✅ | None | ❌ |
| merchant_goran | REQUEST_SERVICE | 0.456 | ❌ узел не найден | None | ❌ |
| tavern_keeper_tornin | SPREAD_RUMOR | 0.279 | ❌ | None | ❌ |
| thief_shadow | REQUEST_SERVICE | 0.372 | ❌ | None | ❌ |
...
**NPC с разрывом в pipeline (intent есть, traversal нет):**
  ...
  - `tavern_keeper_tornin`: intent=SPREAD_RUMOR, traversal=❌, coords=None
