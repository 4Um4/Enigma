# ENIGMA Session State — 2026-08-09 21:35
...
| **CVS** (Causal Velocity) | 11.64/мин | ↓ -6.0 | ✅ 11.64/мин: активная сессия |
| **PFI** (Pre-Bus Failure) | 0% | → +0.0% | ✅ норма: пред-шинных отказов нет — CDS видит всё |
| **Tracebacks** | 0 (AttrErr=0, TypeErr=0) | → | ✅ норма |
| **BCI** (Belief Crystallization) | 448 (idx=7.11) | → | ✅ Убеждения формируются |
| **BPI** (Break Progress) | 378 (broken=0) | → | ✅ Давление доходит |
| **NEI** (Need Urgency) | 0 (critical=0) | → | ⚠️ NPC слишком комфортны (NEI=0) |
| **DRI** (Response Integrity) | 100% | ↑ +16.7% | ✅ LLM отвечает на все запросы |
...
_История: `reports/dna_history.jsonl` — 1005 записей_
...
**Tick Pipeline:**
Тиков: 63 | Decisions > 0: 1/63 | LLM: 58 вызовов / 58 ответов | Симуляция: ✅ живёт
...
|-----|--------|-------|-----------|------------|--------------|
| blacksmith_orm | request_service | 0.984 | ❌ | x=6.5 y=5.5 | ❌ |
| guard_borko | seek_ally | 0.234 | ❌ | None | ❌ |
| maid_lusya | offer_job | 0.184 | ❌ | x=10.5 y=6.5 | ❌ |
| merchant_goran | request_service | 0.674 | ❌ | x=6.5 y=5.5 | ❌ |
| tavern_keeper_tornin | spread_rumor | 0.252 | ❌ | x=10.5 y=6.5 | ❌ |
