...
- **`WillResponseDTO`** (`models/will.py`): Результат WillpowerGate. `WillState`, `resistance`, `identity_damage`, `counter_offer`, `embodied_vector`.
...
- **`SceneChange`**: Проекция свершившегося. **Boundary Transition Pipeline (ADR-145):** `target_location_id` заполняется ТОЛЬКО в `_process_traversals()` при факте пересечения boundary node.
...
**Поток:** L0 (Perception) + L1 (Chronicle) → DriveResolver → L3 (EffectiveDrives) → Модуляция Давления/Риска.

**Актуальные DTO:**
- **`TraitDriftEvent`** (`domain/identity_events.py`): Единица записи давления мира в L1 Chronicle (ADR-O-208). Поля: `npc_id` (str), `trait` (str), `delta` (float), `source` (str), `tick` (int). Immutable.
...
- **`L1Chronicle`** (`services/npc/l1_chronicle.py`): Append-only хранилище событий деформации идентичности (L1) (ADR-O-208). Методы: `append(event)`, `query_raw(npc_id)`, `query_weighted(npc_id, current_tick)`. Удаление запрещено.
...
🚫 **КАУЗАЛЬНЫЕ ЗАПРЕТЫ (ADR-O-207, ADR-O-208, ADR-O-211):**
...
- ❌ **Некалиброванный дрейф (ADR-O-211):** Изменение базовых драйвов без работы Calibration Engine запрещено (риск осцилляции).
