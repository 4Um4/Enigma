...
### ADR-049: Топология Решений и Геометрия Восприятия (DecisionContext & Utility Deformation)
**Дата:** 14.05.2026  
**Статус:** Принято

**Контекст:**  
DecisionHub использовал хардкод-множители (например, `fear * 0.45` для APPROACH) и не имел структурированного механизма применения топологического давления из `PerceptualKernel` к ландшафту полезности (utility landscape). Это приводило к плоским и предсказуемым решениям NPC.

**Решение:**  
- Введен `DecisionContext` как мост Ядро→Хаб, проекция геометрии восприятия в геометрию решений (каузальная дискретизация T+1).
- Введена `UtilityFieldDeformation` (непрерывные векторы давления: `aggression_suppression`, `initiative_suppression`, `compliance_bias`, `escape_salience`) и `ActionSpaceCompression` (feasibility-слой — удаление невозможных действий).
- В `PerceptualKernel` и `IdentityPayload` добавлены поля топологии: `aggression_inhibition`, `initiative_suppression`, `compliance_bias`.
- Убит хардкод `fear * 0.45` в DecisionHub. Искривление ландшафта теперь вычисляется через множители из `DecisionContext`.

**Последствия:**  
- Решения NPC стали эмерджентными и зависимыми от топологии восприятия, а не от скриптовых коэффициентов.
- Запрещено использовать прямые хардкод-множители доменов в DecisionHub без прохождения через DecisionContext.
...
**Контекст:**  
Параллельная работа LLM-сессий привела к конфликту нумерации ADR (дублирование ADR-049) и захламлению `ADR-000_IMPACT_TEMPLATE.md`. Инфраструктура Каузальных Песочниц (`tests/sandbox/`) развивалась хаотично, без формального архитектурного покрытия и разделения на категории (феноменология, система, стресс).

**Решение:**  
- **Каузальная Обсерватория** зарегистрирована как самостоятельная архитектурная сущность (ADR-050).
- Аудиты влияния вынесены из общего шаблона в изолированные файлы `docs/audits/ADR-0XX_IMPACT.md`.
- Введена строгая типология Песочниц: `phenomenology/` (верификация проекций и искажений), `system/` (замыкание контура причинности), `stress/` (деградация и восстановление).

**Последствия:**  
- Устранен конфликт нумерации ADR. Каждая Песочница теперь имеет четкий архитектурный фундамент.
- Запрещено создавать песочницы вне директорий `phenomenology/`, `system/`, `stress/` или без привязки к ADR.

---

### ADR-051: LifeEngine De-godification
**Дата:** 15.05.2026  
**Статус:** Принято

**Контекст:**  
LifeEngine действовал как бог-объект: напрямую мутировал позицию NPC (`npc["position"] = ...`), инстанцировал и вызывал `MovementEngine.process_intents()`, минуя единый конвейер оркестратора. Это нарушало принцип единой точки мутации и приводило к каузальным разрывам (телепортация, потеря TraversalState).

**Решение:**  
- LifeEngine лишен права прямой мутации пространственного состояния.
- Методы `_simulate_major`, `_simulate_minor` и `check_random_events` переведены на возврат `tuple[list[SceneChange], list[MovementIntent]]`.
- `TickOrchestrator` распаковывает эти результаты и передает `MovementIntent` в `MovementEngine`, порождая легитимные `TraversalState`.
- Внедрен Когнитивный Страж: расписание блокируется при `initiative_suppression > 0.7` или `recent_directive.interrupts_routine == True` (вместо хардкод-порога `threat_gradient`).

**Последствия:**  
- LifeEngine стал "лоббистом" давления и расписания, а не исполнителем. Локомоция монополизирована MovementEngine.
- Устранена скрытая телепортация NPC через LifeEngine.

---

### ADR-052: Преемник Движения — Замыкание LOD0 и Трубы Расписания
**Дата:** 15.05.2026  
**Статус:** Принято

**Контекст:**  
Микро-движение (LOD0) внутри одной макро-зоны было парализовано из-за несовпадения строковых префиксов (`"main_hall"` vs `"tavern:main_hall"`) в `_resolve_reactive_movement`. Кроме того, обнаружен критический Silent Data Loss в `LifeEngine.tick()`: кортеж результатов вставлялся в список изменений как единый элемент, из-за чего все `MovementIntent` терялись.

**Решение:**  
- Внедрена нормализация префиксов макро-зон (`current_base == target_base`), устраняющая ложную блокировку подхода.
- Починена распаковка результатов `_simulate_major` и `_simulate_minor` на `changes, intents = ...`.
- Архитектурное решение: `LocalSteeringIntent` отвергнут [REJECTED]. Микро-движение (LOD0) реализовано через существующее поле `local_target_xy` в `MovementIntent`.

**Последствия:**  
- Микро-перемещение (подход к игроку внутри зоны) разблокировано без создания новых сущностей.
- Труба расписания больше не теряет пространственные намерения.
...
**Контекст:**  
Хардкод-порог `initiative_suppression > 0.7` для блокировки расписания LifeEngine приводил к "заморозке" NPC даже после того, как угроза исчезала. Также при отсутствии целевого узла в MovementEngine применялся fallback на `entrance`, что вызывало пространственные галлюцинации и мгновенные телепортации.

**Решение:**  
- Переход от хардкод-порога к **Attention Capture**: в `PerceptualKernel` добавлено `recent_directive` (источник, salience, флаг `interrupts_routine`). LifeEngine проверяет этот флаг и сжигает директиву после использования (Consumption Mechanism).
- В MovementEngine внедрен **Safe Spatial Fallback**: при отсутствии узла макро-перемещение отменяется (макро-фоллбэк на `entrance` закомментирован).
- Внедрен **Collision Avoidance LOD0**: `process_intents` принимает `npc_positions` и ищет свободную точку вокруг цели.

**Последствия:**  
- NPC больше не телепортируются в `entrance` при потере пространственных данных.
- Блокировка расписания стала динамической и обратимой (сгорает после реакции).

---

### ADR-057: Legitimacy Gate & Elastic Time
**Дата:** 16.05.2026  
**Статус:** Принято

**Контекст:**  
`GAME_TICK_INTERVAL_SECONDS` был равен 900, что делало симуляцию слишком крупнозернистой. Директивы (приказы), отданные NPC без страха или доверия к командиру, слепо подавляли их волю (`initiative_suppression`), вызывая механическое подчинение без психологической основы, что нарушало Физику Власти.

**Решение:**  
- **Elastic Time:** `GAME_TICK_INTERVAL_SECONDS` снижен до 60 секунд, обеспечивая более гранулярное течение времени.
- **Legitimacy Gate:** В `DirectiveInterpretationSubscriber` внедрена проверка легитимности. При отсутствии страха/доверия директива порождает Irritation (снятие блоков агрессии) вместо Obedience (подавление воли). Флаг `interrupts_routine` всегда True, добавлен флаг `is_obedience`.
- **Topic Injection:** В `npc_tick_pipeline.py` добавлена принудительная инъекция темы (`directive_obedience` или `directive_confrontation`) при наличии `recent_directive`, заставляя DecisionHub реагировать.

**Последствия:**  
- Необоснованные приказы провоцируют конфронтацию и агрессию, а не покорность.
- Течение времени стало более плавным, лучше подходящим для отражения непрерывных процессов.

---

### ADR-058: Dual-Time Ontology и Каузальная Презентация
**Дата:** 18.05.2026  
**Статус:** Принято

**Контекст:**  
Фронтенд мгновенно обновлял позиции NPC из `scene_state`, что приводило к визуальной телепортации при каждом тике симуляции. Попытки сгладить это на стороне клиента приводили к рассинхронизации с серверной истиной. Дискретная природа тиков симуляции конфликтовала с требованием непрерывной визуальной презентации.

**Решение:**  
- Фронтенд перешел на Модель C (Elastic Time Architecture). Симуляция дискретна, презентация непрерывна.
- Фронтенд становится интерполятором `TraversalState`, используя `path_waypoints`, `progress` и `speed * dt`, а не угадывая путь.
- Локальный клиентский pathfinding (`find_path`) полностью удален. Фронтенд — Deterministic Client.
- Внедрена визуализация Cognitive Freeze: при `initiative_suppression > 0.7` рендерер применяет моторный тремор к NPC.

**Последствия:**  
- Визуальная телепортация NPC устранена. Движение стало плавным и детерминированным.
- Фронтенд лишен права пространственного предсказания. Он только визуализирует то, что вычислил бэкенд.

---

### ADR-059: ENIGMA Causal Diagnostic System (CDS) & Multi-LLM Observability
**Дата:** 19.05.2026  
**Статус:** Принято

**Контекст:**  
В проекте параллельно работают три LLM-архитектора, каждый из которых видит только свой слой. Для диагностики каскадных сбоев (например, Спринт 34: `load_editor_json` → узел не найден → SceneChange не создан → координаты None) требовалось 15+ ручных итераций PowerShell. Система не имела наблюдаемого состояния на старте сессии.

**Решение:**  
- Внедрен модуль `diagnostics/`, запускаемый из `game_launcher.py` как фоновый поток (CausalObserver).
- CDS парсит stdout/логи игры, `git log`, `MUTATIONS.md` и TODO-теги.
- На выходе генерируется `reports/LAST_SESSION.md` — LLM-оптимизированный Markdown с тремя секциями (Архитектор Кода, Архитектор UI, Архитектор Симуляции) и блоком автоидентификации.
- В начале файла встроены триггеры для самоидентификации LLM (определение роли по первому промпту).

**Последствия:**  
- **Zero Runtime Impact:** CDS работает в отдельном потоке, краши в CDS не роняют игру (try/except).
- **Контекстная память:** LLM получает актуальный снимок каузальных разрывов и active traversals без ручных запросов.
- **Read-Only:** CDS строго запрещено мутировать состояние симуляции. Это пассивный наблюдатель.
- В `npc_tick_pipeline.py` убраны все fallback-чтения `player_distances` из `scene_state`.

**Реализация (Phase 3 — Derived Presentation Cleanup, 19.05.2026):**
- Мутация `player_spatial` в `scene_init.py` заменена на мутацию канонического `npc_positions.player`.
- Запись `player_distances` и `player_spatial` заблокирована в `scene_state_manager.py`. Чтения заменены на динамическое вычисление через `euclidean_distance`.
- **Статус:** ADR-048 ПОЛНОСТЬЮ РЕАЛИЗОВАН. Система перешла на Single Spatial Authority.ack-чтения `scene_state.get("player_distances")` и `scene_state.get("npc_positions")`. Внедрён `_pos()` helper через `spatial_query.get_entity_position()`.
...
**Контекст:**  
Вычисление каузального тика через календарное время (`current_tick = game_time_seconds // 60`) создало круговую зависимость (Причинность → Календарь → Причинность). При откате игрового времени (регрессия Бага H) `current_tick` становился отрицательным, `progress` обнулялся, и NPC застывали. Кроме того, `WorldSnapshotBuilder` мутировал `scene_state` (удалял завершённые транзиты, перезаписывал `local_position`), нарушая принцип чистой проекции и вызывая race conditions при сборке снапшота.

**Решение:**  
1. **Монотонный Каузальный Тик:** В `scene_state` введено поле `tick: int`. Инкрементируется строго на +1 в `idle_tick` и `_run_pipeline`. Никогда не отматывается назад. Не зависит от `game_time_seconds` (который является лишь проекцией календаря, ADR-002).
2. **Потребление тика фронтендом:** Фронтенд вычисляет `progress` исключительно по формуле `(current_tick - started_tick) / duration_ticks`. Использование `time.time()` или `pygame.time.get_ticks()` для каузальных расчётов запрещено (нарушает детерминизм и воспроизводимость).
3. **Иммутабельность SnapshotBuilder:** Методы `WorldSnapshotBuilder` (включая `_extract_active_traversals`) являются чистыми проекциями. Они читают `scene_state` и возвращают DTO/Dict. Запрещено удалять транзиты (`del traversals[npc_id]`) или мутировать `local_position` внутри билдера.
4. **Финализация в State Manager:** Завершённые транзиты финализируются (обновление `local_position`, удаление записи из `active_traversals`) в `SceneStateManager._enrich_local_positions` на основе монотонного тика.

**Последствия:**  
- Движение NPC детерминировано и не ломается при скачках календарного времени.
- Чёткое разделение: `SceneStateManager` владеет мутациями стейта, `WorldSnapshotBuilder` владеет проекцией.
- Фронтенд полностью синхронизирован с каузальным временем бэкенда.

### ADR-060: Spatial Graph Recovery & Target Resolution Pipeline Restoration
**Дата:** 20.05.2026  
**Статус:** Принято

**Контекст:**  
Пространственный граф `tavern_silver_wolf` был мёртв (SCF=0) из-за отсутствия секции `"positions"` в `location_templates.json`. Это приводило к `feasibility=0` и массовому `FLEE`. Слой 2 (Target Resolution) был слеп к русским именам, так как `npc_positions` не обогащался полем `name` при загрузке сцены. Легаси-поле `"location"` перезаписывало авторитетное `"location_id"`, из-за чего NPC (например, Тень) становились невидимыми для фронтенда. Наконец, `WorldSnapshotBuilder` падал с `NameError` (отсутствовала инициализация `result`), а API крашилось с `TypeError` при попытке вызвать `asdict()` на Pydantic-модели.

**Решение:**  
1. **Пространственный граф:** В `location_templates.json` добавлена секция `"positions"` с макро-узлами. `LocationGraph` теперь успешно парсит их, восстанавливая SCF=1.0.  
2. **Обогащение имён:** В `scene_state_manager.py` (`get_scene_state`) внедрена инъекция `_npc_id_to_display(npc_id)` в `npc_positions`. Это гарантирует наличие русских имён (например, "Тень") для `difflib` в Слое 2.  
3. **Приоритет локации:** В `WorldSnapshotBuilder` изменён приоритет: `data.get("location_id")` теперь имеет преимущество над легаси `data.get("location")`.  
4. **Robust Snapshot Builder:** В `_extract_active_traversals` добавлена обязательная инициализация `result: list = []`. Нарушение принципа чистой проекции устранено.  
5. **Универсальная сериализация API:** В `routes.py` внедрён безопасный конвертер, поддерживающий Dataclass, Pydantic v1/v2 и Dict.  
...
**Контекст:**  
При ходе игрока `DirectiveInterpretationSubscriber` выдавал `[DIRECTIVE_NO_STATE]` и `ObediencePressure=0.00`. Причина: в методе `execute_player_finalize` Фаза 0 (`_phase_0_simulation`), заполняющая `ctx.npc_states`, пропускается. В результате `ctx.all_npcs_raw` оставался пустым, и Труба Воли (приказы) обрывалась. Кроме того, `WorldSnapshotBuilder` слепо пробрасывал `duration_ticks` из `scene_state` вместо вычисления на основе дистанции и скорости, что нарушало гарантии консистентности ADR-059.

**Решение:**  
1. **Инъекция стейта в Player Finalize:** В `tick_orchestrator.execute_player_finalize` добавлено явное заполнение `ctx.npc_states` и `ctx.all_npcs_raw` через `_life_engine.get_npc_states()` сразу после инициализации `_TickContext`.
2. **Вычисление Duration Ticks:** В `WorldSnapshotBuilder._extract_active_traversals` внедрена формула расчёта `duration_ticks = math.hypot(dx, dy) / speed`, гарантирующая честную проекцию кинематики.

**Последствия:**  
- Давление подчинения (ObediencePressure) рассчитывается корректно, Труба Воли замкнута. NPC подчиняются приказам с учётом их страха и легитимности.
- Фронтенд получает детерминированный и вычисленный `duration_ticks`, что исключает рассинхронизацию анимаций Lerp.
- Обнаружена слепая зона Фазы 3 (Память): MemoryProcessor вызывается только для пространственных событий, что приводит к амнезии NPC (требует внедрения SignificanceResolver в следующем спринте).

### ADR-062: Authority of Intent & Legitimacy Physics (Физика Власти)
**Дата:** 21.05.2026  
**Статус:** Принято

**Контекст:**  
LegitimacyGate блокировал подчинение бесстрашных NPC. Формула `legitimacy = max(fear, trust)` и порог `> 0.3` смешивали внутреннее состояние цели и внешнюю силу источника в один скаляр. Если NPC не боялся и не доверял, приказ игрока игнорировался (ObediencePressure=0.00), даже если игрок был вооружён или настаивал.

**Решение:**  
1. **Разделение слоёв:** `AuthorityPressure` (внешняя сила: `source_authority * context_modifier`) отделена от `Legitimacy` (внутренняя готовность: `max(fear, trust)`).
2. **Формула подчинения:** `obedience_intensity = authority_pressure * (0.3 + legitimacy)`. Базовая восприимчивость (0.3) позволяет вооружённому приказу пробить нулевую легитимность.
3. **Формула раздражения:** `irritation_intensity = authority_pressure * 0.3 * (1.0 - legitimacy)`. Давление без уважения вызывает конфронтацию.
4. **Флаг подчинения:** `is_obedience = legitimacy > 0.1`. Истинное подчинение требует легитимности, иначе NPC реагирует, но не подчиняется воле источника.

**Последствия:**  
- Бесстрашные NPC (воры, стражники) больше не игнорируют прямые вооружённые приказы, а реагируют раздражением или неохотным подчинением.
- Устранён магический скаляр, смешивающий физику власти и внутреннюю психологию.

### ADR-063: Ontological Typing for Presentation Layer (Онтология Сущностей)
**Дата:** 21.05.2026  
**Статус:** Принято

**Контекст:**  
Сущность Игрока утекала на экран как обычный NPC. Хардкод-фильтр `if npc_id == "player": continue` на бэкенде был хрупким и не масштабируемым. При появлении новых сущностей (животные, объекты, камеры) фронтенд не имел онтологического контракта для их фильтрации.

**Решение:**  
1. **EntityType Enum:** В `snapshot.py` введён `EntityType(str, Enum)`: PLAYER, NPC, ANIMAL, OBJECT.
2. **Проброс типа:** `NPCPositionDTO` содержит поле `entity_type`. `WorldSnapshotBuilder` помечает `player` как `EntityType.PLAYER`, остальных как `EntityType.NPC`.
3. **Фильтр фронтенда:** `game_screen.py` фильтрует строго по типу: `if entity_type != "npc": continue`. Защита от утечек на уровне презентационного слоя.

**Последствия:**  
- Фронтенд защищён от утечек любых не-NPC сущностей (Player, Horse, Camera).
- Устранён хардкод строк, внедрена масштабируемая онтология.

### ADR-064: Spatial Template Instantiation (Инстанциация Шаблонов)
**Дата:** 21.05.2026  
**Статус:** Принято

**Контекст:**  
При отсутствии Editor JSON для локации, `load_editor_json` возвращал `None`, что убивало пространственный граф (SCF=0). Легитимный fallback на `location_templates.json` был проложен, но прямое использование шаблона создавало риск мутации "идеи" локации при изменении инстанса в `scene_state`.

**Решение:**  
1. **Fallback:** В `graph_compiler.py` добавлен поиск в `location_templates.json` по точному совпадению `location_id`.
2. **Инстанциация:** Шаблон возвращается через `copy.deepcopy()`, отделяя "идею" локации от её "инстанса" в рантайме.

**Последствия:**  
- Пространственный граф собирается даже при отсутствии файлов кампании.
- Мутации `scene_state` не ломают базовый шаблон локации.

### ADR-065: Midnight Temporal Continuity (Защита Полуночи)
**Дата:** 21.05.2026  
**Статус:** Принято

**Контекст:**  
БАГ H (Время откатывается) вернулся в виде регресса. Проверка `if preserved_game_time:` отбрасывала валидное значение `0.0` (полночь), сбрасывая время к дефолту "07:00". Анти-паттерн `or 0` в API-границе также приводил к потере `0.0`.

**Решение:**  
1. Замена `if preserved_game_time:` на `if preserved_game_time is not None:` в `_extract_preserved_time` и `_resolve_initial_time`.
2. Замена `state.shared_context.game_time_seconds or 0` на `getattr(..., 0)`.

**Последствия:**  
- Полночь (`0.0`) — валидное время. Непрерывность времени гарантирована на всём протяжении суток.

### ADR-066: Causal Consciousness Bridge (Каузальный Мост Сознания)
**Дата:** 21.05.2026  
**Статус:** Принято

**Контекст:**  
В idle-пути `tick_decisions` генерировал когнитивные решения (FLEE, APPROACH), но они никогда не конвертировались в `MovementIntent`. Тело двигалось по расписанию (Фаза 0), но было глухо к сознанию (Фаза 5). Аккумулятор давления (`IDLE_DECISION_SCORE_THRESHOLD = 0.15`) работал слишком медленно для пространственных реакций, из-за чего мир замирал после инициализации (SHI=0%).

**Решение:**  
1. **Прямые интенты:** `tick_decisions` напрямую возвращает `idle_movement_goals` (тройки `npc_id, intent, target`), минуя аккумулятор давления.
2. **Мост в Ореркстраторе:** `_phase_5_decision` конвертирует `idle_movement_goals` в `MacroMovementGoal` и передаёт в `MovementEngine` мгновенно.
3. **Разделение каналов:** Давление (pressure) используется только для речи/телеграфа. Движение выполняется сразу по решению `DecisionHub`.

**Последствия:**  
- Тело мгновенно выполняет решения сознания в idle-пути.
- Мир оживает: NPC реагируют пространственно на когнитивные решения (страх, подчинение).

### ADR-067: CDS Dual-Channel Decision Detection & SCF Calibration
**Дата:** 21.05.2026  
**Статус:** Принято

**Контекст:**  
CDS показывал SHI=0% и SCF=0.5 при живой симуляции. Три независимых бага:
1. `TickHealthChecker` не имел метода `on_individual_decision()` — `AttributeError` глотался `try/except` в `_dispatch`, все решения терялись.
2. `[TRACE][DECISION_SCORE]` использовал `print()`, невидимый для пост-мортем лога.
3. `_load_templates()` фоллбэчит с WARNING, но `_find_editor_location()` уже нашёл editor JSON — CDS считал fallback критичным (SCF=0.5), хотя реальный граф загружен корректно.

**Решение:**  
1. **Dual-Channel Detection:** Два канала ловят решения: `[DECISION_HUB]` (основной) и `[TRACE][DECISION_SCORE]` (резервный). Оба вызывают `on_individual_decision()`.
2. **SHI на total_decisions:** `_compute_shi()` использует `total_decisions / total_ticks` вместо `decisions_nonzero_ticks / total_ticks`. Устраняет зависимость от R3_DIRECT-фрейма.
3. **SCF Calibration:** Паттерн `editor_json_found` детектирует успешную загрузку графа. При `editor_json_locations` непустом — spatial_fallback считается некритичным (SCF=1.0).
4. **`print` → `logger.info`:** Все критические маркеры решений переведены на логирование в файл.

**Последствия:**  
- SHI отражает реальное здоровье симуляции (100% при 6 решениях на 2 тика).
- SCF отличает ложный fallback (editor JSON найден) от реальной деградации.
- CDS устойчив к формату лога: если один канал молчит, второй ловит.