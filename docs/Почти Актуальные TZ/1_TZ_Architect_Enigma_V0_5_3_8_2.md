# Техническое задание для архитектора (ТЗ-А) — АКТУАЛИЗИРОВАНО
...
**Дата актуализации:** после S212 (Vertical Slice «Тень и золото» — доказан, SUPERBOX-GORAN 12/12)  
...
**Статус:** ДОКУМЕНТ ИСТОРИЧЕСКИЙ С АКТУАЛЬНЫМИ СТАТУСАМИ

> **КЛЮЧЕВОЕ ИЗМЕНЕНИЕ ЭПОХИ:** после S204 (FOUNDATION FREEZE) доктрина
> изменилась: проблемы этого ТЗ более НЕ являются дорожной картой
> «до всего остального». Vertical Slice «Тень и золото» доказан (S212)
> без закрытия большинства из них. Новый порядок: **таверна (gameplay) →
> infra-долги по gameplay-evidence**. Статусы ниже фиксируют, что
> действительно блокирует, а что стало gaming-задачей.`
...
## 2.1. Сводная статус-таблица проблем (актуализация S206–S212)

| ID | Проблема | Статус | Комментарий |
|---|---|---|---|
| 001 | Прямой доступ NPC к state | 🔴 Открыто | Греп S206: чтения npc_positions живы (pipeline 51,77-81,174,1051+) |
| 002 | Fallback _pos() | 🔴 Открыто | Код не менялся |
| 003 | TODO ExpectationStore | 🔴 Открыто | TODO жив (pipeline 371–376); ADR-атлас утверждает обратное — doc drift |
| 004 | TODO PerceptionEngine | 🔴 Открыто | TODO жив (374–377) |
| 005 | NpcPerceptionCompiler | 🔴 Открыто | PerceptionProjector — по-прежнему UI-only |
| 006 | nearby без фильтрации | 🟡 Частично | Perceiving-канал (filter_perceiving_npcs) существует, но REACTION_SUB fallback жив (лог S212) |
| 007 | Proactive без восприятия | 🟡 Частично | SocialTargetResolver проверяет LoS (лог S212), но nearby-вход не фильтруется |
| 008 | Transfers race | 🔴 Открыто | _pending_transfers не менялись |
| 009 | R7/R8 композиция | 🟡 Реклассифицировано | Закрыто ИНАЧЕ: Epistemic Dispositions S211 (archetype-взвешивание); формализация R7/R8 — gaming-задача |
| 010 | Инварианты в CI | 🟡 Частично | IPT 44/44 (44 инварианта); assertion-layer SUPERBOX — частично |
| 011 | remembered relationship | 🔴 Открыто | — |
| 012 | WillpowerGate observer | 🔴 Открыто | — |
| 013 | EpistemicStore decay | 🔴 Открыто (долг) | Зафиксирован как пост-slice задача; crystallized не decay — согласовано |
| 014 | Type-level разделение | 🔴 Открыто | — |
| 015 | Детерминизм | 🟡 Частично | INV-REPLAY-DETERMINISM WARNING-level; побитовый тест TickState→TickMutation — нет |
| 016 | Persistence tests | 🟡 Существенно закрыто | EpistemicStore round-trip (S193); avatar (S208, 5/5); relationship_cache/memory_weights — нет |
| 017 | Silent failures | 🟡 Частично | DEBT-R4 закрыт (S208); R5 = 41 blanket-except в game_loop — ЖИВ; TICK_CRASH проглатывается (факт S212) |
| 018 | NearbyContract | 🔴 Открыто | — |
| 019 | CausalClosureInvariant | 🟡 Частично | Probes CausalProvenance (S174a) — scenario-level; глобального инварианта нет |
| 020 | AuditoryEvent | 🟡 Частично | ACTION_PERCEPTION_RADIUS SSOT (S210) — топология готова; AuditoryDistortionPolicy — по-прежнему UI-only; LLM-пропозиции (Phase 9.1) — открыто |
...
## 7. Этапы реализации — АРХИВНАЯ ДОКТРИНА (заменена в S204/S212)

> Исходные Фазы 0–7 сохранены в git-истории документа. Они предполагали
> «закрыть все 20 проблем до геймплея». Доктрина изменена решениями:
>
> - **S204 FOUNDATION FREEZE:** эпистемический фундамент стабилизирован;
>   архитектурная экспансия остановлена.
> - **S212:** Vertical Slice «Тень и золото» доказан (SUPERBOX-GORAN 12/12)
>   на фундаменте, где большинство проблем этого ТЗ оставались открытыми.
>   Эмпирический вывод: **они не блокируют каузальную честность ядра** —
>   они блокируют масштаб, полировку и отладку.

### 7.1. Действующая доктрина (S212+)

```
ТАВЕРНА (gameplay vertical slice) — первоочередной объект
  → SMOKE-GORAN (α): emergent crime rate / witness rate /
    false-accusation rate (gameplay research, не архитектурный тест)
  → DEBT-S1: OpportunityContext-строитель (условие возможности α:
    вход opportunity-канала из живого state — attention из PerceptionLayer,
    дистанция из SpatialQuery, allies из RelationshipStore)
  → калибровка dispositions/radii по игровым данным
    (Calibration Laboratory, ADR-O-361, готова к употреблению)

ИНФРА ПО GAMEPLAY-EVIDENCE — второй приоритет
  → R5 (41 blanket-except в game_loop): приоритизируется по частоте
    срабатываний в живых сессиях (TICK_CRASH-громкость — решение при
    первом повторе инцидента класса S212)
  → ARCH-017 расширение IPT — по мере новых феноменов
  → ARCH-001/002/014 (телепатия pipeline): когда таверна покажет
    конкретные телепатические инциденты в gameplay

ЗАКРЫТИЕ ОСТАЛЬНЫХ ПО ЗАКАЗУ ГЕЙМПЛЕЯ
  → ARCH-013 (decay): когда NPC начнут «забывать» в долгих играх
  → ARCH-011 (remembered relationship): вместе с decay
  → ARCH-020 (auditory + LLM Phase 9.1): при первом живом
    заблуждении игрока от неслышанной реплики
  → ARCH-005/006/018 (единый perception compiler): при росте
    числа параллельных perceiving-потребителей
  → ARCH-008/012/015/019: по стандартному триажу

ЗАКОНЫ/ДОЛГИ ЭПОХИ (зафиксированы, исполняются вне этого ТЗ)
  → §18 Singular Epistemic Authority (Устав, S211)
  → DEBT-E1: Player Epistemic Authority Consolidation (6 шагов)
  → реестр DEBT-R1…R10, DEBT-S1, CAL-1, BUILD-P1…P6 (MUTATIONS S206–S212)
```

### 7.2. Приоритеты этого ТЗ в новой рамке

- **P0-инфра (блокирует отладку живой игры):** 017 (R5 + громкость
  TICK_CRASH), 010 (расширение IPT по феноменам).
- **P1-gaming (блокирует правдоподобие таверны):** 013, 011, 020,
  009→dispositions-калибровка (форма изменилась: S211).
- **P2-масштаб (при росте системы):** 001, 002, 005, 006, 014, 018,
  008, 012, 015, 019.
- **Закрыто/реклассифицировано:** 016 (существенно: S193/S208),
  009 (иначе: S211), 007 (частично: SocialTargetResolver LoS).
...
## 11. Критерии готовности — ПЕРЕСМОТРЕНЫ (S212)

Исходный критерий «все 20 закрыто до геймплея» устарел: Vertical Slice
доказан без него (S212, 12/12). Действующие критерии:

1. Каждая проблема имеет **честный актуальный статус** (см. §2.1) — закрыто /
   частично / открыто / реклассифицировано — со ссылкой на сессию-доказательство.
2. Открытые P0-инфра (017/R5, TICK_CRASH-громкость) закрываются **по
   gameplay-evidence** — после первых живых сессий таверны, не раньше.
3. Реклассифицированные закрываются в новой форме: 009 — dispositions
   (уже доказано S211, SUPERBOX-DISPOSITIONS 6/6); 016 — avatar (S208, 5/5) +
   EpistemicStore round-trip (S193).
4. Каждое закрытие сопровождается SUPERBOX-тестом через живой контур —
   стандарт S206–S212 (финальный тест эпохи нашёл 7 живых production-багов;
   каждый следующий должен работать так же).
5. Побитовый детерминизм (п.7 исходный) и TODO-чистота (п.3 исходный)
   остаются **целевыми состояниями P2-масштаба**, не гейтами таверны.
...
### Приложение А. Сводная таблица верификации проблем (историческая, V0.5.3.8.2)

> Статусы «✅ Подтверждено» ниже — верификация НА МОМЕНТ ИСХОДНОГО АУДИТА
> (проблема существовала в V0.5.3.8.2). Актуальные статусы выполнения —
> §2.1 (актуализация S206–S212): часть проблем закрыта, часть
> реклассифицирована, часть подтверждена как живая.
