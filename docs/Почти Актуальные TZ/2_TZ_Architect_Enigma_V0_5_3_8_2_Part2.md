# Техническое задание для архитектора (ТЗ-А2) — АКТУАЛИЗИРОВАНО

**Проект:** Enigma — каузальное ядро симуляции NPC  
**Версия проекта:** V0.5.3.8.5 (актуализация по состоянию после S212)  
**Версия документа:** 2.0 (актуализация; исходная верификация — V0.5.3.8.2)  
**Дата актуализации:** после S212 (Vertical Slice «Тень и золото» — доказан)  
**Адресат:** Архитектор систем симуляции / tech-lead backend  
**Автор верификации:** AI-аудит V0.5.3.8.2 + актуализация по сессиям S206–S212  
**Статус:** ДОКУМЕНТ С ТРЁХКАТЕГОРНОЙ КЛАССИФИКАЦИЕЙ ПРОБЛЕМ
> Актуализация показала: часть проблем 21–40 **перекрыта архитектурой
> Epistemic Core (S188–S212)** — их онтологические предпосылки устарели.
> Проблемы не «закрыты», а **АРХИВНЫ**: их DoD описывал фикс легаси-слоёв,
> которых больше нет в каузальном пути.`
...
## 2.1. Сводная трёхкатегорийная классификация (актуализация S206–S212)

### КАТЕГОРИЯ А — АКТУАЛЬНО (живые проблемы, доктрина S212+)

| ID | Проблема | Статус | Комментарий |
|---|---|---|---|
| 021 | Магические числа без калибровки | 🟡 Сдвинуто | **Calibration Laboratory (ADR-O-361) ПОСТРОЕНА** параллельной сессией: CalibrationProfile-пресеты, config_overlay, preset_io, identity-патч. DoD пп.1–3 фактически готовы; остались: перенос литералов interpretation/life/belief_transition + линтер lint_magic_numbers |
| 022 | max_workers=1 (LLM-сериализация) | 🔴 Живо+подтверждено runtime | «aborting stuck request» воспроизводится в прогонах S212. Root-cause + ADR + per-task timeout — все DoD актуальны. КРИТИЧНО для таверны (canonical-реплики зависают) |
| 023 | Referential Closure без верификации | 🔴 Живо | dm_phase-принципы живы; инварианта нет |
| 026 | SpeechScheduler без сложных тестов | 🔴 Живо+новые факты | S212: PACING=2с wall-clock, DEDUP_TTL=4с — тесты конкурентности тем важнее |
| 027 | Backpressure | 🟡 Частично | INV-DIALOGUE-LIVENESS (pending ≤ 20) — потолок есть; drop/degrade-политики нет. КРИТИЧНО с учётом 022 |
| 028 | Need-priorities без приоритизации | 🔴 Живо | — |
| 029 | Random events фикс. частота | 🔴 Живо | — |
| 030 | STM без забывания | 🟡 Частично перекрыто | Interrupted-диалог: S202 обходит STM-гейт для claim-producing; decay-механизма нет |
| 031 | Initiative: бинарный паралич | 🔴 Живо | — |
| 032 | Social target без отношений | 🟡 Частично | S212-лог: SocialTargetResolver выбирает по LoS/дистанции; отношения в выборе — не видно |
| 033 | Melee-only дистанции | 🔴 Живо | — |
| 036 | Bias без observer-контекста | 🔴 Живо | — |
| 037 | Bypass'ы расписания разрознены | 🔴 Живо | — |
| 038 | Reconstruction fallback'и тихи | 🔴 Живо | S203 чинил соседнее (intent_type-проброс); тишина fallback'ов — как была |

### КАТЕГОРИЯ Б — ПЕРЕКРЫТО (онтология устарела после Epistemic Core)

| ID | Проблема | Чем перекрыто |
|---|---|---|
| 035 | BeliefFragment без валидации против ObjectiveTruthState | **Epistemic Core (S188–S212)**: EpistemicRecord/Proposition — убеждения субъективны по закону (Invariant V; «confidence ≠ truth probability»). Валидация против ObjectiveTruth = ЗАПРЕЩЁННЫЙ паттерн: гейт ACCUSE (S211, §18) доказывает, что истина проверяется эмерджентно (NPC-реакциями), не движком. Ложное обвинение прохождение гейта — ФИЧА slice. DoD противоречит §18 |
| 039 | Belief cooling/decay | Частично перекрыто: EpistemicStore-онтология (S193) ≠ BeliefFragment-легаси; decay — переносится в реестр долгов как DEBT (арх-013 ТЗ-А1, подтверждён пост-slice) |
| 040 | Ambient vs Canonical без границы | Частично перекрыто: критерий формализован ПРАКТИКОЙ (task_scheduler:133 — «WARN/TALK с proposition обязаны быть canonical»; intent_profiles produces_claim — S202; routing ambient→NpcConversation — S198). DialogueClassification-классификатор — не построен, но граница ДЕЙСТВУЕТ и протестирована (S212: 13 canonical не размазались в ambient). Остаток — линтер-запрет прямого task_type: опционально |

### КАТЕГОРИЯ В — АРХИВ (DoD описывал другой мир)

| ID | Проблема | Причина архива |
|---|---|---|
| 034 | *(если есть в полном файле — вне прочитанного фрагмента)* | — |

> **Примечание:** проблема 034 не попала в прочитанный фрагмент файла —
> её статус определяется при первом полном прочтении; категория В
> зарезервирована.
...
> **🔄 АКТУАЛИЗАЦИЯ S212 (Категория А, сдвинуто):** Calibration Laboratory
> (ADR-O-361) ПОСТРОЕНА параллельной сессией — CalibrationProfile-пресеты,
> `config_overlay.py` (identity-патч констант, вложенный overlay запрещён),
> `preset_io.py`, `preset_materializer.py`, experiment_runner. DoD пп.1–3
> инфраструктурно готовы. Остаток: (а) перенос литералов из
> interpretation_engine/life_engine/belief_transition_engine в профиль,
> (б) линтер `lint_magic_numbers.py`, (в) A/B-гейт в drift_laboratory.
> Исполнитель: совместим с SMOKE-GORAN (α) — калибровка dispositions/radii
> пойдёт через эту лабораторию.
...
> **🔄 АКТУАЛИЗАЦИЯ S212 (Категория А, runtime-подтверждено):** баг жив и
> воспроизводится — «[R4A_WORKER] aborting stuck request» в каждом прогоне
> G-теста; canonical-реплики зависают в очереди (13 задач копятся без
> адмита при подвисшем вызове). DoD полностью актуален; приоритет поднят
> до **P0-таверны**: без per-task timeout живые диалоги будут теряться
> молча. Связка: 027 (backpressure) + 038 (reconstruction) — один ADR
> на пул задач LLM-слоя.
...
> **🔄 АКТУАЛИЗАЦИЯ S212 (Категория А, частично):** потолок существует —
> `INV-DIALOGUE-LIVENESS` (pending_tasks ≤ 20, IPT). Политик drop/degrade/
> per_npc_max — нет. С учётом runtime-подтверждённого 022 (зависание
> canonical) backpressure поднимается в приоритете: зависший LLM-вызов +
> отсутствие drop-политики = тихая потеря эпистемически значимых реплик.
...
> **🔄 АКТУАЛИЗАЦИЯ S212 (Категория А, живо):** принципы в dm_phase живы,
> инварианта нет. Остаток DoD актуален; тест-сценарий «тот парень» —
> хороший кандидат в IPT-пополнение при первом живом феномене неразрешённой
> ссылки в таверне.
...
> **⚖️ АРХИВ ПО ОНТОЛОГИИ (S206–S212, §18):** DoD противоречит действующему
> закону. Epistemic Core доказал: убеждения субъективны по построению
> (Invariant V); валидация против ObjectiveTruthState = телепатия истины.
> Факт «NPC ложно уверен бесконечно долго» — не баг, а ФИЧА slice: игрок
> с conf=0.9 обвиняет невиновного (SUPERBOX-ACCUSATION C4, зелёный).
> Коррекция убеждений происходит эмерджентно (вражеское testimony ослабляет
> belief — доказано S206 BEFORE/AFTER), не движком-оракулом. Заменитель
> валидации: decay (см. 039/арх-013 ТЗ-А1).
...
> **⚖️ ПЕРЕКРЫТО ЧАСТИЧНО → ПЕРЕНОС В ДОЛГ (арх-013 ТЗ-А1):** онтологическая
> база проблемы (BeliefFragment/belief_transition_engine) — легаси-слой вне
> каузального пути Epistemic Core (EpistemicStore/BeliefRevisionEngine —
> S188+). Сама идея decay валидна и уже в реестре: пост-slice задача для
> EpistemicStore (per-proposition-type; crystallized не decay — согласовано
> с DoD п.4). DoD переписать под EpistemicRecord при взятии в работу.
...
> **⚖️ ПЕРЕКРЫТО ПРАКТИКОЙ (S198–S212):** граница формализована и ДЕЙСТВУЕТ:
> task_scheduler:133 («WARN/TALK с proposition обязаны быть canonical»),
> intent_profiles.produces_claim (S202), routing ambient→NpcConversation
> (S198, ADR-O-342), steal→THEFT-мост (S209). Поведение протестировано
> живым прогоном S212 (13 canonical не деградировали в ambient; WARN
> материализовался через canonical-путь). Остаток опционален:
> DialogueClassification-классификатор + линтер как P2-гигиена при
> росте числа intent-типов. ADR на классификацию — не требуется,
> действует прецедент S198.
...
> **🔄 АКТУАЛИЗАЦИЯ S212 (Категория А, новые факты):** арбитраж прочитан в
> живом коде: PACING = 2.0с wall-clock, DEDUP_CONTEXT_TTL = 4.0с (мин.
> latency между репликами NPC). Факт S212: pacing блокирует canonical-
> реплику после недавней ambient того же NPC — случаи конкурентности
> из DoD тем актуальнее. SpeechSchedulerProbe — кандидат в IPT.
...
## 15. Предложения к удалению (актуализация S212)

### 15.1. Секции внутри ТЗ-А1/А2 — к удалению/сворачиванию

| Объект | Предложение | Обоснование |
|---|---|---|
| ТЗ-А1 §7 «Фазы 0–7» | ✅ Уже заменено (батч актуализации) | Доктрина S204/S212 |
| ТЗ-А1 §8 «Приоритизация» (таблица P0/P1/P2) | 🗑 **Удалить таблицу, оставить заголовок-ссылку на §2.1 и §7.1** | Приоритеты пересчитаны в §7.1 новой доктрины; старая таблица вводит в заблуждение («P0: 001–019» — но slice доказан без них) |
| ТЗ-А1 §12 «Риски и смягчения» (строки про SubjectivePerception/R7-R8) | 🗑 **Удалить строки про R7/R8-формализацию и determinism-test** | R7/R8 перекрыто dispositions (S211); determinism — P2-масштаб в новой рамке. Остальные строки рисков оставить |
| ТЗ-А1 §13 «Метрики» (метрики по Фазе 1, 100-seeded determinism) | 🗑 **Свернуть до строк, актуальных для SMOKE-GORAN**: crime rate, witness rate, time-to-theft, false-accusation rate, queue-overflow, scheduler-stall | Метрики привязаны к удалённым фазам |
| ТЗ-А2 §4 «Принципы 9–15» | ✏️ **Оставить все** | Все семь живы; п.9 получил исполнение (Calibration Lab), п.11 подтверждён runtime (022), п.12 подтверждён практикой S198 |
| ТЗ-А2 карточки 035, 039 (полные DoD) | 🗑 **Свернуть до стампов** (тексты в git-истории) | Онтология устарела (Категория Б); полные DoD описывают фикс легаси-слоёв вне каузального пути и могут ввести в заблуждение |
| ТЗ-А2 карточка 040 (полный DoD пп.1–5) | ✏️ **Оставить с примечанием «остаток: классификатор P2»** | Граница действует; классификатор — опциональная гигиена |
