...
11. **ЗАКОН ФАЗОВОГО ПЕРЕХОДА ИДЕНТИЧНОСТИ** "Поведение может быть мгновенным. Память — только отфильтрованная стабильность поведения."

L3 — это флюид (текучая физика момента). drives_runtime — это кристалл (закреплённый след). Между ними обязан стоять фильтр фазового перехода, иначе система превратится в осциллограф, а не в эволюционный интеллект.
...
## 7. ЗАКОНЫ ЭПИСТЕМИЧЕСКОЙ НЕОДНОРОДНОСТИ И ВОСПРИЯТИЯ

Система оперирует не объективной реальностью, а распределёнными локальными историями. ENIGMA = N локальных историй, социальная агрегация которых порождает иллюзию общей реальности.

1. **Закон Эпистемической Неоднородности:** L1 Chronicle каждого NPC — персонализированная запись воспринятого, не объективная хроника мира. Разные NPC фиксируют разные наборы событий. Консенсус = реконструкция пересекающихся, но не идентичных хроник.
2. **Закон Перцептивной Истины:** Убеждение (Belief) строго индивидуально. Консенсус — иллюзия. Опыт > Давления: без personal_persistence Belief не формируется.
3. **Закон Тройной Мембраны:** L1 Chronicle фильтруется через 3 мембраны: Физическую (расстояние, видимость), Личностную (compliance_bias, threat_sensitivity) и Социальную (Norm-модулированные пороги). Все слои только фильтруют, ни один слой не инжектит события в L1.
4. **Закон Социальной Линзы:** InstitutionLayer и Norm не создают Belief напрямую. Они модулируют пороги кристаллизации и восприятия, опуская порог записи слабых событий в L1, которые иначе были бы отфильтрованы как шум.
5. **Закон Асимметричной Травмы:** Опровержение (отрицательное подтверждение) в 6 раз сильнее подтверждения при пересмотре убеждений (Belief Revision).

---

## 8. Архитектурное Правило KernelRNG

1. **Правило изоляции подсистем** (Subsystem Isolation Rule):
    Во избежание эволюционного дрейфа RNG (когда добавление нового rng.random() в Фазе 1 сдвигает последовательность в Фазе 5), каждая подсистема ENIGMA (DecisionHub, MovementEngine, StateApplicator, LifeEngine events) обязана использовать независимый поток случайностей, генерируемый через уникальный salt.

2. **Создание RNG**:
    rng = KernelRNG(tick=tick, npc_id=npc_id, salt="<имя_подсистемы>")

3. **Передача RNG**:
    Созданный rng передаётся вниз по стеку вызовов подсистемы. Пересоздание rng внутри подсистемы запрещено.