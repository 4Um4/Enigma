...
Суть: Различие «существует в мире» (Actor) и «управляет своим действием автономно» (AutonomousDecisionAgent) канонизировано осью `ControlSource` (PLAYER_INPUT / NPC_DECISION / SCRIPTED_SYSTEM / COMBAT_CONTROLLER). Единственная точка знания 'player'-строки — `domain/control_source.py`; новые строковые гарды агентности запрещены (легаси мигрируются постепенно). Автономный decision pipeline не порождает авторский акт аватара: CommunicationIntent(speaker="player") запрещён в любой фабрике. Четыре слоя защиты: (0) фильтр decision population (_phase_5_decision, tick_orchestrator), (1) гвард DecisionHub._build_communication, (2) гвард attack-фабрики npc_tick_pipeline, (3) детектор SimulationIntegrityError(INV-PLAYER-AUTHORSHIP) перед возвратом TickMutation (громкое падение, ADR-INV-DEF). Player остаётся легальным target / observer / источником событий / объектом восприятия и убеждений. Психика аватара — player-input pipeline (ADR-TZ08-1) + AvatarStateApplicator (S208; DEBT-R10 → активная онтология уровня 0). Рождён археологией S292: ghost-реплика игрока («Тень — это как зеркало…») воспроизводилась цепью player∈_alive_npcs → DecisionHub → QueuedTask(owner="player") → DialogueExecutor LLM-перефраз → NPC_SPOKE(source="player"); S290-гард в подписчике — проекционный пластырь (снимается в F2 после нормализации RCE). Контекст будущего: мультиплеер (500 NPC / 10000 players) требует Actor ≠ AutonomousDecisionAgent как фундаментального различия; COMBAT_CONTROLLER зарезервирован пошаговым боем.
❌ Taboo: CommunicationIntent(speaker="player") из autonomous pipeline; новые строковые гарды 'player' как предиката агентности (только resolve_control_source); тихое удаление не-NPC интентов (только raise); расширение _PLAYER_ACTORS без записи в ADR; исключение player из мира (позиции/восприятие/цели/события) — запрещено только АВТОРСТВО.
