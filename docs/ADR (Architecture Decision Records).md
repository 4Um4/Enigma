...
`ADR-O-386` [ONTO] **PROTECT — Territory Ownership: власть над пространством (Visible World / Living Activity, срез PROTECT)**
Суть: territory → ownership → trespass → authority to act. NodeRef.owner (Optional; компиляция graph_compiler:113, конструкторы :290/:948 не тронуты — дефолт None) → SpatialService.zone_owner/territory_owners → TRESPASSED-предикат в SpatialEventDetector (Фаза 2, секция переходов узлов; TERRITORY_ENABLED env default OFF = no-op по составу событий, G2-паттерн; payload {trespasser, owner, node}, public r=15; lookup инъектируется оркестратором из разрешённого spatial_service) → территориальный authority-пропуск в COMBAT_CAPABLE-гейт decision_hub:962 (L-A1: территория ИЛИ роль — мирный владелец получает право block_path/ambush). Факт territory_pass предвычислен в npc_tick_pipeline из read-only spatial_service TickState — Hub остаётся чистым (compute → _get_possible_intents → _is_intent_available, дефолты False = инертно). Case-фикс SocialEngine.compute_social_modifiers (автономный, вне флага — прецедент R1; инвариант Мастера «фикс ≠ баланс», 6/6): lowercase-ключи + ремап THREATEN→INTIMIDATE (Intent.THREATEN не существует — решение Мастера, вариант а; max-слияние каналов без двойного учёта) — оживление jealousy/protect/fear/debt-lever. Контент: owner на бар-кластере tavern.json — bar_pass_north/bar_side/behind_bar → tavern_keeper_tornin (членство скорректировано по полной карте против ADR-текста: bar_area — клиентская сторона NodeRole.BAR «для гостей», bar_west — публичный транзит; фиксировано в IMPACT). Evict NPC→игрок — v1.5 (продюсер директивоподобного события через существующий DirectiveInterpretationSubscriber → давление на аватар S208 — труба не строится). Приёмка: SUPERBOX protect_vertical_test 5/5 (P1 владельцы живого графа; P2 TRESPASSED payload; P3 шина/SPY; P4 право: мирная роль «Хозяин таверны» + территория → block_path открыт, без права → блок; C1 OFF — нем, дифф системно доказан). Открытые: TRESPASSED → давление владельца (восприятие→скоринг — калибровка); эмерджентный выбор block_path; автономные нарушители родят WORK/SOCIAL.
Taboo: ❌ роль как источник власти (L-A1 — только территория или боевая роль); ❌ DecisionHub исследует географию (факт приходит параметром); ❌ TRESPASSED без флага/lookup (no-op); ❌ owner на клиентских узлах; ❌ новые Intent ради THREATEN; ❌ модификаторы меняют баланс (только восстановление чтения); ❌ uuid4.
Status: ACTIVE (срез PROTECT v1 доказан: protect_vertical_test 5/5; юниты territory 11/11 + case-fix 6/6; IPT 45/45)
Files: backend/app/models/spatial_contracts.py (NodeRef.owner), backend/app/services/spatial/graph_compiler.py, backend/app/services/spatial/spatial_service.py (zone_owner, territory_owners), backend/app/services/spatial/spatial_event_detector.py (TRESPASSED, territory_enabled), backend/app/services/tick_orchestrator.py (Фаза 2), backend/app/services/npc/npc_tick_pipeline.py (territory_pass), backend/app/services/npc/decision_hub.py (гейт), backend/app/services/social/social_engine.py (case-фикс), backend/app/services/events/event_types.py (TRESPASSED), frontend/map_editor/campaigns/Open_road/locations/tavern.json (owner ×3), backend/tests/test_territory_owner.py, backend/tests/test_social_modifiers_case_fix.py, backend/tests/sandbox/SUPERBOX/scenarios/protect_vertical_test.py, docs/audits/ADR-O-386_IMPACT.md`
