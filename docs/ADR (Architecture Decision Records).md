...
`ADR-O-393` [ONTO] **Determinism Foundation Phase A — P0-энтропия причинного контура устранена (IRON RIVER D-1/F1-F4 + D-2 мини-гейт)**
Суть: Контракт D («same inputs → same trajectory») был невыполним кросс-процессно даже в idle (IRON RIVER: 50 дифов, дивергенция с ≤10 тика). Закрыты все четыре P0-брейкера: wall-clock в состоянии сцены (timestamp→game_time ось + real_ts UI-оси вне канона + TTL на единую ось), боевой RNG (uuid4+hash()→KernelRNG(tick,actor,"combat") + детерминированный event-id), изоляция лаборатории (полное temp-окружение: копия data/ с чистым world_tick, saves в temp; production нетронут), admission речи (SpeechScheduler на game-time: pacing 2 тика / DEDUP 4 тика — калибровка Мастера, темп замедлен сознательно). Построен красно-зелёный D-2 мини-гейт: iron_river_ab (канон-хеш A/B, 2×150 тиков) + iron_river_diff (полевой диф). Базлайн-канон 0c15ff5b... Остаточная недетерминированность измерена и локализована: пограничный eat-онсет maid_lusya (тик 148/149), диффы 2-10 полей — класс async-интерливинга P2-5 + эфемерные поля снапшота; NOT P0, первая цель Phase B.
Taboo: ❌ wall-clock в каузальных полях состояния (timestamp/started_tick/admitted_ts — только оси тика; real_ts — UI-домен, из канона исключён); ❌ uuid4/Python-hash в kernel-событиях и seed'ах (KernelRNG-деривация); ❌ лаборатория без полного temp-окружения (sessions/world_tick изолируются копией data_dir); ❌ admission по реальному времени (game-time ось); ❌ обрезка каузальных входов в тест-копиях (память NPC полная; только replay.db/logs исключаем); ❌ оптимизация до зелёного гейта (каждый P-патч проходит через A/B-базлайн); ❌ вердикты на умирающем диске (место+temp-счётчик — преусловие каждой серии).
Status: ACTIVE (Phase A GREEN: F1-F4 закрыты с runtime-доказательствами; IPT 45/45; residual в реестре Phase B).
Files: `backend/app/services/game_loop/task_scheduler.py`, `backend/app/services/game_loop/speech_scheduler.py`, `backend/app/services/tick_orchestrator.py`, `backend/app/services/combat/combat_subscriber.py`, `backend/tests/sandbox/iron_river_ab.py` (NEW), `backend/tests/sandbox/iron_river_diff.py` (NEW), `backend/tests/sandbox/f3_combat_smoke.py` (NEW)
