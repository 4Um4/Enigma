...
### 9.1. Baseline ДО — ЗАФИКСИРОВАН (2026-09-05, c0d3ff50-контур, живой LLM)

Инструмент: terminal_cockpit.py, env `D8P_BASELINE_NO_DETACH=1` (wait без
класс-отцепления экстрактора — обвязка c7a4a644/c0d3ff50; инцидент частичного
применения c7a4a644: 3.53s v1-прогона НЕДЕЙСТВИТЕЛЕН; после фикса — полное
if/else, deletion-чек, якорный контроль). Сценарии: here-string `new / wait 20 /
exit` → `reports/d8p_baseline_p1_v2.log`; контрольный speech-ход `борко: привет`
+ wait 5 → `reports/d8p_baseline_p1b_r2.log`.

**R2 (семантика): КРАСНЫЙ — 23 события RE-D2 в 20 тиках wait** (P1-retry :68–:158;
плюс 7 в P1b). Guard соседа сработал: экстракция на потоке петли → RuntimeError →
пустой DialogueUpdate = семантическая смерть player-диалогов при живом LLM
([LLM_MANAGER] «уже запущен»; DM-тексты живые). Прогноз досье («R2-репродукция в
тиках wait») — подтверждён живьём. Аномалия-паттерн: все события
partner=thief_shadow (сконцентрированный ambient-производитель, Н-56-класс) —
наблюдение, не фикс.

**R1 (время, cockpit-контекст): ЗЕЛЁНЫЙ — 4.17с / 20 тиков (0.69с / 5 тиков).**
Guard отрабатывает до сетевой попытки → быстрая деградация вместо блокировки.
По вердикту владельца §13.2: R1-зелёный НЕ опровергает D8P — D8P существует для
устранения сцепки, порождающей R2 и потенциально R1. Ожидаемо по прогнозу.

**Контексты не совпадают (важно для ПОСЛЕ):** cockpit wait = поток петли →
R2-домен; uvicorn REST idle_tick (`def`-эндпоинт) = threadpool → worker-ветка
роутера → реальные 3.3–4.3с sync-LLM в HTTP-потоке тика = R1-домен (P2, живой
uvicorn, отдельная ветка прогона; warm-up-тик вне метрики). Исторический красный
R1 (wait-12 → 30+ мин; S248: 60.03с) — до-guard петлевой дедлок.

Открытые пункты: P2 (uvicorn) — исполняется; stdin-v1 — вопрос владельцу
(ход игрока в логе v1 без new-маркеров; буферизация stdout; не блокер).

### 9.2. Baseline P2 — живой uvicorn (2026-09-05; threadpool-контекст, живая LLM)

Контур: `uvicorn app.main:app` (127.0.0.1:8000, PID 16508, старт из backend/);
llama-server HTTP 200 (гейт P2-0). Сценарий: warm-up-тик (вне метрики) + 20×
`POST /api/game/idle_tick/Open_road`; тайминг = curl `%{time_total}` per-request
(в обход PS-таймингов/прокси — урок launcher:25). R2 — только по новому хвосту
cds_backend.log (маркер :4315) + редирект-логам uvicorn. Логи:
`reports/d8p_uvicorn_out.log` / `reports/d8p_uvicorn_err.log`.

**R1 (время, прод-контекст = контекст финального критерия): КРАСНЫЙ —
total=110.96с / 20 тиков** (mean 5.55с; min 0.36; max 10.65). Критерий
`wait 20` ≤ 10с нарушен ×11. Per-tick-профиль бимодален: 2 тихих тика
(0.36/0.42 — без extraction-событий); ~13 тиков 3.1–5.2с (одна sync-экстракция
~3.3–4.3с в HTTP-потоке тика); ~5 тиков 9.6–10.7с (две последовательные
LLM-работы в стеке). NPC_DIALOGUE_SUB = 56 / 21 тик — экстракционные
возможности концентрируются (тот же partner-паттерн, что в P1).

**R2 (threadpool-контекст): 0 RE-D2** (cds-хвост и оба редиректа) — как
предсказано §9.1: guard loop-семантику не встречает; экстракция исполняется
реально (worker-ветка роутера) — медленно, но семантика сохраняется.

**Сведение осей (главный вывод baseline):** сцепка «экстракция ↔ поток
публикатора» проявляется КАК R1-отказ в threadpool-контексте (блокирует
HTTP-поток тика: 110.96с) и КАК R2-отказ в loop-контексте (guard → пустой
DialogueUpdate: 23 события, P1). Один дефект — два режима отказа по контексту.
D8P устраняет сцепку → обе оси зелёные: тик перестаёт ждать экстракцию (R1),
экстракция перестаёт исполняться в потоке публикатора (R2). Историческая
калибровка: 110.96с ≠ 30+ мин wait-12 (до-guard петлевой дедлок 60с×N);
сегодняшний R1 = честная sync-блокировка реальными LLM-вызовами.

**BASELINE ЗАКРЫТ:** R1 КРАСНЫЙ (прод-контекст) + R2 КРАСНЫЙ (cockpit-контекст),
обе оси при живой LLM → Шаг 3 (код D8P) разрешён (порядок §13.4). Мир Open_road
продвинут ~21 тик (согласовано владельцем исполнением P2).

### 9.3. Wired-smoke (Шаг 3.2/3.3) — корректирующая запись честности

**v1 (303b839f, КОНФАУНД):** smoke-команда без D8P_BASELINE_NO_DETACH=1 →
cockpit-патч отцепил экстрактор (no_detach=False) → RE-D2=0 объяснялся
патчем, не очередью; [D8P_Q]-APPLIED невидимы (INFO ниже порога консоли);
сообщение коммита 303b839f завышено («счётчики живы» — не доказано).
**v2 (валидно; no_detach=True + logging.basicConfig(INFO) + живая LLM):**
RE-D2 = **0 при целой сцепке** (baseline P1-retry: 23 RE-D2 тем же
сценарием) — loop-exile by construction доказан; **[D8P_Q] APPLIED ×2**
(event=UUID e10852ee…/33b9dc17…, listener=player/maid_lusya, parent_tick=
103/104) — очередь+Q5 живы, LLM-экстракция исполнилась на пуле (R4A_POOL
395 chars / 4533ms); `wait 5` = 1.08с — тик не ждёт интеллект (R1-ось).
Парное сравнение с P1b (тот же сценарий): ДО 0.69с/7 RE-D2/смысл потерян →
ПОСЛЕ 1.08с/0 RE-D2/2 APPLIED. Уровень = wired-smoke; финальный критерий —
P2-uvicorn ПОСЛЕ-замер (Шаг 4).

**Ruff-история сессии (процессное, уроки):** fix=true проекта трижды
автоприменял правки до ревью (7acbaeb1 — 1 правка; W2 — 5; 303b839f —
1 whitespace conclusion_store-импорт, поведенчески нулевая) — гейты
впредь только `ruff check --no-fix`. 15 F401/W291 в game_loop —
pre-existing (:27–:2100, вне D8P-ханка :388–:412), чужой долг, не чинится
в этой серии (коллизионный файл). Teardown-шум «submit after interpreter
shutdown» — straggler pool-воркер на выходе, наблюдаем, не чиним (pro-релиз
не встречает interpreter shutdown). Wire = replace-семантика (restart →
новая очередь со свежими провайдерами; in-flight прежней завершаются
против её старых провайдеров, STALE-гейт осиротевший мир отсекает).

### 9.4. ПОСЛЕ-замер Шага 4 — ФИНАЛЬНЫЙ КРИТЕРИЙ ТЗ ЗАКРЫТ (2026-09-05, ~22:45–23:00)

Tree-state: HEAD b9b6f90c (+ их WIP: subscriber RE-01-M1b.2.4, task_scheduler
SELF_TALK_SENTINEL — оба композируются с D8P-веткой; BB3/CC4-verified), кэш
очищен (CC1). LLM жива (8181/health 200).

- **P1-ПОСЛЕ (cockpit, сцепка цела, no_detach=1): wall-clock 4.55с / 20 т**
  (baseline 4.17с с мёртвой экстракцией; 23 RE-D2 → **0**); APPLIED=2, STALE=0.
  Хронология лога = рельное доказательство: APPLIED-строка ПОСЛЕ wall-clock —
  экстракция (495 chars/4810ms) завершилась на пуле после ухода тика.
- **P2-ПОСЛЕ (живой uvicorn, D8P_ENABLED=1, 20× idle_tick, curl): 
  total=8.37с (mean 0.42с, min 0.33, max 0.87) — КРИТЕРИЙ `wait 20` ≤ 10с
  ЗАКРЫТ; baseline 110.96с (×13.2).** Диалоги живы (NPC_DIALOGUE_SUB=8),
  RE-D2=0, APPLIED=2 (listener maid_lusya/merchant_goran), STALE=0. Ни один
  тик не содержал LLM-ожидания (max 0.87с при экстракциях ~4.8с на пуле).
- **Production-наблюдения (честно):** (1) 8 событий → 2 задачи — event.id
  детерминирован (репаблицируемый контент режется Q5-дедубом; то же в
  wired-smoke v2); (2) parent_tick=0 — tick_provider свежего uvicorn-процесса
  читал сцену инициализации: возраст-гейт не напрягался (STALE=0 в живом
  холостом прогоне); STALE-семантика доказана сценарной C-группой; (3) tick-
  age — game-семантика по вердикту Q2б (не wall-clock) — флаг N=3 честно
  числится calibration-константой (молотилка idle-тиков обгоняет экстракцию —
  при живом пейсинге игрока окно живёт).
- **Parity-гейты: Люся GREEN — ТОЧНЫЙ parity охраняемых чисел (A flee=0.800,
  B call_for_help=0.272, C flee=0.777, A−C-класс 0.023, D REJECTED×3)**;
  Горан — parity по документированному исходу S243/S247 (request_service=
  0.707 seed-стабильный; flip исключён H1-ландшафтом по определению B0),
  CC2≡EE1 побайтово = детерминизм. bc1 6/6; phase_a 45/45; IPT 45/45 (CC2+EE3).
