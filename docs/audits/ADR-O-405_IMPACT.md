...
## Update
Status-Update (S293, Phase 4.1 UX Vertical): потребитель работает в двух
контурах — игра (REST, персистентность) и полигон редактора (EditorBoardStub,
in-memory). Board = внешний интерфейс мышления игрока: PLAYER HYPOTHESIS и
PLAYER ORGANIZATION получают физическую форму; TRUTH ≠ BELIEF ≠ PLAYER
HYPOTHESIS ≠ PLAYER ORGANIZATION. Доска FULL = рабочий режим (пауза мира,
dim). Табу дополнены: дубль указателя легален; reset кампании стирает доску;
токены цвета только из реестра (+danger); DM-проза не цитирует реплики NPC
(debt, RCE-сессия); досье NPC (имена/события/биографии) — отдельное
проектирование; авто-вывод из гипотез — только явной командой игрока.