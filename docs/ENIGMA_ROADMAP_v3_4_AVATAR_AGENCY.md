...
- [ ] **AVID-1 (P2, avatar-in-idle, NEW из AG1-D5 зонда):** аватар игрока отсутствует в all_npcs_raw idle-тиков (GC-00-зонд: player/Tester NOT IN SNAPSHOT после 3 тиков; в REST-пути присутствует — S198_PIPELINE_ENTER count=7 с 'player'). Следствие: idle-мир (CFL/социальное поле/восприятие NPC) не учитывает аватара. Действие: археология пути попадания аватара в кэш (select_player → LifeEngine) — работает ли он в idle; вердикт «gap»/«by-design». Владелец: итерация «Пункт 5». Закрытие: галочка + строка журнала.
