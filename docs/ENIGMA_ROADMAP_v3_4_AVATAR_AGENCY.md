...
**Версия документа:** 3.3 · 2026-09-04 (+ §4c: внешний аудит-реестр
AUD-D) **Адресат:** LLM-преемник на V.0.5.3.9.6 (ветка
...
**Версия кода:** V.0.5.3.9.6 · `version.txt` = `0.5.3.9.6` ·
...
### §4a. Реестр долгов сессии AG1 (единый; префикс AG1-D; не разрозненный --- это исполняемый реестр)
  ...
  AG1-D2    `Dialogue update failed:` ---  P1       AG1                локализовать except  \[ \]
            тихий глоток в подписчике                                  → лог-гейт           
            диалогов (L4-нарушение)                                                         
  ...
  AG1-D5    Аватар жив с hp=0              P2       body               связать с E5/E12     \[ \]
...
> уже известный долг, независимо верифицированный.
  ...
  AUD-D2    `social_subscriber.py:181–193` --- 5×      P1     events               None-инвариант в            \[ \]
...
требуемый уровень доказательства и нет известного blocker debt.
  ...
  ID      Область             Механизм                    Production path           L3 test Negative    Persistence / Player-visible   Исправлено   Закрыто   Уверены   Доказательство /
                                                                                                        Replay                                                          коммит
  ------- ------------------- --------------------------- ------------------------- ------- ----------- ------------- ---------------- ------------ --------- --------- ----------------
  GC-00   Runtime             живой world tick            TickOrchestrator → NPC    GC-00   NEG-06      SCALE-05      да               \[ \]        \[ \]     \[ \]     
...
- [ ] **AI-D1 (P2, тест-гигиена):** test_game_loop_pipeline.py скипается ВСЕГДА — юнит-гейт run_turn фактически мёртв, регресс не ловится. Причина skip не установлена (первые 40 строк файла — чистые моки, ссылки на guard_borko нет; гипотеза о guard_borko.json снята как непроверенная). Действие: прогон с -rs → локализовать skip-условие → восстановить гейт через production-путь `_run_pipeline`). Владелец: итерация «Пункт 5». Закрытие: галочка + строка журнала.
- [ ] **ST-1 (P1, WS-путь):** stream_turn (1560–1733) не содержит commit_tick_result/unlock_tick/execute_pending в археологическом окне — REST-путь WS-сессии может не персистить сцену и не материализовывать задачи (зеркало run_turn-разрыва до фикса). Требуется тело stream_turn целиком для вердикта. Действие: дочитать stream_turn → вердикт «тот же разрыв»/«персист жив в другом виде» → патч-зеркало или снятие флага. Владелец: итерация «Пункт 5». Закрытие: галочка + строка журнала.
