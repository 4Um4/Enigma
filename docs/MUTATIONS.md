...
### S254: PROTECT — срез власти над пространством (ADR-O-386) | ✅ IPT 45/45, SUPERBOX 5/5
🎯 Второй срез Living Activity: территория → владение → нарушение → право (L-A1: authority из мира, не из строки роли). Мирный «Хозяин таверны» получает право block_path из владения бар-кластером. Пункт 4.1 (замер живой сессией) снят решением Мастера — лаборатории/песочницы/построение; инструмент консервирован.
⚙️ Case-фикс SocialEngine (автономный, вне флага — прецедент R1; инвариант Мастера «фикс ≠ баланс», 6/6): lowercase-ключи эмиссии + ремап THREATEN→INTIMIDATE (Intent.THREATEN не существует — решение (а); max-слияние без двойного учёта) — оживление jealousy/protect/fear/debt-lever. NodeRef.owner (Optional) + компиляция graph_compiler + zone_owner/territory_owners (юниты 2/2). TRESPASSED в SpatialEventDetector (Фаза 2, за TERRITORY_ENABLED default OFF = no-op; проводка zone_owner-lookup в оркестраторе; юниты 5/5). territory_pass предвычислен в npc_tick_pipeline (read-only spatial_service из TickState — Hub остаётся чистым) → гейт decision_hub:962 (территория ИЛИ роль; юниты 4/4). Контент: owner ×3 бар-кластера — членство скорректировано по полной карте (bar_area — клиентская сторона NodeRole.BAR, bar_west — транзит; итог: bar_pass_north/bar_side/behind_bar → tavern_keeper_tornin; фиксировано в IMPACT). SUPERBOX protect_vertical_test 5/5 (P1 владельцы живого графа; P2 TRESPASSED payload; P3 шина; P4 право: мирная роль + территория → block_path, без права → блок; C1 OFF — нем). Методология-находка сессии: исходный ТЗ-аудит восстановлен из CDS-логов (756/920/причины 393/97/93/173 — точно; предикаты откалиброваны: пустота 27.1% vs ТЗ 25.9%, пинг-понг ×2.2) — diagnostics/cds_session_audit.py законсервирован (гейт-режим --run).
⚠️ Уроки: (1) IPT — по флагу риска, не по каденции (правка Мастера); (2) угадывание enum-значений в фикстурах повторяет урок №3 S252 (WillState 'alert'→'free'); (3) плейсхолдеры в исполняемых блоках запрещены — PowerShell резервирует '<' (класс S216). Долги: doc-drift ROADMAP constants.py:307 (чужой, до-существующий); реестр qwen_7b ведёт на мёртвый Q5-путь (косметика — сервер на живом Q4); 2 битые строки scene_changes.jsonl (конкурентная запись без лока); DLG_QUEUE OVERFLOW в логах (Н-56 виден). Открытые: TRESPASSED → давление владельца (восприятие→скоринг); эмерджентный выбор block_path; evict NPC→игрок v1.5; пункты 4.2–4.5 письма EAT.
📁 svc/social/social_engine (case-фикс), models/spatial_contracts (NodeRef.owner), svc/spatial/graph_compiler, svc/spatial/spatial_service (zone_owner, territory_owners), svc/spatial/spatial_event_detector (TRESPASSED, territory_enabled), svc/tick_orchestrator (Фаза 2), svc/npc/npc_tick_pipeline (territory_pass), svc/npc/decision_hub (гейт), svc/events/event_types (TRESPASSED), frontend/map_editor/campaigns/Open_road/locations/tavern.json (owner ×3), tests/test_territory_owner.py (11), tests/test_social_modifiers_case_fix.py (6), SUPERBOX/scenarios/protect_vertical_test.py (5/5), docs/audits/ADR-O-386_IMPACT.md, docs/ADR (атлас O-386), diagnostics/cds_session_audit.py + diagnostics/movement_audit_probe.py (инструменты сессии, законсервированы), docs/MUTATIONS.md (эта запись)
IPT: ✅ 45/45. КРАСНЫЕ ИНВАРИАНТЫ: 0 🔴 → 0 🔴
