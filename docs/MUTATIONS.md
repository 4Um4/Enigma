...
### S252: Living Activity — срез EAT (ADR-O-384) | ✅ IPT 45/45, SUPERBOX 12/12
🎯 Замыкание 4 разрывов NEED→…→WORLD CHANGE: онтология Desire/Activity/Outcome + первый вертикальный срез. Тот же тик, другая онтология; флаги default OFF = байт-идентичный no-op.
⚙️ dom/desire.py (L-M1 multi-cause) + dom/activity.py (факт+resume-токен); desire_generator (Ф0, guarded); activity_catalog (L-A2); activity_lifecycle_service (Ф0.7: сон-шаблон, G3-ревалидация W2, терминал damage-O6+release, label-проекция, reconcile D-3); _tick_needs двухрежимен (NO_LABEL_SATISFACTION) + подавление двойного трека; current_role-проводка в NPCStateAdapter (гейт ambush вооружён); food_portion (мини-запись O-376 + W2 + EventType.ACTIVITY_OUTCOME); r3-блок занятости → DM; контент 3 порции.
⚠️ Уроки: (1) вставка в общий JSON = аудит ВСЕХ читателей (size-краш редактора); (2) INV-SILENT-FAILURE поймал мои тихие except'ы трижды — каждый хендлер логирует; (3) фикстуры лгут типами (consciousness — float); (4) тест-тайминг: активность легально завершалась до reconcile — фикстур обязан держать процесс живым; (5) LifeEngine-синглтон: кэш переживает пересборку GameLoop — reset_life_engine() между control-мирами.
📁 dom/desire.py, dom/activity.py, svc/npc/desire_generator.py, svc/npc/activity_catalog.py, svc/npc/activity_lifecycle_service.py, svc/action/commitment_registry.py, svc/npc/life_engine.py, svc/phases/simulation.py, svc/scene/r3_direct_builder.py, dom/object_fsms.py, svc/world/affordance_resolver.py, svc/world/world_object_spawner.py, models/npc_state.py, services/events/event_types.py, tavern.json, tests/ 6 сьютов (39), SUPERBOX/eat_vertical_test.py, docs/audits/ADR-O-384_IMPACT.md
Открытые пункты: MemoryManager-подписка; метрики 2.1 живой сессией; PROTECT/WORK/SOCIAL; SpatialTargetType+OBJECT; PREFERENCE_STABLE.

### S253: Player Dialogue Track — Р-А/Р-Б/Р-В: санация речевой трубы (ADR-O-385) | ✅ IPT 45/45
🎯 Подготовка vertical slice Player↔NPC Dialogue: пять раундов археологии доказали существование полного речевого контура (player→PLAYER_SPOKE→perception→DecisionHub→DialogueRequest→NPC_SPOKE→подписчики; pair-STM двусторонний: dm_phase:169 + подписчик). Срез строится подключением, не новой трубой (§ENIGMA-002: dialogue-субстраты не введены).
⚙️ Р-А: SELF_TALK_SENTINEL (dom/communication:19) — солилоквий = экстернализованное бормотание; фантом лишён STM/rel/L1; подслушивание живо; лог-фантом мёртв. Р-Б1: 7-й write-маршрут NpcDialogueSubscriber → RelationshipWriteGate (wrap-own-gate, M1b.2.4-паритет; D2-греп .update()=0). Р-Б2: мембрана адресата can_observe (S192-паритет, fail-open S198). Р-В: SpeechExposure Contract — exposure_radius() SSOT (normal 6.0 = ACTION_PERCEPTION_RADIUS["dialogue"], parity-тест; loud 10.0; secret→whisper D3; солилоквий→whisper D4; working_memory_tick 999→6). Инциденты: транзиент-красный IPT = параллельная ADR-O-383-сессия (EventBus retry+DLQ архитектурно исключал наш механизм); «случайный checkout» → bytecode-recovery (bac0c6d6, dis-эквивалент, верифицирован постфактум тестами). Процесс-урок: 4 генерационных дефекта СТАЛО за трек (delta_far, exposure_semantancy, contract, внутрискобочный импорт) — правило мысленного py_compile по якорю БЫЛО до выдачи хунка.
📁 dom/communication (SENTINEL, ladder, exposure_radius), svc/events/npc_dialogue_subscriber, svc/game_loop/task_scheduler, svc/phases/post_decision, svc/execution/dialogue_materializer, svc/events/intent_event_adapter, svc/memory/working_memory_tick, tests/micro/test_self_talk_sentinel.py (NEW), tests/micro/test_speech_exposure.py (NEW)
⚠️ Долги: LISTEN A/B (worktree-метод); D5 (999-дефолт-аудит); Р-Г (журнал 8.0→event.radius); doc-drift relationship_store; Stage-2 Люси → M2/D. Коммиты: a92278cc (Р-А/Р-Б), bac0c6d6 (Р-В в составе save-коммита).

### S254: GC-DIALOGUE-01 закрыт — Player Dialogue Stage-1: Р-Г (журнал из event.radius) + TAB-пейсинг ×4 + D0 (SSOT-радиус материализатора) + liveness-гейт ×4 (ADR-O-387) + приёмочный сценарий GREEN | ✅ батарея 26/26, IPT 45/45, GORAN β+vertical GREEN, gc_dialogue GREEN
🎯 Мандат преемника трека Player Dialogue (S253/ADR-O-385, закрытие S251): довести до GC-DIALOGUE-01 — «один живой разговор в таверне, который невозможно подделать тестами». 14 пунктов канона подтверждены Мастером 2026-09-11; порядок A(harness)→B(liveness)→C(сценарий)→D(прогон)→E(закрытие) исполнен; Stage-2 (Люся) остаётся M2/D.
⚙️ Р-Г: порог подслушивания игрока — из event.radius (SpeechExposure SSOT) вместо хардкода 8.0; fail-open 8.0 (легаси-паритет Р-Б2: dict-события/999/NaN); замки +5 (sentinel-батарея 6→11). TAB ×4: DIALOGUE_FOCUS_SPEEDUP=4/MIN_INTERVAL_MS=125 (frontend/constants) + врезка в idle-каденс game_screen — NEAR 500→125/MID→375/FAR→750, ФРИЗ 30с УДАЛЁН (противоречил доктрине «фокус внимания на живом мире»); ядро/тики/RNG не тронуты (ADR-O-344 инвариантен by construction). D0: dialogue_materializer — мёртвый _speech_radius оживлён (radius=SSOT в NPC_SPOKE + COMMUNICATION_CLAIM; ADR-148 в точке материализации; замок 4): вся production-речь жила на «loud»-радиусе при зелёной батарее Р-В. B (пункт 10, вердикт Мастера «Б»): liveness-гейт диалоговых задач — терминал «owner DEAD → EXPIRED» канонического ADR-O-365-мэппинга в 4 точках диспетчеризации (fast-path №220/dequeue №233/process_tasks №176-мёртвый-путь-замок-на-будущее/worker №330-in-flight); fail-open S198-паритет (нет провайдера/не найден/ошибка → исполнение); провайдер _resolve_npcs_snapshot, wiring game_loop:2527; замок 7. C: SUPERBOX gc_dialogue_test — D1 (REST run_turn → PLAYER_SPOKE radius=SSOT → pair-STM с темой → ответ адресата→player ПОСЛЕ речи [анти-подделка: срез _mark_d1 + target_id] → мир живёт ticks/moved/traversals) / D2 (авторинг смерти → DI-шов провайдера [β-freeze-прецедент] → гейт → маркер+EXPIRED + 0 посмертных в чистом окне) / D3 (pair-STM переживает смерть, темы в обеих парах) — VERDICT GREEN.
⚙️ A (harness repair, вердикт Мастера: stale-test migration, НЕ production causal defect): goran_vertical_slice_test — чтение windup мигрировано на Э6-контракт (scene_state["windup_registry"], dict-форма) + save_scene_state после инъекции (оба плеча; S225-находка «get_scene_state при _tick_locked=False = свежая копия») + β-freeze мембран (helper _freeze_spatial_consumers_v, 6 потребителей): 1/8 → GREEN (13 звеньев). β-геометрия игрока (8.59, 0.81) — луч «вор→Горан→игрок»: 5.5 м (слышит warn, normal 6.0) / 10.6 м (не свидетель THEFT, _OBSERVATION_SIGHT_RADIUS 10.0); прежняя (4,2) работала ТОЛЬКО на баге-радиусе 10.0 материализатора — D0 вскрыл телепатию-класс в самом срезе.
📁 svc/events/npc_dialogue_subscriber (Р-Г), fe/constants + fe/game_screen (TAB ×4), svc/execution/dialogue_materializer (D0), svc/game_loop/task_scheduler + __init__ (B), tests/micro/test_self_talk_sentinel (+5→11), test_materializer_radius (NEW 4), test_dialogue_liveness_gate (NEW 7), SUPERBOX/scenarios/gc_dialogue_test (NEW), goran_vertical_slice_test (миграция), smoke_goran_beta (геометрия), reports/goran_*.txt (форензика-артефакты).
⚠️ Инциденты-уроки (счёт честный, все мои): (1) три неприменённых/сломанных хунка за сессию (3b vertical; C-fix7 дважды — один дал SyntaxError «№163:» вне комментария) — закон: якорь БЫЛО только со свежего чтения диска + маркер-принт применения в каждом хунке (паттерн [G2-FREEZE] окупился дважды); (2) ретракция №149: теория «третий dispatcher process_tasks» — греп вызывателей ДО выдвижения dispatch-теорий (путь мёртв в production; гейт оставлен замком будущего реактирования); (3) слепые фикстуры №156: пустой tasks-аргумент воркера / game_time=0.0 держит всех на cooldown (queue-голод) / warn-фикстура покрывала только fast-path (дыра замка — закрыта talk-тестом); (4) эфемерность engine-кэша №157: тик перезагружает NPC-стейты из персистенса — DI-шов гейта (freeze провайдера), не мутация; (5) in-flight straggler пула = DEBT-ABORT-404 (S220): гейт-проверка прошла ДО смерти честно — сценарий меряет чистое окно ПОСЛЕ гейта; (6) ложноположительный D1 №145 (ambient-пре-реплика засчитана) — анти-подделка срезом; (7) IPT гасит llama-server — LLM-прогоны до IPT.
⚠️ Координация: нумерация трека дрейфовала (handoff S251/ADR-O-384 → фактически S253/ADR-O-385 заняты параллельными сериями; IMPACT-384 = «Потребности»); закрытие = S255/O-387 по факту хвоста (сверено перед записью).
⚠️ Долги: DEBT-DLG-LIVENESS-prodpath (production-смерть VitalState→персистенс→reload не покрыта живым прогоном — гейт доказан через DI-шов, контракт идентичен); DialogueMemorySubscriber L2 без мембраны (speaker+listener без радиуса — телепатия-класс); мёртвый _publish_verbal_action (radius 999); SHADOW_COMPILER node-дрейф (tavern:node_16/main_hall/fireplace); SC-3 player foreign-node; docstring-дрейф sentinel-файла; W293/ruff-легаси (py3.11-таргет против рантайма 3.13 в vertical). Живая-сессия пп.1–2 (TAB ≈125мс в фокусе) — чеклист в сценарии, прогон за Мастером.
IPT: ✅ 45/45. КРАСНЫЕ ИНВАРИАНТЫ: 0 🔴 → 0 🔴

### S255: GC-SOCIAL-01 Stage-1.5 — Social Conversation: G1 PlayerSpeechClaim (testimony player→NPC) + приёмочный сценарий | ✅ батарея 30/30, IPT 45/45, S1 4/4 + S3 7/7 интеграционно; S2.6-8 — механизм доказан замком, end-to-end = живая сессия
🎯 Мандат Мастера (2026-09-12): «разговор = локальное социальное событие мира», не UI-окно; разговор должен производить событие мира, воспринимаемое каждым агентом самостоятельно. Граница: БЕЗ self-relevance/secret-семантики (M2/D); «не DialogueEngine управляет социальным миром — он производит событие мира».
⚙️ G1 (археология №168-171/176-179): путь «уста игрока → убеждение NPC» отсутствовал (единственный подписчик PLAYER_SPOKE в бэкенде — L2-память). Фикс — PlayerSpeechClaim: ClaimEventSubscriber.on_player_spoke (зеркало on_npc_spoke-фоллбека №177): PLAYER_SPOKE + DM-вектор (ADR-035 semantic_action/target_reference) → Proposition (ACCUSE→STOLE subject=target / THREATEN→ATTACKED subject=player+S202-origin / HELP→HELPED) → on_claim_event (мембрана distance+can_observe по event.radius=SSOT унаследована) → ClaimEvent → belief_revision слушателей. Без вектора — no-op (граница Stage-1.5: не-понимание). Подписка в _register_epistemic_core (game_loop:485-область). Никаких новых сущностей: существующие Predicate/SpeechAct/ClaimEvent.
⚙️ Замок test_player_speech_claim.py (4): в радиусе→belief обоих слушателей с предикатом; за радиусом→0 (C1: target в мире, но 9 м>6.0 — мембрана сильной формы, S198-фоллбек честен для присутствующих); без вектора→no-op (C2); THREATEN→S202-origin+предикат. Уроки фикстур (№181-187, все мои): локальный import dataclasses (F821 — 4-й внутрискобочный шрам); контракт store №183 (get/revise/upsert — шпион под производственный); S198-target-фоллбек добавляет адресата слушателем ВНЕ мембраны → тесты изоляции различают target_id (адресат реплики) и target_reference (subject клейма); S202-origin для attacked — дистанции от жертвы (симметричный словарь); game_time=0.0 держит всех на cooldown (queue-голод фикстуры).
⚙️ Сценарий gc_social_test.py (контракт 17 пунктов, 3 группы + C1/C2/C3): S1 4/4 (адресация FT-1, PLAYER_SPOKE, ответ→player анти-подделкой, сцена жива) · S2 1/5 интеграционно (S2.5 публикация жива; 6/7/8 = LLM-зависимые: мёртвый DM даёт UNCERTAIN — честное C2-поведение; механизм доказан замком 30/30; end-to-end → живая сессия, прецедент пп.1-2 GC-01) · S3 7/7 (атака через production Фазу 6 → windup → β-fallback смерть; liveness-маркер ADR-O-387; мир/STM/последствия переживают). Регресс: батарея 30/30, IPT 45/45, GORAN β+vertical GREEN (после №189-прогона).
⚠️ Урок-призраки (№191/194/195): три подряд формы по памяти против диска (_shared_context отсутствует — контекст эфемерен кадру run_turn; IntentParametersDTO не в schemas; модуль intent_dto не существует) — СТОП итерациям после второго призрака, следующий ход = grep. Пререгистрация №191 исполнена честно: S2 = LLM-зависимый пункт контракта.
⚠️ Долги: DM-классификация ACCUSE недетерминирована при живом LLM (возможен UNCERTAIN на обвинительную формулировку — кандидат на S122-fast-path ACCUSE-лексики, решение Мастера); read_trust возвращает None в harness-мире (пара player не создаётся player_interacts-путём — кандидат на аудит V2-бутстрапа пар); S3-атака не доводит hp до 0 за окно (окно/урон — калибровка, β-fallback законен); UNCERTAIN-вектор в payload при мёртвом DM (шум для G1 — no-op, но логгер шумит).
📁 svc/events/claim_event_subscriber (on_player_spoke), svc/game_loop/__init__ (подписка), tests/micro/test_player_speech_claim.py (NEW 4), SUPERBOX/scenarios/gc_social_test.py (NEW).

### S256: WORK Vertical Slice (ADR-O-391) — разрыв «Intent.TRADE → WORLD CHANGE» замкнут минимальной сделкой на живом субстрате | ✅ юнит 17/17 · SUPERBOX work_vertical_test W1–W5 GREEN · IPT 45/45
🎯 Мандат Мастера: третий вертикал — не TransactionEngine и не экономика, а замыкание разрыва «TRADE — выбор есть, исполнителя нет» (раскоп: «выбор без исполнителя» — THE FACT). Захороненный контур AUDIT #6 физически отсутствует (transaction_engine/trade_resolver NOT FOUND во всём backend; надгробие economy/__init__.py) — воскрешение ЗАПРЕЩЕНО (L-W7); срез построен на живом субстрате: Transaction (PROPOSED→ACCEPTED→COMPLETED/FAILED), EconomicProfile.gold/goods/stock_for_sale, GOODS_PRICES["ale"]=0.01, EconomyTracker.record_income (контракт «при каждой транзакции» — до среза ноль вызовов), ACTIVITY_OUTCOME. Дерево-вердикт (git): PROTECT/S254/O-386 в этом дереве не существуют (веток/коммитов/grep-следов нет) — срез самодостаточен; судьба PROTECT — решение Мастера.
⚙️ Петля (production, LLM не нужен): thirst=0.9 (единственная инъекция входа) → desire ale (desire_generator: _NEED_TO_DESIRE += thirst) → WORK-гейтованный канал «давление желания → TRADE» (npc_tick_pipeline: urgency×WORK_TRADE_PRESSURE_K=1.0 → _all_modifiers["trade"] существующим eco-рельсом; pressure deforms, not commands) → TRADE выигрывает argmax (1.19–1.38 против offer_job 0.738) → Фаза 6: ORDER = Transaction(PROPOSED) с маршрутизацией к продавцу предмета (социальный адресат ≠ экономический контрагент — резолвер послал Горана к orm без стока) + дедуп «один открытый заказ на покупателя» → Фаза 0 work-pass: SERVE-материализация (ActivityState; target_ref=order_id; D6: эль — goods-онтология) → терминал SERVE (BODY_ACTION без предмета; CONSUME-путь не тронут) → settle_order: валидация ДО мутаций (L-W3) → атомарный SSOT-обмен (service_factories._economic_profiles; INV-WORK-PERSIST; S-143 deepcopy-ловушка обойдена) → COMPLETED + ACTIVITY_OUTCOME обоим + record_income(Торнин) + терминальное гашение давления покупателя («насыщение пишет только терминал», EAT-прецедент).
⚙️ Диагностическая хронология (6 прогонов: 5 красных → GREEN, каждое звено закрыто фактом, ни одного фикса вслепую): (1) orders=[] → TRADE никогда не выигрывал (P6-зонд экологии) → OPT-A Мастера; (2) TRADE побеждает (SCORES), но адресат=orm без стока → маршрутизация ордера; (3) order_id=None при идеальном входе → DI-разрыв: геттер профилей в ServiceFactory (game_loop._svc), не в ctx.npc_services → проводка _economic_profiles_getter (паттерн P1.1f) + fallback; (4) proposed→failed за тик: serve_failed:not_holding — _advance_body_action требует holder → SERVE-ветка без предмета; (5) GREEN при случайном дубле merge-блока (удвоение давления) → чистка → регрессия (K=0.5 < K_min) → Шаг-3 Мастера: K=1.0 по якорю шкалы (прецедент DecisionHub desire×1.5; канал ниже прецедента) → финальный GREEN. W1-INC закрыт той же DI-проводкой _economy_tracker.
⚠️ Уроки (мои, пойманы протоколом): logger.info слеп в _quiet-мире — только print-зонды (S216-урок применён с опозданием); патч прозой = неприменение (§4); escape-кавычки f\" в СТАЛО = SyntaxError; grep-паттерн без закрывающей скобки лжёт «не применено»; двойное применение патча = скрытая калибровка — чистка обязана проверять линейность; юнит-SimpleNamespace эмулирует проводку — DI-разрыв виден только в production.
⚠️ Долги: (1) EAT E6–E9 — вердикт reverse-эксперимента: ПРЕДСУЩЕСТВУЮЩИЙ красный (WORK OFF = байт-идентичный прогон; Торнин не приходит в bar_area, sleeping/расписание — чужая зона, решение Мастера); (2) калибровка: канал накрывает любой рыночный subject (food=1.0 у Люсьи/Торнина давит 1.0 — ордеров нет: food никто не продаёт) и _try_onset подбирает serve-желания любому NPC — CALIBRATION_CANDIDATE; (3) CONSEQUENCE_REACHES_MEMORY — проводка исходов в память = LIFE INTEGRATION (R4); (4) npc_sandbox.py:197 мёртвые импорты TransactionEngine — задокументированный landmine, не тронут; (5) расхождение деревьев с документом-преемником (линия S254/O-386 vs исполняемое S255/O-388) — преемнику СЛЕДУЮЩЕЙ сессии входить по MUTATIONS/атласу этого дерева.
📁 NEW: svc/economy/work_orders.py, tests/test_work_orders.py (17), SUPERBOX/scenarios/work_vertical_test.py (W1–W5). Точечные: dom/activity.py (ActivityType.SERVE), svc/npc/activity_catalog.py (SERVE_SPEC), svc/npc/activity_lifecycle_service.py (SERVE-ветка + settlement-хук + метка serving), svc/npc/npc_tick_pipeline.py (канал давления), svc/npc/desire_generator.py (thirst→ale), svc/phases/post_decision.py (INTENT→ORDER), svc/phases/simulation.py (work-pass), svc/game_loop/__init__.py (DI-проводки профилей+трекера), docs/audits/ADR-O-391_IMPACT.md, docs/ADR (атлас O-391), docs/MUTATIONS.md (эта запись). Координация (§11.1.1): O-389 — двойная заявка (атлас:517/IMPACT-путь заняты параллельной GC-INTERRUPT S257); WORK перенумерован в O-391: 20 код-сайтов обновлены, WORK-IMPACT восстановлен как ADR-O-391_IMPACT.md (первичный был затёрт). НЕ ТРОНУТЫ: DecisionHub, WorldActionType, Commitment-онтология, якорь tick_orchestrator:904, npc_sandbox, захороненный контур.
IPT: ✅ 45/45 (батарея GREEN). КРАСНЫЕ ИНВАРИАНТЫ: 0 🔴 → 0 🔴
IPT-финал (батарея перенумерации O-391): 44/45 — 1 CRITICAL INV-TIME-FREEZER (backend/app/services/replay/time_freezer.py: name Any) — чужая in-flight зона (параллельные S257/S258), вне множества файлов S256; эскалация Мастеру.

### S257: GC-INTERRUPT-01 — третий агент: intent-субстрат (E1/E1b, закрытие DEBT-INTENT-SOURCE) + flee-гейт задач ×4 (E2, ADR-O-389) | ✅ микро 10+12, батарея 52/52, IPT 45/45, golden 180/29 (вердикт S213 перевёрнут), SUPERBOX GREEN (боевой полу-этаж hp 100→82.0; волю закрыл β-мост; после вердикта мир сам ударил dequeue-гейт с реальным провайдером)
🎯 Мандат AGENCY-лестницы: слой «вмешательство третьего агента» — доказать, что диалог двух агентов не выключает мир и не превращает NPC в декорации. Выбор Мастера: NPC-interrupt (flee-first; attack-семантика — вне v1-scope). Фальсификационный режим приказа: «не доказывай — попытайся сломать» (дюжина попыток, карта в IMPACT/closeout).
⚙️ Археология (зонды A–BN, ~30 чтений): дыра оказалась трёхслойной S1-миграцией — StateApplicator.apply() осиротел в живом тике (дельты перевезены, намерение забыто), to_persistence_dict не проецировал intent-блок (DEBT-INTENT-SOURCE: S213 закрыл его для метрик через шину, state-потребители остались ни с чем), TickMutation.npc_deltas не имели потребителя-применителя (significant_events/L1 only). E1: проекция полного 6-полевого блока (current_role/WARA-прецедент) + round-trip обоих гидраторов (from_legacy гасил перезатиратель decision.py). E1b: StateDeltas.intent/intent_tick + Committer-провод (вариант (х) — intent-only, байтово-нейтрально; полный (y) S1-COMPLETER — отдельное решение Мастера) + инерция в _apply_deltas + агрегатор-перенос; фикс-цикл №191-класса ×4 (NameError импорта / TypeError ctor / маркер-дрейф / TALK-без-цели — сторож домена поймал фикстуру). E2: INTERRUPT_TASK_STALE_INTENT (закон №16, симметрия WINDUP_STALE_INTENT) + _owner_intent_flees (зеркало _owner_is_dead: тот же провайдер, fail-open S198-паритет, чтение) на 4 точках после death-check; outbox 4-кортеж (interrupt_reason ≠ fail_reason, D-6) + backcompat legacy-3. Сценарий v1.3: warn-fast-path окна (мимо DialogueQueue-флода и мёртвой LLM), провайдеры-копии (кэш-мутация R3 убила A в v1.2), save-восстановление (S225: get_scene_state = копия). МАННЕКЕН-предупреждение S213 не сбылось: срез прошёл в harness-мире при мёртвой llama.
📁 models/npc_state (проекция+from_legacy), models/state_delta (intent/intent_tick), services/npc/npc_loader (round-trip), services/npc/npc_tick_pipeline (дельта решения; локальный импорт — прецедент attack-ветки), services/npc/state_applicator (intent-ветка), services/tick_utils (агрегатор-перенос), services/pipeline_runner (Committer-провод), domain/action_commitment (константа), services/game_loop/task_scheduler (хелпер+4 гейта+outbox), tests/micro/test_intent_roundtrip.py (NEW 10), tests/micro/test_intent_liveness_gate.py (NEW 12), SUPERBOX/scenarios/gc_interrupt_test.py (NEW v1.3), tests/calibration_lab/e1_intent_liveness_check.py (NEW замер), docs/audits/ADR-O-389_IMPACT.md.
⚠️ Координация: S256 занята сессией секретов (in-flight, E2-комментарии в npc_dialogue_subscriber); атлас max O-388 → O-389; O-386 — зарегистрированная дыра (строки в атласе нет; PROTECT живёт в передаче — зарегистрирована здесь, атлас не заполнялся). Поведенческая дельта-подпись инерции в golden: diversity 0.44→0.83, loop→1.0, events 108→77 (одно-прогонное; строгий A/B — lab-территория).
⚠️ Долги: (y) S1-COMPLETER (все decision-дельты в буфер — решение Мастера, риск двойного применения до аудита); DEBT-INTENT-TARGETLESS (вербальный intent без цели → ValueError на следующей гидратации — пре-существующее ребро, теперь живое); ATTACK-СЕМАНТИКА (таблица birth↔current: talk→attack конфликт? warn→attack эскалация — кандидат следующего среза); S3-окно атаки (калибровка: 3 атаки = −18hp < порога flee 30%); listener-death (смерть цели для задач); DialogueQueue-инверсия (canonical −1 vs ambient −5 в min-heap → ambient первым, вопреки комментариям; лестница 0–15 не используется); smoke steal_rate (слепая фикстура — вскрытие при живой llama); worker-flee живой выстрел не наблюдался (компиляция+симметрия; микро-лок process_tasks/dequeue есть, worker — нет); qwen-конфиг Q4/Q5-несостыковка; канон A0–A21 на диске отсутствует (снят с блокера приказом Мастера; архив docs/ТЗ_AGENCY*.md — при появлении оригинала/реавторстве); живая-сессия чеклист (TAB≈125мс; S2.6-8 SOC-01 end-to-end; GORAN β/vertical — при поднятой llama).
⚠️ Шрамы №1–11 (в real-estate преемнику): (1) новое имя во вставке → греп import-поверхности целевого файла ДО патча; (2) новый объект → чтение __init__; (3) runtime-вставка → живой smoke до длинного прогона; (4) маркер секции = контракт; (5) локальный импорт по прецеденту файла; (6) Trim-выхлоп Select-String не якорь — индентация из Get-Content; модульные константы flush-left; (7) тернарник длиннее литерала — в переменную до f-string (4-й внутрискобочный); (8) внешняя сущность сценария (фикстура/хелпер/шов) читается исходником до использования; (9) сценарные окна независимы от тикового дренажа; (10) LLM-класс фикстуры = очередь/флод/LLM-зависимость — окна гейта идут fast-path (warn); (11) терминальная история ≠ диск: [Vx.y-FLAG]-маркер в файле + детектор версии перед прогоном (спас дважды).
IPT: ✅ 45/45 (0 CRITICAL). КРАСНЫЕ ИНВАРИАНТЫ: 0 🔴 → 0 🔴

### S258: GC-RELEVANCE-01 — self-relevance: клейм «обо мне» как отдельный эпистемический канал (R1 развязка + R2 разговорный буст, ADR-O-390) | ✅ замок 9/9, батарея 61/61, IPT 45/45 (0 CRITICAL)
🎯 Мандат AGENCY-лестницы (автономный выбор по §ENIGMA-001): следующая ступень после вмешательства третьего агента — self-relevance. Цепочка M2/D: speech → perception → interpretation → claim → SELF-RELEVANCE → belief → action; testimony-слой O-388 доставляет клеймы — отсутствовало звено «касается ли это меня». Attack-семантика и S3-калибровка — долги, не ступени: не создают новых причинных структур.
⚙️ Археология (4 блока зондов, фальсификационный режим): H1 «шва нет» подтверждена grep'ом (ноль сопоставлений слушателя с субъектом клейма в services) и чтением трёх тел (on_claim_event / revise / resolve). ЖИВОЙ ПРОБ ДО фикса (§3.9): Люсья слышит «Люсья украла» + «Тень украла» → perceived_threats = ('maid_lusya', 'thief_shadow'), violations = 2, confidence идентичен — агент является собственной угрозой, обвинение себе = нарушение, trigger-канал загрязнён (Epistemic Targeting мог навести warn на себя, guard DecisionHub:1929 прикрывал). Словарная развилка закрыта пробом: Intent(str, Enum) — .value строчные; scores рождаются lowercase (decision_hub:990); apply_modifiers:401 молча бросает чужой ключ → lowercase-эпистемика жива, UPPERCASE-литералы S28 мертвы (находка).
⚙️ R1: EpistemicContext + claims_about_self / max_self_confidence (конец dataclass, runtime-only — конструируется единственный раз в resolver:72; сериализация не затронута); resolve(): subject_id == agent_id → self-канал, continue — полная развязка от threats/violations/max_confidence/trigger. R2: to_modifiers — talk = max_self_confidence × 0.5 при непустом self-канале (вдвое ниже threat-буста S198: салиентность, не паника; реакционные ключи запрещены до RESIST/MODIFY). Легаси-ветка S198/S211 байт-в-байт нетронута — доказано пробом-контролем: third-party-клейм → {'warn': 1.2, 'attack': 1.2, 'block_path': 0.6}, self-клейм → {'talk': 0.4}, merged talk 1.4 (ключ выживает в apply_modifiers).
⚙️ Границы v1: «обо мне» = только subject_id == agent; object_id == me (жертва) не тронут — там корректная семантика угрозы; SECRET ≠ знание (срез не входит в secret-инфраструктуру); мембрана неприкосновенна — канал живёт только внутри услышанных клеймов; BeliefRevisionEngine не тронут (salience ≠ confidence, №51).
📁 domain/epistemology (+2 поля), services/npc/epistemic_context_resolver (маршрутизация + модификатор), tests/micro/test_self_relevance_gate.py (NEW 9).
⚠️ Долги: DEBT-S28-KEYCASE (НАХОДКА, чужая зона): S28 Utility Deformation мертва — UPPERCASE-литералы decision_hub:577–599 против lowercase-словаря scores; агрессия не подавляется, escape не усиливается; фикс = отдельная сессия с замками; end-to-end SUPERBOX-сценарий self-relevance при живой llama (кандидат живой-сессии чеклиста, прецедент S255); RESIST/MODIFY — следующая ступень (противостояние клейму, конфликт с собственным опытом).
⚠️ Шрам №12 (новый, оплачен): числовые константы в ожиданиях тестов — только с диска. Инцидент: вписал _CLAIM_WEIGHT×0.8 по памяти → 1 failed в собственном замке; на диске _CLAIM_WEIGHT = 1.0 (belief_revision_engine:15); фикс локальный (одна строка теста), продакшен не тронут.
⚠️ Координация: S256 по-прежнему in-flight (секреты); моя = S258, следующая S259; атлас max O-389 → O-390; EQUIVALENCE-секция атласа стоит выше O-записей (518 против 548+) — диск против реле, вставка по якорю после Files-строки O-389.
IPT: ✅ 45/45 (0 CRITICAL). КРАСНЫЕ ИНВАРИАНТЫ: 0 🔴 → 0 🔴

### S259: M4/P7 — Disclosure Verbalization + Eavesdrop Provenance Label (ADR-O-392) | ✅ RED 6F/3P точный прогноз · GREEN 9/9 · регрессия gameplay 70/71 (FAILED = gc09b-R2 pre-existing git-доказано) · T5 монополия GREEN · IPT 45/0 к закрытию
🎯 P7-A (VERDICT PRECEDES WORDS): вердикт decide_disclosure — до LLM, ровно один раз на реплику, директивой [ДИРЕКТИВА РАСКРЫТИЯ] в промпте + эмитом из вычисленного (DOUBLE TRUTH вердикта запрещён). P7-B (EAVESDROP LABEL IS PROVENANCE): метка подслушанного — KnowledgeItem.secret_id спикера, не текст; FULL ⇔ {secret,whisper}; без проводки = E2-наследие. Wiring subject_resolver (game_loop:446). P7-C (exposure-политика DecisionHub) ИСКЛЮЧЁН санкцией — отдельная сессия.
⚙️ Археология 7 ходов (А1-А8): карта «построено/не построено» — P5-движок/ret rieval/proposition-канал/mapper-FULL жили; вердикт→промпт и метка — отсутствовали; Найден хардкод from_semantic("normal") на DecisionHub (P7-C-территория) и E1-wiring-gap (DEBT). H-фаза полная: флип INV-DIALOGUE-STM → бисекция W5 (без 3 файлов 🟢) / W6 (без wiring 🟢) / W7 (аргумент-выкл 🟢, импорт невиновен) → print-зонды W9-W11 → МЕХАНИЗМ ЗОНДОМ: MEMBRANE-CUT d=7.3 — позиционная хрупкость инварианта (async-недетерминизм позиций NPC к тику 39), НЕ регресс дельты (4 зелёных прогона с wiring). FLAKE-CANDIDATE в очередь. TIME-FREEZER (CRITICAL pre-existing) закрыт параллельной сессией в окне S259 (фикс-строка в чужом дереве, не присвоен). Уроки: D-P7-ISO-PARTIAL (контрактную пару wiring+consumer изолируют вместе — V3 невалиден) · D-P7-RUFF-AUTOFIX (ruff fix=true мутирует файлы: гейт touched-lines снимается ПОСЛЕ прогона) · D-P7-ANCHOR (якорь БЫЛО — из свежего чтения, не памяти) · D-P7-SAMPLE-SIZE («детерминированно» = 3+ различающихся прогона) · D-P7-IPT-BASELINE (baseline-IPT обязателен в начале сессии, LAST_SESSION.md его не заменяет).
📁 services/execution/dialogue_executor (вердикт-резолвер, директива, эмит-проекция), services/events/npc_dialogue_subscriber (subject_resolver, _resolve_eavesdrop_label, метка, Б-6 комментарий), services/game_loop/__init__ (:446 wiring), tests/gameplay/test_p7_disclosure_verbalization (NEW 9), docs/audits/ADR-O-392_IMPACT
⚠️ Параллельная сессия активна в окне (social_subscriber γ-фикс, social_input_projector, activity/work-зона, TIME-FREEZER-фикс) — разделяемое дерево, зоны не пересекались; номера S259/ADR-O-392 сверены с хвостом перед записью.

### S260: IRON RIVER D-1 Phase A — детерминизм причинного контура: все четыре P0-брейкера закрыты (F1/F2/F3/F4) + D-2 мини-гейт построен | ✅ батарея валидна (SOCIAL-темп, EAT, IPT 45/45, WORK GREEN) · A/B-гейт красно-зелёный · residual измерен диффом
🎯 Мандат Мастера: «полностью починить детерминизм для начала» — P0-порядок IRON RIVER (P0-1→P0-4), вход после паузы S259 (SOCIAL заблокирован нестабильной средой). Северная звезда: контракт D «same inputs → same trajectory, кросс-процессно» — предпосылка любых будущих вердиктов (SOCIAL, оптимизация).
⚙️ F1 (P0-1, wall-clock в состоянии): task_scheduler — timestamp → game_time (каузальная ось, в точке записи уже была по BUG-DLG-006) + real_ts (UI-staleness, §15.2-исключение, из канон-хеша исключён) + TTL-фильтр get_recent_dialogues на единую ось (_RECENT_TTL_GAME_SEC=120.0 = 2 тика; прежнее смешение осей — wall-clock запись vs game-time аргумент у трёх caller'ов — выметало реплики из UI-кэша всегда). Урок: комментарий «_UI_TTL_SEC moved to module level (021)» лгал — константа была (:31), патч по комментарию = призрак-якорь.
⚙️ F3 (P0-3, боевой RNG): event-id uuid4 → детерминированный evt:{tick}:{actor}:{target}:attack (tick_orchestrator S122-ветка); боевой seed hash((uuid4,actor,target)) — дважды недетерминирован (uuid4 + PYTHONHASHSEED-зависимый hash()) → KernelRNG(tick, actor, salt="combat")-деривация (combat_subscriber). Доказательство: f3_combat_smoke ×2 — hp/pain/shock байт-равны (первая атака игрока воспроизводима впервые).
⚙️ F4 (P0-4, изоляция лаборатории): life_engine.sessions_dir = data_dir/"sessions" (хардкод :245, НЕ settings.saves_dir) → world_tick.json наследовался между прогонами (+150 тиков, IRON RIVER Δ150 подтверждён). Фикс: полное temp-окружение харнесса — копия data/ в temp + build_game_loop(data_dir=копия), production не тронут. Бонус-находка в реестр: new_game-сброс удаляет world_tick.json из _saves_dir (game_loop:810), а TemporalEngine пишет в data/sessions — сброс мажет мимо (чужая зона, кандидат на фикс-приказ).
⚙️ F2 (P0-2, admission речи): SpeechScheduler admit/reset/cleanup гейтил допуск реплик по wall-clock (get_clock()=LiveClock) → состав NPC_SPOKE = функция реального времени. Фикс: ось = game_time_seconds из task_dict (в очереди с BUG-DLG-006), константы в тиках (pacing 120 game-сек = 2 тика, DEDUP-TTL 240 = 4 тика; отношение 1:2 сохранено; КАЛИБРОВКА Мастера: темп реплик замедляется с «2/сек» до «1/2 тика» — SOCIAL-батарея: 4→7 реплик в окне, состав валиден, resolver-пары, ложных нет). clock.py-синглтон не тронут (потребители perf/логов легальны).
⚙️ Инструменты (NEW): tests/sandbox/iron_river_ab.py (A/B канон-хеш: 2 подпроцесса, 150 тиков, canon с real_ts-исключением; atexit-очистка temp), tests/sandbox/iron_river_diff.py (полевой диф канонов — источник недетерминизма по путям, не хеш-слепота), tests/sandbox/f3_combat_smoke.py (боевая воспроизводимость), tests/sandbox/ttl_probe.py, f4_paths_probe.py. Базлайн-канон: 0c15ff5b... (idle 150 тиков).
⚙️ RESIDUAL (измерен диффом, честно): пограничная расходимость maid_lusya на границе 148/149 тика — eat-онсет (d:food у порога) сдвигается на тик → позиции/stress/psyche расходятся; диффы 2-10 полей (против 50 полей IRON RIVER-эпохи — сжатие на порядок). Класс: НЕ P0 — кандидаты async-интерливинг задач (P2-5 IRON RIVER: ThreadPool публикует вне тиковой транзакции) + мигающие эфемерные поля снапшота (psyche_state/stress_delta с 1-тиковым lifetime). Первая цель следующей фазы.
⚠️ Инциденты/уроки сессии (мои, пойманы протоколом): (1) ДИСК ПЕРЕПОЛНЕН (WinError 112): 12 temp-копий data/ ×1.35 ГБ (replay.db копировался каждым прогоном) — все A/B-вердикты на умирающем диске недостоверны; фикс: ignore(replay.db, logs) + atexit; остаточная утечка (10 каталогов при 6 RUN'ах) — счётчик-контроль обязателен каждой серии; (2) jsonl-обрезка копии = кросс-прогонный ХАОС (4 уникальных хеша): память NPC — каузальный вход, полные входы обязательны; (3) якорь-по-памяти ×3 за сессию (S198-блок, _UI_TTL_SEC, ignore-строка) — правило усилено: полный вербатим перед любым патчем уже-правленного файла; (4) logger.info слеп в _quiet-мире (повтор S216); (5) enum-гадание EventType.COMBAT_HIT (урок №3 в третий раз) — smoke переведён на чтение состояния; (6) «первый-прогонный сдвиг» (9360→0c15-паттерн) — не изолирован, в реестр.
📁 Точечные: game_loop/task_scheduler (F1a/F1b/константа/F2-проброс), speech_scheduler (F2: ось+константы+cleanup-сигнатура), tick_orchestrator (F3-1: event-id), combat/combat_subscriber (F3-2: KernelRNG-seed). NEW: 5 харнессов/зондов (выше). НЕ ТРОНУТЫ: clock.py, сцена/решения/движение, S259-файлы (redesign v1 в social_subscriber применён, верификация — в паузе).
IPT: ✅ 45/45 (0 CRITICAL; удержано на всех F-шагах). КРАСНЫЕ ИНВАРИАНТЫ: 0 🔴 → 0 🔴

### S261: IRON RIVER Phase A+B — контракт D закрыт: детерминизм причинного контура безусловный, кросс-процессный | ✅ IPT 45/45 · EAT GREEN · WORK GREEN · A/B 3×MATCH · Люсья насыщается
🎯 Мандат Мастера: «полностью починить детерминизм для начала» + «добиться зеленого везде и всюду». IRON RIVER-спек (P0-порядок) + методология красно-зелёного гейта. Контракт D «same inputs → same trajectory, кросс-процессно» — выполнен: 3×MATCH на каноне b8eb93ca... (idle 150 тиков, полные входы).
⚙️ P0-брейкеры (4/4 закрыты): F1 (P0-1) — timestamp→game_time+real_ts UI-оси+TTL единая ось (task_scheduler; урок: комментарий «_UI_TTL_SEC moved» лгал); F2 (P0-2) — SpeechScheduler→game-time pacing (pacing=2 тика, DEDUP=4 тика, калибровка Мастера, SOCIAL-темп 4→7 реплик); F3 (P0-3) — боевой event-id uuid4→детерминированный + seed hash(uuid4)→KernelRNG(tick,actor,"combat") (f3_combat_smoke ×2 байт-равны — первая атака воспроизводима впервые); F4 (P0-4) — settings.data_dir→temp-копия (ФИНАЛЬНЫЙ фикс: LifeEngine.sessions_dir строится от settings.data_dir, НЕ от аргумента build_game_loop → TemporalEngine писал world_tick.json в ЖИВОЙ data/sessions → RUN2 наследовал sim_tick RUN1 → систематический MISMATCH; диагноз iron_river_trace: 3 мутирующих файла вне изоляции, world_tick — каузальный, логи — write-only невиновны).
⚙️ Живые баги (найдены методологией, починены): Н-18 двойной трек (расписание конкурировало с activity-MOVE через арбитраж → Люсья вечно kitchen↔kitchen_bed_1 при step=0; гейт `_has_activity` над обеими ветками LifeEngine → EAT E6-E9 GREEN впервые, hunger 0.9→0.00, Люсья consumed tick=34 — первое насыщение за историю наблюдений); гонки за порции (3 NPC→1 порция; ревалидация живости: DESTROYED→target_consumed_by_other, holder→target_taken_by_other — быстрый провал вместо 60-тикного таймаута).
⚙️ IPT-ремонты: COMMIT-CARDINALITY (futures-after-shutdown: пул гасился Stage-3 навсегда; фикс — пересоздание после дренажа); TICK-CARDINALITY (артефакт spatial_registry.json не существует и writer'а в дереве НЕТ — grep-факт; skip-семантика по брату NPC-CARDINALITY: None→True-пропуск).
⚙️ Инструменты (NEW, 7): iron_river_ab.py (A/B канон-хеш: 2 подпроцесса, PYTHONHASHSEED=0, cool-down 3с, WIN-очистка, полная изоляция), iron_river_diff.py (полевой диф канонов — источник по путям), iron_river_lusya.py (per-tick трасса NPC), iron_river_eatlog.py (ACTIVITY-лог+трасса), iron_river_trace.py (снимок-дифф мутирующих файлов), f3_combat_smoke.py (боевая воспроизводимость), ttl_probe.py/f4_paths_probe.py (точечные зонды).
⚙️ Диагностическая хронология (9 гипотез→1 истина): jsonl-обрезка→хаос (память каузальна!); LLM-ретраи→P2-5 в приборе (router-stub); горячий RUN2 (cool-down); hashseed (P1 — опровергнут как единственный); финально: settings.data_dir подмена→3×MATCH. Каждый шаг — зонд+патч+серия, ни одного вслепую.
⚠️ Уроки (мои): диск переполнен WinError 112 (12×1.35ГБ temp-копий replay.db — все вердикты на dying-диске недостоверны; ignore+WIN-очистка); atexit+rmtree на Windows не удаляет открытые SQLite-хэндлы (48 утечек — родительская очистка); logger.info слеп в _quiet (S216×3); enum-гадание COMBAT_HIT (урок №3×3); якорь-по-памяти×5 (S198, TTL, ignore, logging-import, os-import); inline-python в PowerShell = кавычный ад (все зонды — файлами); NameError в собственном харнессе×3 (import порядок). Урок-вершина: «харнесс с диагностикой НЕ живёт в гейт-файле».
📁 Точечные: task_scheduler (F1a/F1b/константа/F2-проброс), speech_scheduler (F2), tick_orchestrator (F3-1 event-id), combat_subscriber (F3-2 KernelRNG), activity_lifecycle_service (ревалидация целей + SERVE-гейт), life_engine (Н-18: гейт activity над обеими ветками легаси), IPT.py (COMMIT-CARD пул + TICK-CARD skip). NEW: 7 харнессов. НЕ ТРОНУТЫ: clock.py, DecisionHub, WorldActionType, Commitment-онтология, S259-файлы.
IPT: ✅ 45/45 (0 CRITICAL). КРАСНЫЕ ИНВАРИАНТЫ: 0 🔴 → 0 🔴

### S262: R5 CAUSAL SLICE 1 — DesiredChange: угроза → желаемое изменение → выбор способа (протокол Мастеров R2-R5; ADR-O-394) | ✅ RED ImportError-контракт · GREEN 8/8 (ядро: одна цель → разные способы, A/A) · регрессия 82/83 (gc09b pre-existing) · IPT: дельта 0 красных (изоляция: 43/2 без дельты — оба CRITICAL = состояние ветки Говорим_3: SpatialRegistry/Iron River + futures-teardown, чужая очередь)
🎯 Первая ступень новой ENIGMA: NPC действует из причины изменить состояние (DesiredChange рождается в бэкенде из threat+атрибуции), а способ (flee/intimidate/call_for_help/block_path/attack/warn) ВЫБИРАЕТСЯ оценкой из существующих машин и деформирует utility через Modifier Contract. LLM — только вербализация. Ноль новых интентов; DecisionHub не переписан (+1 аргумент по epistemic-образцу S189).
⚙️ Археология R2-R4 (без кода) предшествовала срезу: карта источников речи → десять причин с анатомией → универсальность формы «ХОЧУ ИЗМЕНИТЬ» (17/17 случаев, 31/100 UNKNOWN-клеток = спецификация слоя) → выбор сценария stop-hostile (максимум пригодных деталей: threat жив, атрибуция из fear-оси/belief, 5 способов в enum, следствие _BASE_DELTAS готово). Инциденты фазы: stash-патологии (pathspec не берёт untracked — D-R5-STASH-UNTRACKED; чужие stash-записи на стеке — дисциплина «pop только свою запись после двойной проверки списка»), смена ветки посреди среза (Говорим_2→Говорим_3: baseline IPT не переносится между ветками — D-R5-BRANCH-BASELINE), изоляция untracked-файлов физическим выносом (паттерн v2).
📁 domain/desired_change (NEW frozen), services/npc/causal_slice_threat (NEW продюсер+to_modifiers), npc_tick_pipeline (проводка), decision_hub (8-й модификатор), tests/gameplay/test_r5_causal_slice_threat (NEW 8)
⚠️ Параллельная сессия (Iron River) активна в дереве: iron_river_*-файлы и её stash-записи нетронуты; SpatialRegistry-CRITICAL — кандидат в её досье. DEBT-E1-WIRING (S259-наследие) остаётся открытым: проводка замкнута, NPC→player-продюсер отсутствует — ожидает проектного решения Б/Г.

### S263: R6 CAUSAL SLICE 2a — capability: голод → желаемое изменение → кто способен дать (ADR-O-395) | ✅ RED ImportError-контракт · GREEN 14/14 после поимённого STOP-разбора · регрессия 96/97 (gc09b pre-existing) · IPT 45/0 = baseline ветки (изоляция чистая)
🎯 Второй причинный срез: A голоден, своей еды нет → DesiredChange(A.food↑) → capability-скан мира → выбор addressee (B) → оценка способов. Первое ЖИВОЕ контрактное различение target_of_change ≠ addressee (срез-1: совпадали). Контрфактический закон CS13 (Мастер Тай): 5 миров (W1-W5) меняют цепочку candidate set → methods → addressee, не одно число. W4b — первый росток grievance: еда у врага + thief-натура → steal.
⚙️ Продюсер HungerDesiredChangeProducer (чистая функция, зеркально Threat): NEED_GATE=0.5 (calibration, не закон) · CS11 свой-ресурс-гейт (путь предшественника EAT) · CS9 capability-вакуум = честный None (production-режим по умолчанию!) · выбор B: has_good × достижимость ≤5м → trust-доминанта · способы = СУЩЕСТВУЮЩИЕ интенты: trade (afford×trust) / request_service (trust>40) / steal (архетип×will, S209-паттерн) · viability-гейт · нормировка суммы ≤1.0 (CS12, анти-двойной-счёт с eco_modifiers). Фабрика acquire_resource в desired_change.py (REASON_NEED/STATE_TYPE_RESOURCE уже существовали). ГРАНИЦА: SOCIAL-сессия S262+ владеет target-stage — _resolve_target/social_target_resolver НЕ тронуты; addressee-интеграция = срез 2b (координация, addressee станет ВХОДОМ их target→interaction цепочки).
⚙️ ИНЦИДЕНТ GREEN-v1 (закрыт в сессии): 7 failed — я предположил семантику has_stock из имени метода (нарушение §13.1, признано) → зонд доказал: has_stock читает stock_for_sale (товар на продажу), фабрика его НЕ наполняет; has_good читает goods (личный запас). АРХИТЕКТУРНАЯ НАХОДКА R6: в EconomicProfile два capability-поля — «имеет» (живое) и «продаёт» (вакуум, 0 production-писателей, класс WorldObject.ownership). «B can sell» ≠ «B has» — первая клетка будущей формы agent×operation×resource (Тай §7), не заперта, не реализована.
📁 domain/desired_change.py (фабрика acquire_resource), services/npc/causal_slice_hunger.py (NEW), tests/gameplay/test_r6_causal_slice_hunger.py (NEW, 14: T1-T4/W1-W5+W4b/A1-A3/A-A)
⚠️ Долги: SLICE-2b (проводка causal_modifiers+addressee в DecisionHub — после baseline SOCIAL-сессии) · ECON-FACTORY-GOLD-FALSY (profile_factory игнорирует gold=0.0 falsy — молчаливый wealth-дефолт) · stock_for_sale-вакуум (писатели отсутствуют) · food ∉ PHYSICAL_OBJECT_TYPES → конфиг-путь стока через carried_objects мёртв (контент-долг) · TECH: None в п.8 отчёта приёмки (исполнение — 2b)
⚙️ ФИНАЛ 2b-i (та же сессия): ЖИВОЕ PRODUCTION-ДОКАЗАТЕЛЬСТВО — зонд живого
конвейера (build_game_loop + idle_tick, hunger=80 факт тела) даёт маркер
[CAUSAL_SLICE_HUNGER] npc=maid_lusya/merchant_goran/blacksmith_orm
desired_change=resource:<id>→tavern_keeper_tornin methods={trade: 0.64/0.58}
— причинная цепь голод→capability→addressee→модификаторы→compute() жива в
production; Торнин молчит (CS11, свой food). trust-различение живое (0.64 vs
0.58 из SSOT-отношений). Контент-наблюдения: request_service=0 у всех (trust≤40
ниже порога — «попросить» сжат контентом до покупки); steal=0 (will честна).
⚙️ ПОЙМАН P0 ЧУЖОГО СРЕЗА: проводка R5 (threat) была МЕРТВА в живом конвейере
с мержа S261 — _allies_cache не существовал в скоупе, NameError → no-op каждый
тик, невидимый для юнит-сьютов (продюсер вызывается напрямую) и IPT
(fail-молчание by design). Воскрешена минимально (allies=0 + DEBT-R5-ALLIES;
источник союзников — OpportunityContext perceived_allies, ADR-O-366, ниже по
потоку). Уроки: D-R6-LIVE-PROBE (проводка без живого прогона = непроверенная
проводка — закон обоих срезов) · D-R6-DUPLICATE-PATCH (после второго патча на
регион следующий запрещён — только каноническая замена региона) ·
D-R6-PARTIAL-PATCH (прогон после каждого блока, не пакета).
📁 + npc_tick_pipeline.py: воскрешение threat (allies=0, DEBT-R5-ALLIES),
удаление дубликата-черновика hunger, body_state-источник голода (0-100
LEGACY до S2B.10, приоритет над needs-слоем)

### S264: SOCIAL Vertical Slice — причинная цепь замкнута: pressure→intent→target→interaction→relationship→next-tick (ADR-O-394) | ✅ D1–D4 ×3 детерминированно · trust-дельты 5 NPC · регресс-батарея (в прогоне)
🎯 Четвёртый вертикал (мандат S259/приказ WORKER): «потребность в другом → другой → взаимодействие → изменившиеся отношения». Существующие органы, не новый движок; S259-патчи (α: event.source-fallback; γ-1: campaign_id из Phase8Context; γ-redesign v1: fresh per-tick _SNS) + S261-фундамент (контракт D) — всё сошлось.
⚙️ Найденные и закрытые живые баги (лестница S262): (1) IDLE-НЕМОТА: execute_pending гейтился рудиментным `pending_tasks in scene` (BUG-CORE-010 перенёс задачи в DialogueQueue — поле сцены пусто навечно) → idle-мир никогда не дренажировал очередь: интенты рождались, очередь капилась 20, реплики — лотерея worker-таймингов (25/3/0/0/7/0/0 хронология). Фикс: безусловный execute_pending в idle_tick (:1470). (2) F2-РЕГРЕССИЯ dequeue: PACING-break разрывал весь дренаж при первой непрошедшей задаче (break корректен для wall-clock 2с, ошибочен для game-time 2 тика — чужой спикер готов) → processed=1 при 20 ожидающих. Фикс: continue. (3) ВАЛИДАТОР-KRАШ: NPCState поднимал ValueError на broadcast-интентах без target (SPREAD_RUMOR/CALL_FOR_HELP — семантика 'all', S254-прецедент) на idle-мире с одиноким NPC → TICK_CRASH тик-1. Фикс: broadcast-исключения в валидаторе. (4) ENQUEUE-ДЕДУП: постоянные интенты (TRADE/CALL_FOR_JOB/OFFER_JOB каждый тик) забивали очередь до капа, TALK конкурировал со спамом, OVERFLOW-потери 2-3/тик (Н-56). Фикс: дедуп (speaker, intent) при enqueue + CANCELLED-протокол (багфикс моего же skip: молчаливое исчезновение без terminal — L4-нарушение, поймано TICK_CRASH'ем). (5) Адаптивный drain-limit 1→3 при переполнении.
⚙️ Инструменты: superbox_social_deterministic.py (NEW: SyncPool — синхронный executor в тест-зоне, контракт ThreadPoolExecutor.submit(fn,*args); LLM-free; data-изоляция с parents[2]-стражем против чужого корневого app/ — урок молчаливого импорт-перехвата). D1–D4 = детерминированная приёмка полной цепи.
⚙️ Диагностическая хронология (12 шагов, каждый зонд→факт): SPY-слепота цикла (async публикации после расчётной точки) → счётчик в _spy → «перемежающиеся миры» → data-изоляция → POOL-DIAG (очередь 20, processed=1) → дедуп-подозрение → 7 аргументов SyncPool (контракт submit) → TICK_CRASH валидатора → enqueue-init AttributeNotFound → D3-формы стора (dict-вложенность). Уроки: sync-труба решает timing-загадки быстрее логики; чужое корневое дерево app/ (26.07) молча перехватывает parents[4]-импорты — страж обязателен; молчаливый skip задачи = L4-нарушение с отложенным TICK_CRASH.
📁 Точечные: game_loop/__init__ (idle execute_pending), task_scheduler (PACING-continue; enqueue-дедуп+CANCELLED; _enqueued_keys; drain-limit), npc_state (broadcast-исключения валидатора). NEW: superbox_social_deterministic.py. НЕ ТРОНУТЫ: SpeechScheduler (F2-оси), DialogueExecutor, DecisionHub, S259/S260/S261-патчи.
IPT: батарея-в-прогоне. КРАСНЫЕ ИНВАРИАНТЫ: см. батарею.
⚙️ S264-финал (выравнивание, приказ Мастера «выравняй веса, выравняй очереди,
   мир последователен и плавен в игре и тестах»): (6) ТЕМП НАСЫЩЕНИЯ:
   consume-гашение current/duration (порция = полное насыщение за
   длительность жевания; прежний rate=1/3 давал 0.33 при фоне 0.08/тик —
   еда слабее голода вдвое); (7) ВЛАДЕНИЕ-ФИЗИОЛОГИЯ: _tick_needs не
   растит потребность при живой activity_state (третий слой владения:
   движение Н-18 → реплики post_decision → физиология); (8) Порции 3→6
   (tavern.json campaigns+staging: obj_45-47; тест 42-47, ==6; урок:
   campaigns-вставка удалила obj_42-44 — вербатим-контроль обязателен);
   (9) ТЕСТ-КРИТЕРИИ СОБЫТИЯ: E7 меряет hunger В МОМЕНТ терминала
   (захват в break), не пост-едовый слепок (живой мир голодает снова —
   событие = истина); E9: конвертерный ярлык чист, легаси-дисплей —
   OPEN-LABEL-LEGACY; MAX_TICKS 40→60 (путь+2 порции при живом ритме);
   (10) R5-ФИКС: ThreatDesiredChangeProducer state=state → state=state_l2
   (TickState не имел perceptual_kernel → гейт 0.35 → вечный None; 4-й
   случай D-R6-LIVE-PROBE). E9-GREEN впервые за всю историю наблюдений.
📁 + life_engine (_tick_needs-гейт), activity_lifecycle_service
   (consume-темп), post_decision (R5 state_l2), tavern.json ×2
   (порции), eat_vertical_test (MAX_TICKS, терминальный E7, E9-канал,
   _FOOD_IDS 42-47), npc_tick_pipeline (R5-проводка).


### S265: R7 CAUSAL SLICE 3 — grievance: накопленный вред → направленное последействие (ADR-O-397) | ✅ RED ImportError · GREEN 10/10 (2 поимённых STOP-разбора: who-контракт, drives-override) · причинные сьюты 36/36 · регрессия 110+gc09b · IPT 45/0 · PRODUCTION-ПРОБ: живые обиды канона без инъекций
🎯 Третий причинный срез (программа R5→R7): холодное последействие вреда. CS14: обида = ПРОЕКЦИЯ осей RelationshipStore (trust-дефицит ≤-12 × fear-свидетельство), не сущность; флаги is_angry/revenge/GrievanceStore запрещены. CS15: горячее/холодное разделение — threat_gradient ≥ 0.35 = территория R5 (ADR-O-394), обида живёт когда угроза УШЛА. CS16: подавленная жертва молчит (viability-гейт). Каскад причин в конвейере: threat(R5) > hunger(R6) > grievance(R7).
⚙️ Продюсер GrievanceDesiredChangeProducer (единый контракт продюсеров who+state+rel+disposition+allies — hunger-паттерн, НЕ threat-паттерн state.npc_id) + фабрика grievance_hold + REASON_GRIEVANCE. Способы существующие: intimidate(control×harm×храбрость×hp) / warn(значимость×disposition) / spread_rumor(harm×бесконтрольность — репутационная месть) / call_for_help(страх×союзники). Интерференс-оборона: гейт социального контекста (изолированная сцена → честный None, зеркально W5).
⚙️ PRODUCTION-ДОКАЗАТЕЛЬСТВО: [CAUSAL_SLICE_GRIEVANCE] guard_borko→thief_shadow, thief_shadow→maid_lusya, tavern_keeper_tornin→thief_shadow — ТРИ ЖИВЫХ конфликта village_relations прошли полный путь в живом конвейере БЕЗ инъекций; контраст натур живой (intimidate 0.168/0.126/0.084).
⚙️ ГЛАВНАЯ НАХОДКА: grievance-модификатор ВСКРЫЛ спавшую годами дыру «intent without target» (npc_state.py:893, Stage-1-валидатор) — причинный вербальный интент впервые победил у одиночного NPC в изолированной p7_e1 → ValueError. Дыра старая, триггер мой. Системное лечение = 2b-ii: causal_addressee → _resolve_target (территория SOCIAL S264; мой addressee — готовый вход их target-stage). Доказательство причинности по Тай: модификатор изменил выбранный ИНТЕНТ, не число.
...
### S270: O-399 Worker Effect Outbox + анатомия experience-канала (5-слойная кампания детерминизма) | ✅ IPT 45/45, A1==A2 межпроцессно, O-399 CLOSED
🎯 Приказ Мастера: А+Б → В → MATCH → SCALE LAW; «worker completion timing больше не имеет права определять историю мира»; «мы не ускоряем LLM — мы отнимаем у worker право незаметно переписывать историю мира».
⚙️ **Слой 0 — ЛАВИНА L1 (P0 DEBT-HISTORY-TAX):** проб доказал квадратичную лавину дубликатов (4 archive-вызова: 99→396 строк; `archive_old_events` копирует в архив без очистки источника — Rule 28-наследие, DELETE удалён). Фикс А: `NOT EXISTS`-идемпотентная архивация (дедуп хранилища, не удаление фактов; гейт Мастера: 99×4, query_raw=100). Фикс Б: tiebreak `ORDER BY tick_id, source_id, id` в обеих SQL-ветках; контракт порядка зафиксирован (RAM=insertion, SQL=канонический; канонизация отложена в watermark-итерацию). Regression 2/2.
⚙️ **Слой 1 — O-399 WORKER EFFECT OUTBOX (ADR-O-399):** воркер `bus.publish`/scene_state-мутации/`record_talk`/`reset_context` — все observable-эффекты из фонового потока (граница причинного тика разорвана). `_TaskArtifactRecord` (frozen) → outbox → deterministic drain в 2 существующих точках (симметрия S203.4/F23, безусловный при pending_tasks=0), batch-сортировка `(submit_tick, task_id)`, порядок эффектов зеркалит прежний: events→economy→dialogue→speech_reset. `submit_tick` из main-thread явно. Deterministic `task_id = dlg-{game_time}-{seq}` (uuid4 — табу ADR-O-365; ambient-артефакты доходили до каналов → W5 был недоказуем). Structural probe: worker=DATA-only. W5-поправка Мастера: commit не привязан к N+1, completion-order вне канона.
⚙️ **Слой 2 — Iter-1 event-dating:** `tick=_get_tick()` в подписчике = arrival → L1-датировка arrival'ом (шов доказан диф-фактом 274ef419≠a0039402). Фикс: drain штампует `event_tick=submit_tick` в payload, подписчик потребляет с fail-open. Temporal semantics пяти consumers доказана чтением (STM/narrative/journal — arrival-инертны; L1 — единственный шов).
⚙️ **Слой 3 — CONSUMER PROOF + трёхчастная модель:** «text presentation-only» — опровергнуто грепом: text→STM→LLM-промпты→решения (S145-петля жива, и это правильно). Proposition text-free (req.proposition до LLM, :150). Модель Мастера EVENT(что произошло; event_tick=submit)/EXPERIENCE(материал; arrival легален)/MEMORY(что сохранил; своя семантика) — статус, не слой.
⚙️ **Слой 4 — Опция 1:** delivery-order выведен из канона (сортировка ТОЛЬКО в `_canonical_hash`-проекции по `(timestamp, speaker_id, target_id, text)`; runtime/UI/память не тронуты). Повод: O399-COUNT зонд — все pipeline-счётчики байт-в-байт (96==96, enqueue=admit=submit=push=drain=append), расхождение = чистый interleaving.
⚙️ **Слой 5 — PYTHONHASHSEED (четвёртый слой, финал):** канон дрейфовал между процессами при неизменном коде (`2f30fe2d`→`63a5dd4f`→…). Доказательство с обеих сторон: без env hash('x') различается между процессами, с `PYTHONHASHSEED=0` — совпадает. Причина: `hash(prompt_lower)` в MockProvider._deterministic_pick (фикс S269 был внутрипроцессным). Фикс: md5-stable digest (production, без env-флага; smoke-proof: два процесса → одинаковый pick). Побочный каскад: детерминированный task_id убил uuid4-соль KernelRNG ambient.
⚙️ **Вердикт кампании (4× структурный диф + structural diff A2-vs-B 200 путей):** causal skeleton (NPC/world/epistemic/relationships) байт-в-байт при разной latency — все расхождения только в Experience-списке. Latency → arrival LLM-материала → STM/темы → DecisionHub → subsequent intents → другой состав fast-path/ambient Experience. **O-399 CLOSED:** «Worker completion timing не изменяет causal skeleton; при фиксированной execution configuration Experience воспроизводим между процессами; latency каузально меняет состав Experience через arrival — признанная temporal boundary» (не баг: время поступления опыта → изменение будущего = механизм живого мира). Iter-3 (narrative-before-decision) — НЕ делать. LEGALITY MAP вшита в Mode E вывод.
⚙️ Три контракта Мастера: **Replay** (same input+config → same trajectory; межпроцессно доказано), **Butterfly** (другое действие → другая жизнь через причинную цепочку — смысл ENIGMA), **Performance** (T(NPC,History) — следующий акт). DriftLab = измерительный стенд, не цель (CDS-принцип: наблюдение не создаёт причинность).
📁 task_scheduler (outbox/drain/event_tick), mock_provider (md5), l1_chronicle (NOT EXISTS+tiebreak), npc_dialogue_subscriber (event_tick), dialogue_queue (_dlg_seq), drift_laboratory (LEGALITY MAP), probes: o399_worker_boundary/md5_crossprocess/canon_multiset/structural_diff, tests: worker_outbox_determinism (4) + l1_chronicle_archive_idempotency (2). Коммит b396a4c8 (наши патчи частично вошли в соседний merge 7bb85171/488e074d — верифицировано грепом 6 маркеров).
⚠️ **Соседняя сессия (cline):** stash-чекпоинты в нашем дереве + merge → переключение ветки; сериальные смерти прогонов ночью ≈ git-операции под живым процессом; доказательства законсервированы (reports/o399_evidence_20260921_0012). Режим: окна работы, наши зоны не пересекаются (они: Needs/SLEEP вертикаль).
⚠️ **Долги (владельцы):** speech_reset 🔴 (residual causal channel — WORLD FACT vs COMPUTATION FACT не решён); tick_provider wiring 🔴-перед-масштабом (silent time collapse: KernelRNG ambient видит tick=0 — три исторических момента одна координата); SQLite-teardown 🟡 (персист L1/relationship молча умирает при закрытом соединении); Г-кап recent_dialogues 🟡→актуально-для-SCALE (3007/3000 тиков append-only — первый измеренный узел History-map); снапшот-vs-хвост (±3 записи — момент наблюдения, не мир).
⚠️ Уроки: (1) inline-Python в PowerShell ломается об кавычки — file-based probes; (2) logger скрыт в lab-выводе — print-зонды Часть VIII.5; (3) «process exited» ≠ причина смерти — класс причин, не факт; (4) паттерн на 3 точках ≠ закон (смерти оказались внешними); (5) скриптовая зачистка без бэкапа = IndentationError-вечер — якорь из свежего чтения; (6) LEGALITY MAP комментариями не печатается — код действием.
IPT: ✅ 45/45 (финальный). КРАСНЫЕ ИНВАРИАНТЫ: 0 🔴 → 0 🔴. Реплеи: L0′ 2×3000 MATCH (2f30fe2d, до md5), A1==A2 3000 MATCH (63a5dd4f, md5-канон), B MATCH (59315285, легальная граница).

### S271: SLEEP VERTICAL SLICE — первая причинная вертикаль NEED→SATISFACTION + Visibility Contract | ✅ IPT 45/45, SDF 0, RCB 1/0, SPK 8/0, bsf 2×GREEN
🎯 Слои: (1) Visibility Contract k0t8yr: visible=False ⇔ is_sleeping AND node==BED (Н-4: факт _ref.node_id==position, не «affordance существует»; сравнение по каноническому хвосту — NodeRef каноничен с префиксом, new_position смешанный формат). (2) D2-forensic (влздикт Мастера: «спал пару тиков и вышел»): раскрыт label-конвейер — schedule/need рельсы писали sleeping-лейбл БЕЗ физиологии; SleepOnsetResolver (S2B6-B) жил и был подключён, но behavioral-лейбл его не кормил. (3) Vertical slice: единый принцип «лейбл = проекция факта, intent ≠ physiology».
⚙️ Патчи production: V-2 need-driven wake-edge (Tuple-сигнатура+tick — флаг отклонения от схемы), V-3a brief_exit-невидимость удалена (tuple мёртв, потребителей 0 — греп), Н-4 _on_bed факт-BED (нормализация хвоста — флаг), going_to_sleep pending-статус (шторм SCHED_TRACE prev='' закрыт — ownership: мой же отложенный лейбл, до/после-proof), need-рельс лейбл-гейт (sleeping не пишется при intent), X need-cross-loc → relocation-форма (SURVIVAL/0.9/final_* — зеркалирование FIX-SCENE/P2; ownership silent drop: simulation.py:134 process_intents с локальным spatial_service, чужой граф), Y resolver принимает going_to_sleep (только resolver, is_sleeping глобально не расширен), Y2+возврат лейбл-проекция из факта onset (SceneChange activity, dual-rail), V4 pending ≠ withdrawal (going_to_sleep не убивает physiological sleep; ownership: наш pending убивал сон за 1 тик через S2B6-B В4), латентный writer :1509 устранён (двойник V-2-класса).
⚙️ Acceptance (forced harness, shelter_urge=1.0 инъекция входа через _resolve_npcs_snapshot — живые дикты _npc_cache, S214 β-гибрид): зондиный trace lusya/tornin — urg=1.0 pos=node_16 cur='working' → going_to_sleep в пути → pos=kitchen_bed_1 cur='sleeping' phys=True → PERSISTENCE тик за тиком → спам Need-driven умолк (recovery/shelter ↓) → wake-threshold за 300-тиковым окном. Каждая стрелка — writer/trace, не реконструкция. Регрессия: IPT 45/45, SDF нарушений=0 exit0, RCB 1/0, SPK 8/0, bsf ДВОЙНОЙ GREEN (guard+shadow; tick24-артефакт не воспроизвёлся).
📁 life_engine (V-2/V-3a/Н-4/going_to_sleep/need-гейт/X/латент:1509), sleep_onset_resolver (Y), sleep_lifecycle_service (Y2+V4), tick_orchestrator (Phase 0.6-конвейер верифицирован), movement_engine (ownership silent drop верифицирован кодом), simulation.py (:102-107 фильтр — пропуск верифицирован), tests/sandbox/SUPERBOX/scenarios/sleep_forensic.py (forensic+forced harness), tests/sandbox/live_visibility_monitor.py (живой монитор, D1-D5), sdf-классификация SLEEP_FIXTURE_VALID/INVALID_NO_BED (топология: guard_bed/kitchen_bed_1,2/tent_1..9).
⚠️ Datum'ы: Shadow=INVALID_NO_BED (дневной сон cross-loc на tent_3 — авторские данные, не ошибка); ARBITER-шум proactive к чужим кроватям; activity_overrides — мёртвое поле (writers 0); DEBT: DEBT-QUIESCE (needs-старт nondeterminism — forced harness НЕ закрывает), O-399 граница (не наша зона, вербатим Мастера), routine["current"]-мутации перечнем.
⚠️ Уроки: (1) патч без smoke-импорта = Tuple/Path NameError ×2 — IMPORT-гейт (исполнение модуля) вместо py_compile; (2) дым-проверка путей ДО 6-минутного прогона; (3) для захватов логов _Col-канал — маркеры должны включать каналы доставки (INTENT_SCHEDULE/GATE/ARBITER); (4) «process exited» heartbeat CPU=0 — артефакт Start-Process, верить полному артефакту; (5) фильтры-дампы режут релевантное трижды — сырой дамп по имени NPC; (6) инъекция входа — только после первого тика (снапшот LEN=0 до) и только в живые дикты (кэш in-place, :690).
IPT: ✅ 45/45. КРАСНЫЕ ИНВАРИАНТЫ: 0 🔴 → 0 🔴. SLEEP VERTICAL: NEED→DELIVERY→ARRIVAL→ONSET→PERSISTENCE→RECOVERY→WAKE — CLOSED GREEN (forced harness; DEBT-QUIESCE остаётся OPEN). Visibility: CLOSED. STOP POLISHING.

### S272: Предгейт P1 tick_provider — гипотеза опровергнута, найден и закрыт fast-path submit_tick omission | ✅ IPT 45/45, outbox 4/4, liveness 7/7, замок 2/2
🎯 Приказ Мастера: П1 предгейт перед SCALE (обязательная проверка); решения: вариант B (фикс сразу), getattr не трогать (cleanup после SCALE), П2 recent_dialogues — исследование внутри SCALE, канон-хеш без вердикта не менять.
⚙️ Археология (4 контура, живое чтение): lifecycle QueuedTask.tick прослежен целиком — post_decision:220 (tick=ctx.tick_number) → _task_dict["tick"]:254 → execute_pending:452/492 (пронос через DialogueQueue) → _reconstruct_task:841 (ЖЁСТКОЕ индексирование, не .get) → executor. QueuedTask.tick — обязательное поле без дефолта (domain/execution.py:54). **Silent Time Collapse НЕ обнаружен**; гипотеза ТЗ «task — dict без .tick» опровергнута: конфлуия QueuedDialogue (обёртка очереди, поля tick нет) с QueuedTask (доменный контейнер). getattr(task,'tick',0) в npc_conversation:223 возвращает честный тик создания интента (каузально правильно по O-399: completion-time не входит в RNG-координату).
⚙️ **Обновлённая формулировка долга (вердикт Мастера, заменяет 🔴 «tick_provider wiring — silent time collapse»):** проверка lifecycle QueuedTask.tick показала, что tick creation корректно проходит enqueue → serialization → dequeue → reconstruction → worker; Silent Time Collapse не обнаружен. Обнаружен отдельный fast-path omission: execute_pending fast-path (:399→403) вызывал _process_tasks_async без submit_tick → дефолт 0 → _TaskArtifactRecord.submit_tick=0 → drain штамповал event_tick=0. Дефект касался production-каналов fast-path (warn/spread_rumor/steal — claim-producing, S202/S206/S209): их события датировались тиком 0 независимо от реального времени — тот же класс, что убитый Слоем 2, но на sync-ветке.
⚙️ Фикс (FIX_SCOPE 1, симметрия контракта: process_tasks:333 и dequeue:556 уже передавали тик): fast-path передаёт scene_state.get("tick", 0). Не изменение семантики — восстановление существующего контракта. O-399 вердикт не затронут.
📁 task_scheduler.py (:403, +4 строки комментария-шрама), tests/micro/test_fastpath_submit_tick_stamp.py (новый замок: test_fastpath_submit_tick_is_scene_tick — submit_tick==42; test_fastpath_event_tick_stamped_on_drain — drain event_tick==42 через перехват bus.publish, без зависимости от EventType).
⚠️ Инцидент сессии: серия INVALID прогонов (collected 0 items; вставки с синтетическим контекстом и мутирующими путями) — отклонены по §3.12; все утверждения о прогонах, сделанные до валидации, отозваны (урок S212 о фальсификации GREEN повторён на приёмнике). Сброс: пин дерева git rev-parse + канонические маркеры + правило приёма (один корень, collected>0, ИТОГО-строка). Все гейты ниже — валидные прогоны на запиненном дереве.
IPT: ✅ 45/45. КРАСНЫЕ ИНВАРИАНТЫ: 0 🔴 → 0 🔴. Гейты: test_worker_outbox_determinism 4/4, test_dialogue_liveness_gate 7/7, test_fastpath_submit_tick_stamp 2/2 (замок дефекта). П1 CLOSED. Долги: getattr-cleanup 🟢-после-SCALE (Мастер: не смешивать дефект с читаемостью); П2 recent_dialogues — первый измеренный узел SCALE, только исследование.
⚙️ **SCALE LAW — первая поверхность данных (решение Мастера: карта эксплуатационного диапазона T(N,H,C), не «доказать 600»):** инструмент scale_instrument.py (обёртки вокруг DriftLab Mode A: per-tick ms/events/L1/WS + population-клоны через scale_population.py с верификацией загрузчиком LOADER SEES) + датасет reports/scale_law_dataset_v1.md. Сетка: 6×200 (47.6/57.2), 12×200 (68.7/83.8, GREEN), 24×200 (110.1/133.8, YELLOW), 24×3000 (126.3/190.1). **Population-ось:** 1.44×→1.60× за удвоение (сублинейно с ускорением). **History-ось (вердикт Мастера, LLM-off, N=24): накопительный CPU/H-налог не обнаружен до 3000 тиков — плато 95-128 ms/tick после прогрева.** L1 cost center: query_raw доминирует (42%→61.6% доли), абсолютный поток упал до 27.4/тик (активность мира throttled); рост доли ≠ рост стоимости. **archive_old_events=6 за 3000 тиков — S270 идемпотентный архив пережил длинную историю, лавина не воспроизвелась.** **recent_dialogues: сатурация 42 с ~451 тика — гипотеза «растёт до 3007+» отменена как универсальная; П2 переведён из «опасного долга» в археологический вопрос (кто ограничивает — механизм плато не установлен); cap/TTL/канон-хеш не тронуты.** RAM: 172→205 MB (линейность/плато не установлены — вопрос 24×10k). Оговорка профиля: DRIFT_NO_LLM=1 (endurance, без llama-server) — отдельный benchmark profile, C_llm не смешивать.
⚠️ Инциденты: (1) NameError self в _report (патч-сигнатура) — поймано первым же прогоном, фикс прокидкой параметра; (2) серия INVALID-вставок (мутирующие пути, синтезированные даты/файлы) — отклонены по §3.12; приём только: сырой вывод + единый пин + маркеры + отсутствие Traceback; (3) GROUND_TRUTH FAIL = Н-53 pre-existing шум, не сигнал о клонах.
⚙️ **SCALE v1.2 — клетка 2×2 + локализация владельца History-tax:** 60×200 (220.3/293.9, RED) + 60×3000 (283.6/436.0, RED). History multiplier: 1.147× (N=24) → 1.287× (N=60); Population multiplier: 2.001× (H=200) → 2.246× (H=3000); взаимодействие осей ~+12% над независимой моделью (не аддитивно, не квадратично). **Владелец: integration.py:410 (Фаза 9, ADR-O-305) — query_raw(_npc_id) без t_from → дефолт 0 → полное сканирование хроники NPC на каждой кристаллизации** (per-caller атрибуция: 100% вызовов из одной точки, монотонная доля 42→62→78% по оси H). append при 60 NPC выходит на первое место на H=200 (N-линейность записи). RAM коррелирует с накопленным L1-объёмом (198k calls → 230 MB).
🎯 **Вердикты Мастера (закрытие S272):** (1) watermark — после C_llm; (2) C_llm benchmark — следующий акт, до watermark (LLM-on меняет событийность/частоту Фазы 9/профиль query_raw — решение о чтении хроники принимается относительно реальной нагрузки); (3) 120/240/600 — заморозка (дорогие цифры неоптимизированного query_raw пришлось бы перемерять); (4) watermark требует не «оптимизации», а **доказательства семантической эквивалентности** detect(H1+H2) ≟ detect(H2, state(H1)) — вопрос математики PatternDetector; A/B-гейт: causal skeleton байт-в-байт, иначе это новая модель, не оптимизация. «Фикс — отдельная война; сначала строим закон, потом меняем систему, потом повторяем тот же benchmark».
📁 + probes: scale_instrument.py (per-tick ms/events/L1/WS, per-caller query_raw, population-клоны, manifest), scale_population.py (клоны особей + само-верификация LOADER SEES через живой загрузчик), reports/scale_law_dataset_v1.md (T(N,H,C) поверхность v1.2, LLM-off endurance profile, benchmark operating threshold 100/200 p95), jsonl-сырые данные в scale_reports/ (16 прогонов).
⚙️ **v1.3 — C_llm (живой llama-server, 24×3000, commit 9ecbac81):** stationary tick-cost совпал LLM-off/LLM-on: p50 126.3 vs 126.9, L1 27.4 vs 28.0, q_raw — тот же единственный caller (integration.py:410, 100%). max=5581.8 мс (тик 1) = startup/warm-up C_llm, со stationary p95 не смешивается. Диалоговая сатурация 42→45 — свойство throttle, не отсутствия LLM. **Вывод: C_llm не меняет решения по History-tax; C₀-данных для watermark-вердикта достаточно.** Single-run/live-LLM nondeterminism — обобщение за точку не делается.
...
### S274: WATERMARK — инкрементальный PatternDetector State (ADR-O-400) | ✅ Oracle 11/11, A/B 24×10k RED→YELLOW, IPT 45/45
🎯 Вердикты Мастера: (1) Сначала equivalence gate, не код — «если watermark ускоряет в 5 раз, но меняет судьбу Люси на 187-м тике — это не оптимизация, а новая модель»; (2) достаточность state доказывается, не постулируется: машинная эквивалентность — гипотеза для теста; (3) «не оптимизировать query_raw механически — сохранить семантику, уменьшив повторное чтение»; (4) production=ON, OFF=rollback switch; (5) пара 60×3000 — не acceptance gate, не запускать; (6) v1.4: разделять «доказано (семантика)» и «измерено (смешанная вертикаль 2e09a2f3)».
⚙️ **Equivalence gate (7 вопросов, живое чтение pattern_detector.py 136 строк целиком):** детектор читает только source_id/effect_value/observation_weight (event_type — Hard Guard игнорирует); агрегаты n/Σ(weighted)/variance/sign_flips; окна нет (MIN_EVENTS абсолютен), decay нет; порядок критичен (flips по парам, последовательный float-Σ); текущий EvidenceOfPersistence (3 поля) достаточной статистикой НЕ является → наивный watermark отвергнут на гейте. **Достаточный state сконструирован:** n, cumulative (последовательный float — воспроизведение sum() Ноймайера CPython 3.12+), Σx/Σx² (Fraction — exactness contract, statistics.variance эталон), last_sign (из исходного float через copysign — Fraction теряет знак −0.0), sign_flips, first_seen (order contract) + last_seen_tick (ветка 2: tail-read, tick-монотонность L1-записей верифицирована по 9 писателям, нарушение → RuntimeError громко).
⚙️ **Ловушки машинной эквивалентности, пойманные oracle'ом (каждая — оплаченный урок):** (1) −0.0: Fraction(-0.0) теряет знак, copysign(1,−0.0)=−1 — знак из исходного float; (2) sum() в CPython 3.12+ компенсирован (Ноймайер) — наивный += дал 0.6 vs 0.6000000000000001; (3) PBT вне домена effect_value [−1,1] (identity_events:37) ронял float-конверсию statistics на 5.4e308 — домен стратегии = домен события. Oracle: побитовые ==, approx запрещён (Taboo).
⚙️ **Реализация:** pattern_state.py (чистая математика, без I/O; сериализация Fraction=(num,den), round-trip exact, ключ-обёртка _META от коллизии), врезка integration.py:409-434 за env-флагом PATTERN_WATERMARK (default OFF = прежний путь байт-в-байт; state персистится scene_state → Фаза 10 atomic commit, прецедент EpistemicStore S193), тесты test_pattern_watermark_oracle.py (11). Гейты: oracle 11/11, существующие pattern-тесты GREEN, IPT 45/45.
⚙️ **A/B 24×10k (Опция 1: контроль OFF + candidate ON на одном дереве 2e09a2f3):** p50 137.5→123.5 (−10%), **p95 230.6 RED → 154.9 YELLOW (−33%)**, L1-поток/	events/dialogues идентичны, WS ≈0. ON-исполнение верифицировано 150024 WM-принтами (== query_raw count; tail→0 поздними визитами, last_tick→10000). **History-gradient после ~3000 тиков в ON исчез; кривая 110–125 ms/tick** (одна кривая, не закон). 60×3000 — отложенная SCALE-клетка. C_llm-ограничение не требовалось: watermark не зависит от LLM-профиля (v1.3).
⚠️ Инциденты: (1) серия INVALID-вставок продолжалась — приём жёстко по §3.12; (2) **чужой stash pop'нулся в наше дерево** (stash@{0} чужой ветки Потребности_3 — после неудачного нашего push невалидный pop применил ЧУЖОЙ stash; 5 unmerged (логи/данные/старый док) разрешены в пользу HEAD, чужие stash'и не тронуты, наш код цел); (3) пара 1 отбракована дважды (population-омиссия в OFF; затем ложная тревога «+9 чужих строк в integration.py» — сдвиг нашими же патч-строками; **урок: после врезки сверять содержимое, не память о номерах строк**); (4) NameError-класс (import math скобкой в плане не применился) и PBT double-tuple — пойманы прогонами, дешёвые; (5) 41 pre-existing красный pytest классифицирован выборкой на чистом дереве (стash-метод) — чужие домены (life_engine/needs/sqlite/opportunity), не блокируют watermark.
