...
⚙️ (4) B4 проводка: _phase_0_6 оркестратора — bed_ok/settled → резолвер → сервис (сигнатура +Optional eligibility, обратно совместима). Ко-локация: W3-shadow-блок (их ADR-O-373 Gate-1) в том же методе — мерж-точка двух сессий, не тронут (git-дифф: ровно два hunk, построчная граница чиста).
...
### S237: W3 Object FSM — Execution Substrate + Gate-1 Shadow (ADR-O-373) | ✅ IPT 45/45, W-сьюты 101+1skip, GORAN β G1 GREEN+ambient
