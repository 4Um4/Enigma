...
### S211: Vertical Slice, слой 3 — Характеры (R7: Epistemic Dispositions) | ✅ IPT 44/44
🎯 R7-монокультура WARN мертва: один belief → разные действия по натуре. Borko доносит (report=1.89), Lusya сплетничает (spread_rumor=1.755), Goran предупреждает (warn=1.89, паритет прежней эры), Торнин обсуждает, thief узнаёт — и молчит.⚙️ Доменная таблица EPISTEMIC_DISPOSITIONS (архетип → веса интентов; калибруема; табу npc_id-хардкодов); to_modifiers(+archetype, обратно совместим: без архетипа — байт-в-байт S198-легаси); проброс profile_l0.archetype в pipeline:496 (единственный живой вызов). Оси НЕ смешаны: _DUTY_TABLE (reaction_priority) — вертикальная (реакции на события, порядок говорения); dispositions — горизонтальная (действия по убеждению). SUPERBOX-DISPOSITIONS 6/6 (доминирование по архетипу, молчание thief, легаси-паритет, 4 словаря + детерминизм). Регресс-полк: AGENCY-STEAL 6/6, PERCEPTION-TOPOLOGY 4/4, IPT 44/44.📁 dom/epistemic_dispositions (новый), svc/npc/epistemic_context_resolver, svc/npc/npc_tick_pipeline, tests/sandbox/SUPERBOX/scenarios/epistemic_dispositions_test
