...
class PerceptionArchitectureLinter:
    ...
    def lint(self):
        ...
        arch_data = self._load_yaml(os.path.join(self.arch_dir, "perception_architecture.yaml"))
        if arch_data:
        ...
        man_data = self._load_yaml(os.path.join(self.arch_dir, "manifestation_signals.yaml"))
        if man_data:
        ...
        fact_data = self._load_yaml(os.path.join(self.arch_dir, "observed_fact_types.yaml"))
        if fact_data:
    ...
    def _check_causal_loop(self, data: Dict):
        ...
        for edge in edges:
            ...
            if src in forbidden_sources and dst in forbidden_targets:
                self.errors.append(f"CAUSAL LOOP VIOLATION: Edge from {src} to {dst} is forbidden (Invariant 2).")
    ...
    def _check_consumer_isolation(self, data: Dict):
        ...
        for edge in edges:
            ...
            if src in forbidden_sources and dst in consumer_names:
                self.errors.append(f"CONSUMER ISOLATION VIOLATION: Consumer '{dst}' reads directly from '{src}' (Invariant 3).")
    ...
    def _check_observation_relation_purity(self, data: Dict):
        ...
        found_forbidden = [kw for kw in forbidden_keywords if kw in desc and "запрещено" not in desc.split(kw)[0][-20:]]
        ...
        if not all(kw in desc and "запрещено" in desc for kw in forbidden_keywords):
             self.warnings.append("Invariant 4 (Observation Purity): Ensure description explicitly forbids NPC id, Faction, Mood, Memory.")
    ...
    def _check_manifestation_immutability(self, data: Dict):
        ...
        for ch_name, ch_data in channels.items():
            ...
            if upstream != "Reality":
                self.errors.append(f"MANIFESTATION BRIDGE VIOLATION: Channel '{ch_name}' has upstream '{upstream}', must be 'Reality' (Invariant 5).")
    ...
    def _check_no_resolution_in_manifestation(self, data: Dict):
        ...
        for ch_name, ch_data in channels.items():
            if "requires_resolution" in ch_data:
                self.errors.append(f"RESOLUTION LEAK: Channel '{ch_name}' has 'requires_resolution'. This belongs to PerceptionPhysics, not Manifestation.")
    ...
    def _check_atomic_facts(self, data: Dict):
        ...
        composite_patterns = [
            r"hand_on_weapon", r"avoiding_eye_contact", r"staring_at_player", 
            r"trembling", r"posture_tense", r"fleeing", r"speaking_fast"
        ]
            ...
            for fact_name in facts.keys():
                for pattern in composite_patterns:
                    if re.match(pattern, fact_name, re.IGNORECASE):
                        self.errors.append(f"ATOMIC FACT VIOLATION: Fact '{fact_name}' in '{category}' is composite, not atomic (§17.2).")
            ...
            if isinstance(possible_causes, dict): # Если это словарь с priors
                for cause, priors in possible_causes.items():
                    if isinstance(priors, dict) and "prior" in priors:
                        self.errors.append(f"AUTHORING PRIOR VIOLATION: Signal '{signal}' cause '{cause}' contains static 'prior'. Priors must be dynamic.")
    ...
    def _report(self):
        ...
        if self.errors:
            ...
            for e in self.errors:
                print(f"❌ {e}")
            print("\n" + "=" * 60)
            print("🚨 LINTING FAILED: Architecture contracts violate Epistemology Invariants.")
            print("=" * 60)
            sys.exit(1)
        else:
            print("\n" + "=" * 60)
            print("✅ LINTING PASSED: All perception architecture invariants are intact.")
            print("=" * 60)
            sys.exit(0)
