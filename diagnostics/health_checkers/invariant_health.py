...
class InvariantHealthChecker:
    ...
    def on_tick_complete(self, tick: int, game_time_seconds: float,
                         decisions_count: int, verbal_intents_count: int,
                         npc_moved_count: int) -> None:
        self._window.append({
            'tick': tick,
    ...
    def on_sim_integrity(self, invariant_id: str, file: str, line: int) -> None:
        self._runtime_violations.append(InvariantViolation(
    ...
    def _check_invariants(self) -> None:
        ...
        if self._window[-1]['game_time'] <= self._window[0]['game_time']:
            self._add_postmortem(InvariantViolation(
                invariant_id="INV-TIME-FREEZE",
                severity="CRITICAL",
                source="POST-MORTEM",
                message=(f"game_time_seconds не изменился за {len(self._window)} тиков: "
        ...
        if total_verbal > 0 and total_dialogues == 0:
            self._add_postmortem(InvariantViolation(
                ...
                severity="CRITICAL",
                source="POST-MORTEM",
                message=(f"За {len(self._window)} тиков было {total_verbal} вербальных интентов, "
    ...
    def _add_postmortem(self, v: InvariantViolation) -> None:
        if not any(x.invariant_id == v.invariant_id and x.source == "POST-MORTEM"
